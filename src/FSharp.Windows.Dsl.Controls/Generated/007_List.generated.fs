...
type ListProp =
    | Base of BlockProp
    | MarkerStyle of System.Windows.TextMarkerStyle
    | MarkerOffset of float
    | StartIndex of int

module List =
    let markerStyle v : obj = box (ListProp.MarkerStyle v)
    let markerOffset v : obj = box (ListProp.MarkerOffset v)
    let startIndex v : obj = box (ListProp.StartIndex v)

    let apply (el: System.Windows.Documents.List) (prop: ListProp) =
        match prop with
        | ListProp.Base p -> Block.apply el p
        | ListProp.MarkerStyle v -> el.SetValue(System.Windows.Documents.List.MarkerStyleProperty, box v)
        | ListProp.MarkerOffset v -> el.SetValue(System.Windows.Documents.List.MarkerOffsetProperty, box v)
        | ListProp.StartIndex v -> el.SetValue(System.Windows.Documents.List.StartIndexProperty, box v)
    ...
    let listItems (cs: VirtualNode list) : obj = box (CollectionProp("ListItems", cs))

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props

        { Type = typeof<System.Windows.Documents.List>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
