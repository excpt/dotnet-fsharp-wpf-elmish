...
type RowDefinitionProp =
    | Base of DefinitionBaseProp
    | Height of System.Windows.GridLength
    | MinHeight of float
    | MaxHeight of float

module RowDefinition =
    let height v : obj = box (RowDefinitionProp.Height v)
    let minHeight v : obj = box (RowDefinitionProp.MinHeight v)
    let maxHeight v : obj = box (RowDefinitionProp.MaxHeight v)

    let apply (el: System.Windows.Controls.RowDefinition) (prop: RowDefinitionProp) =
        match prop with
        | RowDefinitionProp.Base p -> DefinitionBase.apply el p
        | RowDefinitionProp.Height v -> el.SetValue(System.Windows.Controls.RowDefinition.HeightProperty, box v)
        | RowDefinitionProp.MinHeight v -> el.SetValue(System.Windows.Controls.RowDefinition.MinHeightProperty, box v)
        | RowDefinitionProp.MaxHeight v -> el.SetValue(System.Windows.Controls.RowDefinition.MaxHeightProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Controls.RowDefinition>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
