...
type CanvasProp = Base of PanelProp
...
module Canvas =
    ...
    let left (v: float) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Controls.Canvas.LeftProperty, box v))
                :: node.Props }

    let top (v: float) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Controls.Canvas.TopProperty, box v))
                :: node.Props }

    let right (v: float) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Controls.Canvas.RightProperty, box v))
                :: node.Props }

    let bottom (v: float) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Controls.Canvas.BottomProperty, box v))
                :: node.Props }
