...
module Canvas =
    ...
    let apply (el: System.Windows.Controls.Canvas) (prop: CanvasProp) =
        match prop with
        | CanvasProp.Base p -> Panel.apply el p
    ...
    let left (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(System.Windows.Controls.Canvas.LeftProperty, box v)) :: node.Props }
    let top (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(System.Windows.Controls.Canvas.TopProperty, box v)) :: node.Props }
    let right (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(System.Windows.Controls.Canvas.RightProperty, box v)) :: node.Props }
    let bottom (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(System.Windows.Controls.Canvas.BottomProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<System.Windows.Controls.Canvas>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
