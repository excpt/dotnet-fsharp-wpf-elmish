...
type BlockProp =
    ...
    | BreakPageBefore of bool
    | BreakColumnBefore of bool
    | ClearFloaters of System.Windows.WrapDirection

module Block =
    let margin v : obj = box (BlockProp.Margin v)
    let padding v : obj = box (BlockProp.Padding v)
    let borderThickness v : obj = box (BlockProp.BorderThickness v)
    let borderBrush v : obj = box (BlockProp.BorderBrush v)
    let flowDirection v : obj = box (BlockProp.FlowDirection v)
    let breakPageBefore v : obj = box (BlockProp.BreakPageBefore v)
    let breakColumnBefore v : obj = box (BlockProp.BreakColumnBefore v)
    let clearFloaters v : obj = box (BlockProp.ClearFloaters v)

    let apply (el: System.Windows.Documents.Block) (prop: BlockProp) =
        match prop with
        | BlockProp.Base p -> TextElement.apply el p
        | BlockProp.Margin v -> el.SetValue(System.Windows.Documents.Block.MarginProperty, box v)
        | BlockProp.Padding v -> el.SetValue(System.Windows.Documents.Block.PaddingProperty, box v)
        | BlockProp.BorderThickness v -> el.SetValue(System.Windows.Documents.Block.BorderThicknessProperty, box v)
        | BlockProp.BorderBrush v -> el.SetValue(System.Windows.Documents.Block.BorderBrushProperty, box v)
        | BlockProp.FlowDirection v -> el.SetValue(System.Windows.Documents.Block.FlowDirectionProperty, box v)
        | BlockProp.BreakPageBefore v -> el.SetValue(System.Windows.Documents.Block.BreakPageBeforeProperty, box v)
        | BlockProp.BreakColumnBefore v -> el.SetValue(System.Windows.Documents.Block.BreakColumnBeforeProperty, box v)
        | BlockProp.ClearFloaters v -> el.SetValue(System.Windows.Documents.Block.ClearFloatersProperty, box v)
    ...
    let isHyphenationEnabled (v: bool) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Documents.Block.IsHyphenationEnabledProperty, box v))
                :: node.Props }

    let textAlignment (v: System.Windows.TextAlignment) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Documents.Block.TextAlignmentProperty, box v))
                :: node.Props }

    let lineHeight (v: float) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Documents.Block.LineHeightProperty, box v))
                :: node.Props }

    let lineStackingStrategy (v: System.Windows.LineStackingStrategy) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Documents.Block.LineStackingStrategyProperty, box v))
                :: node.Props }

    let siblingBlocks (cs: VirtualNode list) : obj =
        box (CollectionProp("SiblingBlocks", cs))
