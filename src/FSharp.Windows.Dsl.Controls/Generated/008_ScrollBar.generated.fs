...
module ScrollBar =
    ...
    let onScroll v : obj =
        box (EventProp(box (ScrollBarProp.OnScroll v)))

    let apply (el: System.Windows.Controls.Primitives.ScrollBar) (prop: ScrollBarProp) =
        match prop with
        | ScrollBarProp.Base p -> RangeBase.apply el p
        | ScrollBarProp.Orientation v ->
            el.SetValue(System.Windows.Controls.Primitives.ScrollBar.OrientationProperty, box v)
        | ScrollBarProp.ViewportSize v ->
            el.SetValue(System.Windows.Controls.Primitives.ScrollBar.ViewportSizeProperty, box v)
