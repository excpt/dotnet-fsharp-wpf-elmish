...
type ColumnDefinitionProp =
    | Base of DefinitionBaseProp
    | Width of System.Windows.GridLength
    | MinWidth of float
    | MaxWidth of float

module ColumnDefinition =
    let width v : obj = box (ColumnDefinitionProp.Width v)
    let minWidth v : obj = box (ColumnDefinitionProp.MinWidth v)
    let maxWidth v : obj = box (ColumnDefinitionProp.MaxWidth v)

    let apply (el: System.Windows.Controls.ColumnDefinition) (prop: ColumnDefinitionProp) =
        match prop with
        | ColumnDefinitionProp.Base p -> DefinitionBase.apply el p
        | ColumnDefinitionProp.Width v -> el.SetValue(System.Windows.Controls.ColumnDefinition.WidthProperty, box v)
        | ColumnDefinitionProp.MinWidth v ->
            el.SetValue(System.Windows.Controls.ColumnDefinition.MinWidthProperty, box v)
        | ColumnDefinitionProp.MaxWidth v ->
            el.SetValue(System.Windows.Controls.ColumnDefinition.MaxWidthProperty, box v)

    let sharedSizeGroup v : obj =
        box (DefinitionBaseProp.SharedSizeGroup v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Controls.ColumnDefinition>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
