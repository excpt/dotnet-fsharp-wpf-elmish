...
module Panel =
    ...
    let apply (el: System.Windows.Controls.Panel) (prop: PanelProp) =
        match prop with
        | PanelProp.Base p -> FrameworkElement.apply el p
    ...
    let zIndex (v: int) (node: VirtualNode) : VirtualNode =
        { node with
            Props =
                box (AttachedProp(System.Windows.Controls.Panel.ZIndexProperty, box v))
                :: node.Props }
