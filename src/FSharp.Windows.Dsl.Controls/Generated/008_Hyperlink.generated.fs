...
type HyperlinkProp =
    ...
    | NavigateUri of System.Uri
    | TargetName of string
    | OnRequestNavigate of System.Windows.Navigation.RequestNavigateEventHandler
    | OnClick of System.Windows.RoutedEventHandler

module Hyperlink =
    let command v : obj = box (HyperlinkProp.Command v)
    let commandParameter v : obj = box (HyperlinkProp.CommandParameter v)
    let commandTarget v : obj = box (HyperlinkProp.CommandTarget v)
    let navigateUri v : obj = box (HyperlinkProp.NavigateUri v)
    let targetName v : obj = box (HyperlinkProp.TargetName v)

    let onRequestNavigate v : obj =
        box (EventProp(box (HyperlinkProp.OnRequestNavigate v)))

    let onClick v : obj =
        box (EventProp(box (HyperlinkProp.OnClick v)))

    let apply (el: System.Windows.Documents.Hyperlink) (prop: HyperlinkProp) =
        match prop with
        | HyperlinkProp.Base p -> Inline.apply el p
        | HyperlinkProp.Command v -> el.SetValue(System.Windows.Documents.Hyperlink.CommandProperty, box v)
        | HyperlinkProp.CommandParameter v ->
            el.SetValue(System.Windows.Documents.Hyperlink.CommandParameterProperty, box v)
        | HyperlinkProp.CommandTarget v -> el.SetValue(System.Windows.Documents.Hyperlink.CommandTargetProperty, box v)
        | HyperlinkProp.NavigateUri v -> el.SetValue(System.Windows.Documents.Hyperlink.NavigateUriProperty, box v)
        | HyperlinkProp.TargetName v -> el.SetValue(System.Windows.Documents.Hyperlink.TargetNameProperty, box v)
        | HyperlinkProp.OnRequestNavigate h -> el.RequestNavigate.AddHandler(h)
        | HyperlinkProp.OnClick h -> el.Click.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Documents.Hyperlink>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
