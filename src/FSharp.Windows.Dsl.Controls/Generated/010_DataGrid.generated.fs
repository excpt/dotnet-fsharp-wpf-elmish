...
type DataGridProp =
    ...
    | OnInitializingNewItem of System.Windows.Controls.InitializingNewItemEventHandler
    | OnSelectedCellsChanged of System.Windows.Controls.SelectedCellsChangedEventHandler
    | OnSorting of System.Windows.Controls.DataGridSortingEventHandler
    | OnAutoGeneratedColumns of System.EventHandler
...
module DataGrid =
    ...
    let onInitializingNewItem v : obj =
        box (EventProp(box (DataGridProp.OnInitializingNewItem v)))

    let onSelectedCellsChanged v : obj =
        box (EventProp(box (DataGridProp.OnSelectedCellsChanged v)))

    let onSorting v : obj =
        box (EventProp(box (DataGridProp.OnSorting v)))

    let onAutoGeneratedColumns v : obj =
        box (EventProp(box (DataGridProp.OnAutoGeneratedColumns v)))

    let apply (el: System.Windows.Controls.DataGrid) (prop: DataGridProp) =
        match prop with
        | DataGridProp.Base p -> Selector.apply el p
        ...
        | DataGridProp.OnInitializingNewItem h -> el.InitializingNewItem.AddHandler(h)
        | DataGridProp.OnSelectedCellsChanged h -> el.SelectedCellsChanged.AddHandler(h)
        | DataGridProp.OnSorting h -> el.Sorting.AddHandler(h)
        | DataGridProp.OnAutoGeneratedColumns h -> el.AutoGeneratedColumns.AddHandler(h)
