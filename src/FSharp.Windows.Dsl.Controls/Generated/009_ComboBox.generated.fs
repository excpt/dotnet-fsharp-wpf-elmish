...
type ComboBoxProp =
    ...
    | OnDropDownOpened of System.EventHandler
    | OnDropDownClosed of System.EventHandler
...
module ComboBox =
    ...
    let shouldPreserveUserEnteredPrefix v : obj =
        box (ComboBoxProp.ShouldPreserveUserEnteredPrefix v)
    ...
    let onDropDownOpened v : obj =
        box (EventProp(box (ComboBoxProp.OnDropDownOpened v)))

    let onDropDownClosed v : obj =
        box (EventProp(box (ComboBoxProp.OnDropDownClosed v)))

    let apply (el: System.Windows.Controls.ComboBox) (prop: ComboBoxProp) =
        match prop with
        | ComboBoxProp.Base p -> Selector.apply el p
        | ComboBoxProp.MaxDropDownHeight v ->
            el.SetValue(System.Windows.Controls.ComboBox.MaxDropDownHeightProperty, box v)
        | ComboBoxProp.IsDropDownOpen v -> el.SetValue(System.Windows.Controls.ComboBox.IsDropDownOpenProperty, box v)
        | ComboBoxProp.ShouldPreserveUserEnteredPrefix v ->
            el.SetValue(System.Windows.Controls.ComboBox.ShouldPreserveUserEnteredPrefixProperty, box v)
        ...
        | ComboBoxProp.OnDropDownOpened h -> el.DropDownOpened.AddHandler(h)
        | ComboBoxProp.OnDropDownClosed h -> el.DropDownClosed.AddHandler(h)
