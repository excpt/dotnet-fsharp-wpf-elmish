...
type ScrollViewerProp =
    | Base of ContentControlProp
    | OnScrollChanged of System.Windows.Controls.ScrollChangedEventHandler

module ScrollViewer =
    let onScrollChanged v : obj = box (EventProp(box (ScrollViewerProp.OnScrollChanged v)))

    let apply (el: System.Windows.Controls.ScrollViewer) (prop: ScrollViewerProp) =
        match prop with
        | ScrollViewerProp.Base p -> ContentControl.apply el p
        | ScrollViewerProp.OnScrollChanged h -> el.ScrollChanged.AddHandler(h)

    let content v : obj = box (ContentControlProp.Content v)
    let contentTemplate v : obj = box (ContentControlProp.ContentTemplate v)
    let contentTemplateSelector v : obj = box (ContentControlProp.ContentTemplateSelector v)
    let contentStringFormat v : obj = box (ContentControlProp.ContentStringFormat v)
