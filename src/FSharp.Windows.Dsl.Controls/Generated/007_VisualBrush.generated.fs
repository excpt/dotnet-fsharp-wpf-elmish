...
type VisualBrushProp =
    | Base of TileBrushProp
    | Visual of System.Windows.Media.Visual
    | AutoLayoutContent of bool

module VisualBrush =
    let visual v : obj = box (VisualBrushProp.Visual v)

    let autoLayoutContent v : obj =
        box (VisualBrushProp.AutoLayoutContent v)

    let apply (el: System.Windows.Media.VisualBrush) (prop: VisualBrushProp) =
        match prop with
        | VisualBrushProp.Base p -> TileBrush.apply el p
        | VisualBrushProp.Visual v -> el.SetValue(System.Windows.Media.VisualBrush.VisualProperty, box v)
        | VisualBrushProp.AutoLayoutContent v ->
            el.SetValue(System.Windows.Media.VisualBrush.AutoLayoutContentProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Media.VisualBrush>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
