...
module MediaElement =
    ...
    let onMediaOpened v : obj = box (EventProp(box (MediaElementProp.OnMediaOpened v)))
    let onBufferingStarted v : obj = box (EventProp(box (MediaElementProp.OnBufferingStarted v)))
    let onBufferingEnded v : obj = box (EventProp(box (MediaElementProp.OnBufferingEnded v)))
    let onMediaEnded v : obj = box (EventProp(box (MediaElementProp.OnMediaEnded v)))

    let apply (el: System.Windows.Controls.MediaElement) (prop: MediaElementProp) =
        match prop with
        | MediaElementProp.Base p -> FrameworkElement.apply el p
