...
module ContextMenu =
    let horizontalOffset v : obj =
        box (ContextMenuProp.HorizontalOffset v)
    ...
    let placementRectangle v : obj =
        box (ContextMenuProp.PlacementRectangle v)

    let placement v : obj = box (ContextMenuProp.Placement v)
    let hasDropShadow v : obj = box (ContextMenuProp.HasDropShadow v)

    let customPopupPlacementCallback v : obj =
        box (ContextMenuProp.CustomPopupPlacementCallback v)

    let staysOpen v : obj = box (ContextMenuProp.StaysOpen v)

    let onOpened v : obj =
        box (EventProp(box (ContextMenuProp.OnOpened v)))

    let onClosed v : obj =
        box (EventProp(box (ContextMenuProp.OnClosed v)))

    let apply (el: System.Windows.Controls.ContextMenu) (prop: ContextMenuProp) =
        match prop with
        | ContextMenuProp.Base p -> MenuBase.apply el p
        | ContextMenuProp.HorizontalOffset v ->
            el.SetValue(System.Windows.Controls.ContextMenu.HorizontalOffsetProperty, box v)
        | ContextMenuProp.VerticalOffset v ->
        ...
        | ContextMenuProp.HasDropShadow v ->
            el.SetValue(System.Windows.Controls.ContextMenu.HasDropShadowProperty, box v)
        | ContextMenuProp.CustomPopupPlacementCallback v ->
            el.SetValue(System.Windows.Controls.ContextMenu.CustomPopupPlacementCallbackProperty, box v)
    ...
    let itemContainerTemplateSelector v : obj =
        box (MenuBaseProp.ItemContainerTemplateSelector v)

    let usesItemContainerTemplate v : obj =
        box (MenuBaseProp.UsesItemContainerTemplate v)
