...
module TableColumn =
    let width v : obj = box (TableColumnProp.Width v)
    let background v : obj = box (TableColumnProp.Background v)

    let apply (el: System.Windows.Documents.TableColumn) (prop: TableColumnProp) =
        match prop with
        | TableColumnProp.Base p -> FrameworkContentElement.apply el p
        | TableColumnProp.Width v -> el.SetValue(System.Windows.Documents.TableColumn.WidthProperty, box v)
        | TableColumnProp.Background v -> el.SetValue(System.Windows.Documents.TableColumn.BackgroundProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Documents.TableColumn>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
