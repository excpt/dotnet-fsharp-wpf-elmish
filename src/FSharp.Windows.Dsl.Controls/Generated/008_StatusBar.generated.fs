...
module StatusBar =
    let itemContainerTemplateSelector v : obj =
        box (StatusBarProp.ItemContainerTemplateSelector v)

    let usesItemContainerTemplate v : obj =
        box (StatusBarProp.UsesItemContainerTemplate v)

    let apply (el: System.Windows.Controls.Primitives.StatusBar) (prop: StatusBarProp) =
        match prop with
        | StatusBarProp.Base p -> ItemsControl.apply el p
        | StatusBarProp.ItemContainerTemplateSelector v ->
            el.SetValue(System.Windows.Controls.Primitives.StatusBar.ItemContainerTemplateSelectorProperty, box v)
        | StatusBarProp.UsesItemContainerTemplate v ->
            el.SetValue(System.Windows.Controls.Primitives.StatusBar.UsesItemContainerTemplateProperty, box v)
