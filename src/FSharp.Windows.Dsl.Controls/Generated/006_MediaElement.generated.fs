...
module MediaElement =
    ...
    let scrubbingEnabled v : obj =
        box (MediaElementProp.ScrubbingEnabled v)

    let unloadedBehavior v : obj =
        box (MediaElementProp.UnloadedBehavior v)

    let loadedBehavior v : obj = box (MediaElementProp.LoadedBehavior v)
    let stretch v : obj = box (MediaElementProp.Stretch v)

    let stretchDirection v : obj =
        box (MediaElementProp.StretchDirection v)

    let onMediaOpened v : obj =
        box (EventProp(box (MediaElementProp.OnMediaOpened v)))

    let onBufferingStarted v : obj =
        box (EventProp(box (MediaElementProp.OnBufferingStarted v)))

    let onBufferingEnded v : obj =
        box (EventProp(box (MediaElementProp.OnBufferingEnded v)))

    let onMediaEnded v : obj =
        box (EventProp(box (MediaElementProp.OnMediaEnded v)))
    ...
    let apply (el: System.Windows.Controls.MediaElement) (prop: MediaElementProp) =
        ...
        | MediaElementProp.ScrubbingEnabled v ->
            el.SetValue(System.Windows.Controls.MediaElement.ScrubbingEnabledProperty, box v)
        | MediaElementProp.UnloadedBehavior v ->
            el.SetValue(System.Windows.Controls.MediaElement.UnloadedBehaviorProperty, box v)
        | MediaElementProp.LoadedBehavior v ->
            el.SetValue(System.Windows.Controls.MediaElement.LoadedBehaviorProperty, box v)
        | MediaElementProp.Stretch v -> el.SetValue(System.Windows.Controls.MediaElement.StretchProperty, box v)
        | MediaElementProp.StretchDirection v ->
            el.SetValue(System.Windows.Controls.MediaElement.StretchDirectionProperty, box v)
