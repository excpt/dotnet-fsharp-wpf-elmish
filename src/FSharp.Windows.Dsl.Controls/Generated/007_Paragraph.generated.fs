...
type ParagraphProp =
    ...
    | TextIndent of float
    | MinOrphanLines of int
    | MinWidowLines of int
    | KeepWithNext of bool
    | KeepTogether of bool

module Paragraph =
    let textDecorations v : obj = box (ParagraphProp.TextDecorations v)
    let textIndent v : obj = box (ParagraphProp.TextIndent v)
    let minOrphanLines v : obj = box (ParagraphProp.MinOrphanLines v)
    let minWidowLines v : obj = box (ParagraphProp.MinWidowLines v)
    let keepWithNext v : obj = box (ParagraphProp.KeepWithNext v)
    let keepTogether v : obj = box (ParagraphProp.KeepTogether v)

    let apply (el: System.Windows.Documents.Paragraph) (prop: ParagraphProp) =
        match prop with
        | ParagraphProp.Base p -> Block.apply el p
        | ParagraphProp.TextDecorations v ->
            el.SetValue(System.Windows.Documents.Paragraph.TextDecorationsProperty, box v)
        | ParagraphProp.TextIndent v -> el.SetValue(System.Windows.Documents.Paragraph.TextIndentProperty, box v)
        | ParagraphProp.MinOrphanLines v ->
            el.SetValue(System.Windows.Documents.Paragraph.MinOrphanLinesProperty, box v)
        | ParagraphProp.MinWidowLines v -> el.SetValue(System.Windows.Documents.Paragraph.MinWidowLinesProperty, box v)
        | ParagraphProp.KeepWithNext v -> el.SetValue(System.Windows.Documents.Paragraph.KeepWithNextProperty, box v)
        | ParagraphProp.KeepTogether v -> el.SetValue(System.Windows.Documents.Paragraph.KeepTogetherProperty, box v)
    ...
    let inlines (cs: VirtualNode list) : obj = box (CollectionProp("Inlines", cs))

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props

        { Type = typeof<System.Windows.Documents.Paragraph>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
