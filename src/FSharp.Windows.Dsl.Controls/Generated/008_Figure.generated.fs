...
type FigureProp =
    | Base of AnchoredBlockProp
    | HorizontalAnchor of System.Windows.FigureHorizontalAnchor
    | VerticalAnchor of System.Windows.FigureVerticalAnchor
    | HorizontalOffset of float
    | VerticalOffset of float
    | CanDelayPlacement of bool
    | WrapDirection of System.Windows.WrapDirection
    | Width of System.Windows.FigureLength
    | Height of System.Windows.FigureLength

module Figure =
    let horizontalAnchor v : obj = box (FigureProp.HorizontalAnchor v)
    let verticalAnchor v : obj = box (FigureProp.VerticalAnchor v)
    let horizontalOffset v : obj = box (FigureProp.HorizontalOffset v)
    let verticalOffset v : obj = box (FigureProp.VerticalOffset v)
    let canDelayPlacement v : obj = box (FigureProp.CanDelayPlacement v)
    let wrapDirection v : obj = box (FigureProp.WrapDirection v)
    let width v : obj = box (FigureProp.Width v)
    let height v : obj = box (FigureProp.Height v)

    let apply (el: System.Windows.Documents.Figure) (prop: FigureProp) =
        match prop with
        | FigureProp.Base p -> AnchoredBlock.apply el p
        | FigureProp.HorizontalAnchor v -> el.SetValue(System.Windows.Documents.Figure.HorizontalAnchorProperty, box v)
        | FigureProp.VerticalAnchor v -> el.SetValue(System.Windows.Documents.Figure.VerticalAnchorProperty, box v)
        | FigureProp.HorizontalOffset v -> el.SetValue(System.Windows.Documents.Figure.HorizontalOffsetProperty, box v)
        | FigureProp.VerticalOffset v -> el.SetValue(System.Windows.Documents.Figure.VerticalOffsetProperty, box v)
        | FigureProp.CanDelayPlacement v ->
            el.SetValue(System.Windows.Documents.Figure.CanDelayPlacementProperty, box v)
        | FigureProp.WrapDirection v -> el.SetValue(System.Windows.Documents.Figure.WrapDirectionProperty, box v)
        | FigureProp.Width v -> el.SetValue(System.Windows.Documents.Figure.WidthProperty, box v)
        | FigureProp.Height v -> el.SetValue(System.Windows.Documents.Figure.HeightProperty, box v)

    let margin v : obj = box (AnchoredBlockProp.Margin v)
    let padding v : obj = box (AnchoredBlockProp.Padding v)

    let borderThickness v : obj =
        box (AnchoredBlockProp.BorderThickness v)

    let borderBrush v : obj = box (AnchoredBlockProp.BorderBrush v)
    let textAlignment v : obj = box (AnchoredBlockProp.TextAlignment v)
    let lineHeight v : obj = box (AnchoredBlockProp.LineHeight v)

    let lineStackingStrategy v : obj =
        box (AnchoredBlockProp.LineStackingStrategy v)

    let baselineAlignment v : obj = box (InlineProp.BaselineAlignment v)
    let textDecorations v : obj = box (InlineProp.TextDecorations v)
    let flowDirection v : obj = box (InlineProp.FlowDirection v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Documents.Figure>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
