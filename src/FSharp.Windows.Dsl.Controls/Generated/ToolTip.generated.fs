...
module ToolTip =
    ...
    let onOpened v : obj = box (EventProp(box (ToolTipProp.OnOpened v)))
    let onClosed v : obj = box (EventProp(box (ToolTipProp.OnClosed v)))

    let apply (el: System.Windows.Controls.ToolTip) (prop: ToolTipProp) =
        match prop with
        | ToolTipProp.Base p -> ContentControl.apply el p
