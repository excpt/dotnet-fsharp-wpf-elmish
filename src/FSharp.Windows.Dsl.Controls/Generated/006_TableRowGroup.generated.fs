...
type TableRowGroupProp = Base of TextElementProp

module TableRowGroup =

    let apply (el: System.Windows.Documents.TableRowGroup) (prop: TableRowGroupProp) =
        match prop with
        | TableRowGroupProp.Base p -> TextElement.apply el p
    ...
    let rows (cs: VirtualNode list) : obj = box (CollectionProp("Rows", cs))

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props

        { Type = typeof<System.Windows.Documents.TableRowGroup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
