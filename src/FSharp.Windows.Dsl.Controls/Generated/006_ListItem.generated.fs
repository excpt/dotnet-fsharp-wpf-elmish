...
module ListItem =
    let margin v : obj = box (ListItemProp.Margin v)
    let padding v : obj = box (ListItemProp.Padding v)
    let borderThickness v : obj = box (ListItemProp.BorderThickness v)
    let borderBrush v : obj = box (ListItemProp.BorderBrush v)
    let textAlignment v : obj = box (ListItemProp.TextAlignment v)
    let flowDirection v : obj = box (ListItemProp.FlowDirection v)
    let lineHeight v : obj = box (ListItemProp.LineHeight v)

    let lineStackingStrategy v : obj =
        box (ListItemProp.LineStackingStrategy v)

    let apply (el: System.Windows.Documents.ListItem) (prop: ListItemProp) =
        match prop with
        | ListItemProp.Base p -> TextElement.apply el p
        | ListItemProp.Margin v -> el.SetValue(System.Windows.Documents.ListItem.MarginProperty, box v)
        | ListItemProp.Padding v -> el.SetValue(System.Windows.Documents.ListItem.PaddingProperty, box v)
        | ListItemProp.BorderThickness v ->
            el.SetValue(System.Windows.Documents.ListItem.BorderThicknessProperty, box v)
        | ListItemProp.BorderBrush v -> el.SetValue(System.Windows.Documents.ListItem.BorderBrushProperty, box v)
        | ListItemProp.TextAlignment v -> el.SetValue(System.Windows.Documents.ListItem.TextAlignmentProperty, box v)
        | ListItemProp.FlowDirection v -> el.SetValue(System.Windows.Documents.ListItem.FlowDirectionProperty, box v)
        | ListItemProp.LineHeight v -> el.SetValue(System.Windows.Documents.ListItem.LineHeightProperty, box v)
        | ListItemProp.LineStackingStrategy v ->
            el.SetValue(System.Windows.Documents.ListItem.LineStackingStrategyProperty, box v)
    ...
    let siblingListItems (cs: VirtualNode list) : obj =
        box (CollectionProp("SiblingListItems", cs))

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props

        { Type = typeof<System.Windows.Documents.ListItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
