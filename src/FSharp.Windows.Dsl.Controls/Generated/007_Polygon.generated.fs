...
module Polygon =
    let points v : obj = box (PolygonProp.Points v)
    let fillRule v : obj = box (PolygonProp.FillRule v)

    let apply (el: System.Windows.Shapes.Polygon) (prop: PolygonProp) =
        match prop with
        | PolygonProp.Base p -> Shape.apply el p
        | PolygonProp.Points v -> el.SetValue(System.Windows.Shapes.Polygon.PointsProperty, box v)
        | PolygonProp.FillRule v -> el.SetValue(System.Windows.Shapes.Polygon.FillRuleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Shapes.Polygon>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
