...
module TreeViewItem =
    ...
    let isSelectionActive v : obj =
        box (TreeViewItemProp.IsSelectionActive v)

    let onExpanded v : obj =
        box (EventProp(box (TreeViewItemProp.OnExpanded v)))

    let onCollapsed v : obj =
        box (EventProp(box (TreeViewItemProp.OnCollapsed v)))

    let onSelected v : obj =
        box (EventProp(box (TreeViewItemProp.OnSelected v)))

    let onUnselected v : obj =
        box (EventProp(box (TreeViewItemProp.OnUnselected v)))
    ...
    let apply (el: System.Windows.Controls.TreeViewItem) (prop: TreeViewItemProp) =
        ...
        | TreeViewItemProp.IsSelectionActive v ->
            el.SetValue(System.Windows.Controls.TreeViewItem.IsSelectionActiveProperty, box v)
