...
module Polyline =
    let points v : obj = box (PolylineProp.Points v)
    let fillRule v : obj = box (PolylineProp.FillRule v)

    let apply (el: System.Windows.Shapes.Polyline) (prop: PolylineProp) =
        match prop with
        | PolylineProp.Base p -> Shape.apply el p
        | PolylineProp.Points v -> el.SetValue(System.Windows.Shapes.Polyline.PointsProperty, box v)
        | PolylineProp.FillRule v -> el.SetValue(System.Windows.Shapes.Polyline.FillRuleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<System.Windows.Shapes.Polyline>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
