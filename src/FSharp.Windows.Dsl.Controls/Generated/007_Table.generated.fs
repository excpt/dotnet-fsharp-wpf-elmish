...
module Table =
    let cellSpacing v : obj = box (TableProp.CellSpacing v)

    let apply (el: System.Windows.Documents.Table) (prop: TableProp) =
        match prop with
        | TableProp.Base p -> Block.apply el p
        | TableProp.CellSpacing v -> el.SetValue(System.Windows.Documents.Table.CellSpacingProperty, box v)
    ...
    let rowGroups (cs: VirtualNode list) : obj = box (CollectionProp("RowGroups", cs))

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props

        { Type = typeof<System.Windows.Documents.Table>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
