...
type PopupProp =
    ...
    | OnOpened of System.EventHandler
...
module Popup =
    ...
    let customPopupPlacementCallback v : obj =
        box (PopupProp.CustomPopupPlacementCallback v)
    ...
    let onOpened v : obj =
        box (EventProp(box (PopupProp.OnOpened v)))

    let onClosed v : obj =
        box (EventProp(box (PopupProp.OnClosed v)))
    ...
    let apply (el: System.Windows.Controls.Primitives.Popup) (prop: PopupProp) =
        ...
        | PopupProp.CustomPopupPlacementCallback v ->
            el.SetValue(System.Windows.Controls.Primitives.Popup.CustomPopupPlacementCallbackProperty, box v)
        | PopupProp.StaysOpen v -> el.SetValue(System.Windows.Controls.Primitives.Popup.StaysOpenProperty, box v)
        | PopupProp.HorizontalOffset v ->
            el.SetValue(System.Windows.Controls.Primitives.Popup.HorizontalOffsetProperty, box v)
        ...
        | PopupProp.PopupAnimation v ->
            el.SetValue(System.Windows.Controls.Primitives.Popup.PopupAnimationProperty, box v)
        | PopupProp.AllowsTransparency v ->
            el.SetValue(System.Windows.Controls.Primitives.Popup.AllowsTransparencyProperty, box v)
        | PopupProp.OnOpened h -> el.Opened.AddHandler(h)
        | PopupProp.OnClosed h -> el.Closed.AddHandler(h)
