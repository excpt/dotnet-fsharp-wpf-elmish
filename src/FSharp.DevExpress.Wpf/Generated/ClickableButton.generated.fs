...
module ClickableButton =
    let onClick v : obj = box (EventProp(box (ClickableButtonProp.OnClick v)))

    let apply (el: DevExpress.Xpf.Controls.Internal.ClickableButton) (prop: ClickableButtonProp) =
        match prop with
        | ClickableButtonProp.OnClick h -> el.Click.AddHandler(h)
