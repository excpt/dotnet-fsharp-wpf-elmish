...
module ScaleBreak =
    let edge1 v : obj = box (ScaleBreakProp.Edge1 v)
    let edge2 v : obj = box (ScaleBreakProp.Edge2 v)
    let visible v : obj = box (ScaleBreakProp.Visible v)

    let apply (el: DevExpress.Xpf.Charts.ScaleBreak) (prop: ScaleBreakProp) =
        match prop with
        | ScaleBreakProp.Base p -> ChartElement.apply el p
        | ScaleBreakProp.Edge1 v -> el.SetValue(DevExpress.Xpf.Charts.ScaleBreak.Edge1Property, box v)
        | ScaleBreakProp.Edge2 v -> el.SetValue(DevExpress.Xpf.Charts.ScaleBreak.Edge2Property, box v)
        | ScaleBreakProp.Visible v -> el.SetValue(DevExpress.Xpf.Charts.ScaleBreak.VisibleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.ScaleBreak>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
