...
type ChartMirrorControlProp =
    | Base of ChartElementBaseProp
    | Series of System.Windows.UIElement
    | MirrorBrush of System.Windows.Media.Brush
    | MirrorOpacityBrush of System.Windows.Media.Brush
    | IsPseudo3D of bool

module ChartMirrorControl =
    let series v : obj = box (ChartMirrorControlProp.Series v)
    let mirrorBrush v : obj = box (ChartMirrorControlProp.MirrorBrush v)
    let mirrorOpacityBrush v : obj = box (ChartMirrorControlProp.MirrorOpacityBrush v)
    let isPseudo3D v : obj = box (ChartMirrorControlProp.IsPseudo3D v)

    let apply (el: DevExpress.Xpf.Charts.ChartMirrorControl) (prop: ChartMirrorControlProp) =
        match prop with
        | ChartMirrorControlProp.Base p -> ChartElementBase.apply el p
        | ChartMirrorControlProp.Series v -> el.SetValue(DevExpress.Xpf.Charts.ChartMirrorControl.SeriesProperty, box v)
        | ChartMirrorControlProp.MirrorBrush v -> el.SetValue(DevExpress.Xpf.Charts.ChartMirrorControl.MirrorBrushProperty, box v)
        | ChartMirrorControlProp.MirrorOpacityBrush v -> el.SetValue(DevExpress.Xpf.Charts.ChartMirrorControl.MirrorOpacityBrushProperty, box v)
        | ChartMirrorControlProp.IsPseudo3D v -> el.SetValue(DevExpress.Xpf.Charts.ChartMirrorControl.IsPseudo3DProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.ChartMirrorControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
