...
type MapSearchPanelProp =
    | IsBusy of bool
    | FixedWidth of bool

module MapSearchPanel =
    let isBusy v : obj = box (MapSearchPanelProp.IsBusy v)
    let fixedWidth v : obj = box (MapSearchPanelProp.FixedWidth v)

    let apply (el: DevExpress.Xpf.Map.MapSearchPanel) (prop: MapSearchPanelProp) =
        match prop with
        | MapSearchPanelProp.IsBusy v -> el.SetValue(DevExpress.Xpf.Map.MapSearchPanel.IsBusyProperty, box v)
        | MapSearchPanelProp.FixedWidth v -> el.SetValue(DevExpress.Xpf.Map.MapSearchPanel.FixedWidthProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.MapSearchPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
