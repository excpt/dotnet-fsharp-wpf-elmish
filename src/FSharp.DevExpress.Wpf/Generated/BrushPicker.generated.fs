...
type BrushPickerProp =
    | Brush of obj
    | BrushType of DevExpress.Xpf.Editors.BrushType
    | EditMode of DevExpress.Xpf.Editors.EditMode

module BrushPicker =
    let brush v : obj = box (BrushPickerProp.Brush v)
    let brushType v : obj = box (BrushPickerProp.BrushType v)
    let editMode v : obj = box (BrushPickerProp.EditMode v)

    let apply (el: DevExpress.Xpf.Editors.Internal.BrushPicker) (prop: BrushPickerProp) =
        match prop with
        | BrushPickerProp.Brush v -> el.SetValue(DevExpress.Xpf.Editors.Internal.BrushPicker.BrushProperty, box v)
        | BrushPickerProp.BrushType v -> el.SetValue(DevExpress.Xpf.Editors.Internal.BrushPicker.BrushTypeProperty, box v)
        | BrushPickerProp.EditMode v -> el.SetValue(DevExpress.Xpf.Editors.Internal.BrushPicker.EditModeProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.Internal.BrushPicker>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
