...
type SimpleDiagram2DProp =
    | Base of Diagram2DProp
    | Dimension of int
    | LayoutDirection of DevExpress.Xpf.Charts.LayoutDirection
    | Elements of obj

module SimpleDiagram2D =
    let dimension v : obj = box (SimpleDiagram2DProp.Dimension v)
    let layoutDirection v : obj = box (SimpleDiagram2DProp.LayoutDirection v)
    let elements v : obj = box (SimpleDiagram2DProp.Elements v)

    let apply (el: DevExpress.Xpf.Charts.SimpleDiagram2D) (prop: SimpleDiagram2DProp) =
        match prop with
        | SimpleDiagram2DProp.Base p -> Diagram2D.apply el p
        | SimpleDiagram2DProp.Dimension v -> el.SetValue(DevExpress.Xpf.Charts.SimpleDiagram2D.DimensionProperty, box v)
        | SimpleDiagram2DProp.LayoutDirection v -> el.SetValue(DevExpress.Xpf.Charts.SimpleDiagram2D.LayoutDirectionProperty, box v)
        | SimpleDiagram2DProp.Elements v -> el.SetValue(DevExpress.Xpf.Charts.SimpleDiagram2D.ElementsProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.SimpleDiagram2D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
