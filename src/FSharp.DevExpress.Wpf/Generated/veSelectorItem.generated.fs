...
type veSelectorItemProp =
    | Base of veContentControlProp
    | IsSelected of bool

module veSelectorItem =
    let isSelected v : obj = box (veSelectorItemProp.IsSelected v)

    let apply (el: DevExpress.Xpf.WindowsUI.Base.veSelectorItem) (prop: veSelectorItemProp) =
        match prop with
        | veSelectorItemProp.Base p -> veContentControl.apply el p
        | veSelectorItemProp.IsSelected v -> el.SetValue(DevExpress.Xpf.WindowsUI.Base.veSelectorItem.IsSelectedProperty, box v)
