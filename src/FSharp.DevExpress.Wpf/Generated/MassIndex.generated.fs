...
type MassIndexProp =
    ...
    | MovingAveragePointsCount of int
    | SumPointsCount of int

module MassIndex =
    let movingAveragePointsCount v : obj = box (MassIndexProp.MovingAveragePointsCount v)
    let sumPointsCount v : obj = box (MassIndexProp.SumPointsCount v)

    let apply (el: DevExpress.Xpf.Charts.MassIndex) (prop: MassIndexProp) =
        match prop with
        | MassIndexProp.Base p -> SeparatePaneIndicatorWithPoints.apply el p
        | MassIndexProp.MovingAveragePointsCount v -> el.SetValue(DevExpress.Xpf.Charts.MassIndex.MovingAveragePointsCountProperty, box v)
        | MassIndexProp.SumPointsCount v -> el.SetValue(DevExpress.Xpf.Charts.MassIndex.SumPointsCountProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.MassIndex>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
