...
type CustomLegendItemProp =
    | Base of ChartNotificationElementProp
    | Text of string
    | MarkerTemplate of System.Windows.DataTemplate
    | MarkerBrush of System.Windows.Media.Brush

module CustomLegendItem =
    let text v : obj = box (CustomLegendItemProp.Text v)
    let markerTemplate v : obj = box (CustomLegendItemProp.MarkerTemplate v)
    let markerBrush v : obj = box (CustomLegendItemProp.MarkerBrush v)

    let apply (el: DevExpress.Xpf.Charts.CustomLegendItem) (prop: CustomLegendItemProp) =
        match prop with
        | CustomLegendItemProp.Base p -> ChartNotificationElement.apply el p
        | CustomLegendItemProp.Text v -> el.SetValue(DevExpress.Xpf.Charts.CustomLegendItem.TextProperty, box v)
        | CustomLegendItemProp.MarkerTemplate v -> el.SetValue(DevExpress.Xpf.Charts.CustomLegendItem.MarkerTemplateProperty, box v)
        | CustomLegendItemProp.MarkerBrush v -> el.SetValue(DevExpress.Xpf.Charts.CustomLegendItem.MarkerBrushProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.CustomLegendItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
