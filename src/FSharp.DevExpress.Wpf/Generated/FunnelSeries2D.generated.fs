...
type FunnelSeries2DProp =
    | Base of SeriesProp
    | Model of DevExpress.Xpf.Charts.Funnel2DModel
    | Border of DevExpress.Xpf.Charts.SeriesBorder
    | PointDistance of int
    | HeightToWidthRatio of float
    | HeightToWidthRatioAuto of bool
    | AlignToCenter of bool
    | PointAnimation of DevExpress.Xpf.Charts.Funnel2DSeriesPointAnimationBase
    | ColorEach of bool
    | Brush of System.Windows.Media.SolidColorBrush
    | PointShape of DevExpress.Xpf.Charts.FunnelPointShape

module FunnelSeries2D =
    let model v : obj = box (FunnelSeries2DProp.Model v)
    let border v : obj = box (FunnelSeries2DProp.Border v)
    let pointDistance v : obj = box (FunnelSeries2DProp.PointDistance v)
    let heightToWidthRatio v : obj = box (FunnelSeries2DProp.HeightToWidthRatio v)
    let heightToWidthRatioAuto v : obj = box (FunnelSeries2DProp.HeightToWidthRatioAuto v)
    let alignToCenter v : obj = box (FunnelSeries2DProp.AlignToCenter v)
    let pointAnimation v : obj = box (FunnelSeries2DProp.PointAnimation v)
    let colorEach v : obj = box (FunnelSeries2DProp.ColorEach v)
    let brush v : obj = box (FunnelSeries2DProp.Brush v)
    let pointShape v : obj = box (FunnelSeries2DProp.PointShape v)

    let apply (el: DevExpress.Xpf.Charts.FunnelSeries2D) (prop: FunnelSeries2DProp) =
        match prop with
        | FunnelSeries2DProp.Base p -> Series.apply el p
        | FunnelSeries2DProp.Model v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.ModelProperty, box v)
        | FunnelSeries2DProp.Border v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.BorderProperty, box v)
        | FunnelSeries2DProp.PointDistance v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.PointDistanceProperty, box v)
        | FunnelSeries2DProp.HeightToWidthRatio v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.HeightToWidthRatioProperty, box v)
        | FunnelSeries2DProp.HeightToWidthRatioAuto v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.HeightToWidthRatioAutoProperty, box v)
        | FunnelSeries2DProp.AlignToCenter v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.AlignToCenterProperty, box v)
        | FunnelSeries2DProp.PointAnimation v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.PointAnimationProperty, box v)
        | FunnelSeries2DProp.ColorEach v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.ColorEachProperty, box v)
        | FunnelSeries2DProp.Brush v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.BrushProperty, box v)
        | FunnelSeries2DProp.PointShape v -> el.SetValue(DevExpress.Xpf.Charts.FunnelSeries2D.PointShapeProperty, box v)
    ...
    let labelPosition (v: DevExpress.Xpf.Charts.Funnel2DLabelPosition) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.FunnelSeries2D.LabelPositionProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.FunnelSeries2D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
