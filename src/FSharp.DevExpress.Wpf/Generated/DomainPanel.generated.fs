...
type DomainPanelProp =
    | Base of SimplePanelProp
    | Pane of DevExpress.Xpf.Charts.Pane

module DomainPanel =
    let pane v : obj = box (DomainPanelProp.Pane v)

    let apply (el: DevExpress.Xpf.Charts.DomainPanel) (prop: DomainPanelProp) =
        match prop with
        | DomainPanelProp.Base p -> SimplePanel.apply el p
        | DomainPanelProp.Pane v -> el.SetValue(DevExpress.Xpf.Charts.DomainPanel.PaneProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.DomainPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
