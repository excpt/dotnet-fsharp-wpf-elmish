...
type WizardPageProp =
    ...
    | ShowFooter of bool
    | ShowNext of bool
    | ShowSideContent of bool
    | SideContent of obj
    | SideContentTemplate of System.Windows.DataTemplate
    | SideContentTemplateSelector of System.Windows.Controls.DataTemplateSelector
    | Title of string

module WizardPage =
    let allowBack v : obj = box (WizardPageProp.AllowBack v)
    let allowCancel v : obj = box (WizardPageProp.AllowCancel v)
    let allowFinish v : obj = box (WizardPageProp.AllowFinish v)
    let allowNext v : obj = box (WizardPageProp.AllowNext v)
    let headerBackground v : obj = box (WizardPageProp.HeaderBackground v)
    let nextPage v : obj = box (WizardPageProp.NextPage v)
    let previousPage v : obj = box (WizardPageProp.PreviousPage v)
    let showBack v : obj = box (WizardPageProp.ShowBack v)
    let showCancel v : obj = box (WizardPageProp.ShowCancel v)
    let showFinish v : obj = box (WizardPageProp.ShowFinish v)
    let showFooter v : obj = box (WizardPageProp.ShowFooter v)
    let showNext v : obj = box (WizardPageProp.ShowNext v)
    let showSideContent v : obj = box (WizardPageProp.ShowSideContent v)
    let sideContent v : obj = box (WizardPageProp.SideContent v)
    let sideContentTemplate v : obj = box (WizardPageProp.SideContentTemplate v)
    let sideContentTemplateSelector v : obj = box (WizardPageProp.SideContentTemplateSelector v)
    let title v : obj = box (WizardPageProp.Title v)

    let apply (el: DevExpress.Xpf.Controls.WizardPage) (prop: WizardPageProp) =
        match prop with
        | WizardPageProp.AllowBack v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.AllowBackProperty, box v)
        | WizardPageProp.AllowCancel v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.AllowCancelProperty, box v)
        | WizardPageProp.AllowFinish v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.AllowFinishProperty, box v)
        | WizardPageProp.AllowNext v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.AllowNextProperty, box v)
        | WizardPageProp.HeaderBackground v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.HeaderBackgroundProperty, box v)
        | WizardPageProp.NextPage v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.NextPageProperty, box v)
        | WizardPageProp.PreviousPage v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.PreviousPageProperty, box v)
        | WizardPageProp.ShowBack v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.ShowBackProperty, box v)
        | WizardPageProp.ShowCancel v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.ShowCancelProperty, box v)
        | WizardPageProp.ShowFinish v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.ShowFinishProperty, box v)
        | WizardPageProp.ShowFooter v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.ShowFooterProperty, box v)
        | WizardPageProp.ShowNext v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.ShowNextProperty, box v)
        | WizardPageProp.ShowSideContent v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.ShowSideContentProperty, box v)
        | WizardPageProp.SideContent v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.SideContentProperty, box v)
        | WizardPageProp.SideContentTemplate v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.SideContentTemplateProperty, box v)
        | WizardPageProp.SideContentTemplateSelector v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.SideContentTemplateSelectorProperty, box v)
        | WizardPageProp.Title v -> el.SetValue(DevExpress.Xpf.Controls.WizardPage.TitleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Controls.WizardPage>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
