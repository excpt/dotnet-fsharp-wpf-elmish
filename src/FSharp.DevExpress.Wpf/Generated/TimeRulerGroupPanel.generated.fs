...
type TimeRulerGroupPanelProp =
    | Base of GroupPanelBaseProp
    | TimeRulers of obj
    | TimeRulerSize of float
    | TimeRulerHeadersSize of float
    | CellsCount of float
    | TimeMarkerPosition of float
    | ScrollOffset of float
    | TimeMarkerStyle of System.Windows.Style
    | TimeMarkerVisibility of System.Windows.Visibility

module TimeRulerGroupPanel =
    let timeRulers v : obj = box (TimeRulerGroupPanelProp.TimeRulers v)
    let timeRulerSize v : obj = box (TimeRulerGroupPanelProp.TimeRulerSize v)
    let timeRulerHeadersSize v : obj = box (TimeRulerGroupPanelProp.TimeRulerHeadersSize v)
    let cellsCount v : obj = box (TimeRulerGroupPanelProp.CellsCount v)
    let timeMarkerPosition v : obj = box (TimeRulerGroupPanelProp.TimeMarkerPosition v)
    let scrollOffset v : obj = box (TimeRulerGroupPanelProp.ScrollOffset v)
    let timeMarkerStyle v : obj = box (TimeRulerGroupPanelProp.TimeMarkerStyle v)
    let timeMarkerVisibility v : obj = box (TimeRulerGroupPanelProp.TimeMarkerVisibility v)

    let apply (el: DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel) (prop: TimeRulerGroupPanelProp) =
        match prop with
        | TimeRulerGroupPanelProp.Base p -> GroupPanelBase.apply el p
        | TimeRulerGroupPanelProp.TimeRulers v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.TimeRulersProperty, box v)
        | TimeRulerGroupPanelProp.TimeRulerSize v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.TimeRulerSizeProperty, box v)
        | TimeRulerGroupPanelProp.TimeRulerHeadersSize v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.TimeRulerHeadersSizeProperty, box v)
        | TimeRulerGroupPanelProp.CellsCount v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.CellsCountProperty, box v)
        | TimeRulerGroupPanelProp.TimeMarkerPosition v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.TimeMarkerPositionProperty, box v)
        | TimeRulerGroupPanelProp.ScrollOffset v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.ScrollOffsetProperty, box v)
        | TimeRulerGroupPanelProp.TimeMarkerStyle v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.TimeMarkerStyleProperty, box v)
        | TimeRulerGroupPanelProp.TimeMarkerVisibility v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel.TimeMarkerVisibilityProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduling.Panels.TimeRulerGroupPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
