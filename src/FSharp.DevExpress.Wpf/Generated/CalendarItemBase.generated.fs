...
type CalendarItemBaseProp =
    | Text of string

module CalendarItemBase =
    let text v : obj = box (CalendarItemBaseProp.Text v)

    let apply (el: DevExpress.Xpf.Editors.RangeControl.CalendarItemBase) (prop: CalendarItemBaseProp) =
        match prop with
        | CalendarItemBaseProp.Text v -> el.SetValue(DevExpress.Xpf.Editors.RangeControl.CalendarItemBase.TextProperty, box v)
