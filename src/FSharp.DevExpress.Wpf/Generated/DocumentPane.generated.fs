...
type DocumentPaneProp =
    | Base of psvContentControlProp
    | TabbedTemplate of System.Windows.DataTemplate
    | MDITemplate of System.Windows.DataTemplate

module DocumentPane =
    let tabbedTemplate v : obj = box (DocumentPaneProp.TabbedTemplate v)
    let mDITemplate v : obj = box (DocumentPaneProp.MDITemplate v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.DocumentPane) (prop: DocumentPaneProp) =
        match prop with
        | DocumentPaneProp.Base p -> psvContentControl.apply el p
        | DocumentPaneProp.TabbedTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentPane.TabbedTemplateProperty, box v)
        | DocumentPaneProp.MDITemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentPane.MDITemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.DocumentPane>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
