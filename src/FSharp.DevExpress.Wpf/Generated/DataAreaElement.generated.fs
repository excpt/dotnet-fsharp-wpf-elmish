...
type DataAreaElementProp =
    | ValueItem of DevExpress.Xpf.PivotGrid.Internal.ScrollableAreaItemBase

module DataAreaElement =
    let valueItem v : obj = box (DataAreaElementProp.ValueItem v)

    let apply (el: DevExpress.Xpf.PivotGrid.DataAreaElement) (prop: DataAreaElementProp) =
        match prop with
        | DataAreaElementProp.ValueItem v -> el.SetValue(DevExpress.Xpf.PivotGrid.DataAreaElement.ValueItemProperty, box v)
