...
type DocumentGroupProp =
    | Base of TabbedGroupProp
    | MDIStyle of DevExpress.Xpf.Docking.MDIStyle
    | ClosePageButtonShowMode of DevExpress.Xpf.Docking.ClosePageButtonShowMode
    | ShowWhenEmpty of bool
    | TabbedGroupDisplayMode of DevExpress.Xpf.Docking.TabbedGroupDisplayMode

module DocumentGroup =
    let mDIStyle v : obj = box (DocumentGroupProp.MDIStyle v)
    let closePageButtonShowMode v : obj = box (DocumentGroupProp.ClosePageButtonShowMode v)
    let showWhenEmpty v : obj = box (DocumentGroupProp.ShowWhenEmpty v)
    let tabbedGroupDisplayMode v : obj = box (DocumentGroupProp.TabbedGroupDisplayMode v)

    let apply (el: DevExpress.Xpf.Docking.DocumentGroup) (prop: DocumentGroupProp) =
        match prop with
        | DocumentGroupProp.Base p -> TabbedGroup.apply el p
        | DocumentGroupProp.MDIStyle v -> el.SetValue(DevExpress.Xpf.Docking.DocumentGroup.MDIStyleProperty, box v)
        | DocumentGroupProp.ClosePageButtonShowMode v -> el.SetValue(DevExpress.Xpf.Docking.DocumentGroup.ClosePageButtonShowModeProperty, box v)
        | DocumentGroupProp.ShowWhenEmpty v -> el.SetValue(DevExpress.Xpf.Docking.DocumentGroup.ShowWhenEmptyProperty, box v)
        | DocumentGroupProp.TabbedGroupDisplayMode v -> el.SetValue(DevExpress.Xpf.Docking.DocumentGroup.TabbedGroupDisplayModeProperty, box v)

    let showTabForSinglePage v : obj = box (TabbedGroupProp.ShowTabForSinglePage v)
    ...
    let onSelectedItemChanged v : obj = box (EventProp(box (LayoutGroupProp.OnSelectedItemChanged v)))
    ...
    let pinLocation (v: DevExpress.Xpf.Layout.Core.TabHeaderPinLocation) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentGroup.PinLocationProperty, box v)) :: node.Props }
    let pinned (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentGroup.PinnedProperty, box v)) :: node.Props }
    let showPinButton (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentGroup.ShowPinButtonProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Docking.DocumentGroup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
