...
type UniformStackPanelProp =
    | Base of PanelBaseProp
    | ChildSpacing of float
    | Orientation of System.Windows.Controls.Orientation

module UniformStackPanel =
    let childSpacing v : obj = box (UniformStackPanelProp.ChildSpacing v)
    let orientation v : obj = box (UniformStackPanelProp.Orientation v)

    let apply (el: DevExpress.Xpf.Core.UniformStackPanel) (prop: UniformStackPanelProp) =
        match prop with
        | UniformStackPanelProp.Base p -> PanelBase.apply el p
        | UniformStackPanelProp.ChildSpacing v -> el.SetValue(DevExpress.Xpf.Core.UniformStackPanel.ChildSpacingProperty, box v)
        | UniformStackPanelProp.Orientation v -> el.SetValue(DevExpress.Xpf.Core.UniformStackPanel.OrientationProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.UniformStackPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
