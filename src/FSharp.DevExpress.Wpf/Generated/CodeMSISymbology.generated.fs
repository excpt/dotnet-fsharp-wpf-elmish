...
type CodeMSISymbologyProp =
    | MSICheckSum of DevExpress.XtraPrinting.BarCode.MSICheckSum

module CodeMSISymbology =
    let mSICheckSum v : obj = box (CodeMSISymbologyProp.MSICheckSum v)

    let apply (el: DevExpress.Xpf.Controls.CodeMSISymbology) (prop: CodeMSISymbologyProp) =
        match prop with
        | CodeMSISymbologyProp.MSICheckSum v -> el.SetValue(DevExpress.Xpf.Controls.CodeMSISymbology.MSICheckSumProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Controls.CodeMSISymbology>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
