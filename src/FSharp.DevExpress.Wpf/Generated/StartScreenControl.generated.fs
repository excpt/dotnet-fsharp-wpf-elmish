...
type StartScreenControlProp =
    | RecentFiles of obj
    | NumberOfRecentFiles of int
    | ShowOpenFileButton of bool

module StartScreenControl =
    let recentFiles v : obj = box (StartScreenControlProp.RecentFiles v)
    let numberOfRecentFiles v : obj = box (StartScreenControlProp.NumberOfRecentFiles v)
    let showOpenFileButton v : obj = box (StartScreenControlProp.ShowOpenFileButton v)

    let apply (el: DevExpress.Xpf.PdfViewer.StartScreenControl) (prop: StartScreenControlProp) =
        match prop with
        | StartScreenControlProp.RecentFiles v -> el.SetValue(DevExpress.Xpf.PdfViewer.StartScreenControl.RecentFilesProperty, box v)
        | StartScreenControlProp.NumberOfRecentFiles v -> el.SetValue(DevExpress.Xpf.PdfViewer.StartScreenControl.NumberOfRecentFilesProperty, box v)
        | StartScreenControlProp.ShowOpenFileButton v -> el.SetValue(DevExpress.Xpf.PdfViewer.StartScreenControl.ShowOpenFileButtonProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PdfViewer.StartScreenControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
