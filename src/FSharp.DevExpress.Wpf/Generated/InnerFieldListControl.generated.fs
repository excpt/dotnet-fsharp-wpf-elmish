...
type InnerFieldListControlProp =
    | Base of FieldHeadersBaseProp
    | StandartTemplate of System.Windows.Controls.ControlTemplate
    | TreeViewTemplate of System.Windows.Controls.ControlTemplate
    | ShowBorder of bool
    | GroupFields of bool
    | ShowAll of bool

module InnerFieldListControl =
    let standartTemplate v : obj = box (InnerFieldListControlProp.StandartTemplate v)
    let treeViewTemplate v : obj = box (InnerFieldListControlProp.TreeViewTemplate v)
    let showBorder v : obj = box (InnerFieldListControlProp.ShowBorder v)
    let groupFields v : obj = box (InnerFieldListControlProp.GroupFields v)
    let showAll v : obj = box (InnerFieldListControlProp.ShowAll v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl) (prop: InnerFieldListControlProp) =
        match prop with
        | InnerFieldListControlProp.Base p -> FieldHeadersBase.apply el p
        | InnerFieldListControlProp.StandartTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.StandartTemplateProperty, box v)
        | InnerFieldListControlProp.TreeViewTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.TreeViewTemplateProperty, box v)
        | InnerFieldListControlProp.ShowBorder v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.ShowBorderProperty, box v)
        | InnerFieldListControlProp.GroupFields v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.GroupFieldsProperty, box v)
        | InnerFieldListControlProp.ShowAll v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.ShowAllProperty, box v)
    ...
    let fieldListStyle (v: DevExpress.Xpf.PivotGrid.FieldListStyle) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.FieldListStyleProperty, box v)) :: node.Props }
    let isInWindow (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl.IsInWindowProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.InnerFieldListControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
