...
type InnerCommentControlProp =
    | Base of RichEditControlProp

module InnerCommentControl =

    let apply (el: DevExpress.Xpf.RichEdit.InnerCommentControl) (prop: InnerCommentControlProp) =
        match prop with
        | InnerCommentControlProp.Base p -> RichEditControl.apply el p
    ...
    let isTemplatePart (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.RichEdit.InnerCommentControl.IsTemplatePartProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.RichEdit.InnerCommentControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
