...
type FloatGroupProp =
    ...
    | WindowStyle of System.Windows.Style

module FloatGroup =
    let isOpen v : obj = box (FloatGroupProp.IsOpen v)
    let floatLocation v : obj = box (FloatGroupProp.FloatLocation v)
    let windowStyle v : obj = box (FloatGroupProp.WindowStyle v)

    let apply (el: DevExpress.Xpf.Docking.FloatGroup) (prop: FloatGroupProp) =
        match prop with
        | FloatGroupProp.Base p -> LayoutGroup.apply el p
        | FloatGroupProp.IsOpen v -> el.SetValue(DevExpress.Xpf.Docking.FloatGroup.IsOpenProperty, box v)
        | FloatGroupProp.FloatLocation v -> el.SetValue(DevExpress.Xpf.Docking.FloatGroup.FloatLocationProperty, box v)
        | FloatGroupProp.WindowStyle v -> el.SetValue(DevExpress.Xpf.Docking.FloatGroup.WindowStyleProperty, box v)
    ...
    let floatState (v: DevExpress.Xpf.Docking.FloatState) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.FloatGroup.FloatStateProperty, box v)) :: node.Props }
    let sizeToContent (v: System.Windows.SizeToContent) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.FloatGroup.SizeToContentProperty, box v)) :: node.Props }
    let windowTaskbarIcon (v: System.Windows.Media.ImageSource) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.FloatGroup.WindowTaskbarIconProperty, box v)) :: node.Props }
    let windowTaskbarTitle (v: string) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.FloatGroup.WindowTaskbarTitleProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Docking.FloatGroup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
