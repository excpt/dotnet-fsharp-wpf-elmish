...
type AccordionPopupProp =
    | ActualContent of System.Windows.FrameworkElement
    | DefaultWidth of float

module AccordionPopup =
    let actualContent v : obj = box (AccordionPopupProp.ActualContent v)
    let defaultWidth v : obj = box (AccordionPopupProp.DefaultWidth v)

    let apply (el: DevExpress.Xpf.Accordion.AccordionPopup) (prop: AccordionPopupProp) =
        match prop with
        | AccordionPopupProp.ActualContent v -> el.SetValue(DevExpress.Xpf.Accordion.AccordionPopup.ActualContentProperty, box v)
        | AccordionPopupProp.DefaultWidth v -> el.SetValue(DevExpress.Xpf.Accordion.AccordionPopup.DefaultWidthProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Accordion.AccordionPopup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
