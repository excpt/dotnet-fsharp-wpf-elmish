...
module Splitter =
    let viewStyle v : obj = box (SplitterProp.ViewStyle v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.Splitter) (prop: SplitterProp) =
        match prop with
        | SplitterProp.Base p -> BaseSplitterControl.apply el p
        | SplitterProp.ViewStyle v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.Splitter.ViewStyleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.Splitter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
