...
type BaseSplitterItemProp =
    | Base of ThumbProp
    | DragIncrement of float
    | KeyboardIncrement of float
    | Orientation of System.Windows.Controls.Orientation

module BaseSplitterItem =
    let dragIncrement v : obj = box (BaseSplitterItemProp.DragIncrement v)
    let keyboardIncrement v : obj = box (BaseSplitterItemProp.KeyboardIncrement v)
    let orientation v : obj = box (BaseSplitterItemProp.Orientation v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.BaseSplitterItem) (prop: BaseSplitterItemProp) =
        match prop with
        | BaseSplitterItemProp.Base p -> Thumb.apply el p
        | BaseSplitterItemProp.DragIncrement v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseSplitterItem.DragIncrementProperty, box v)
        | BaseSplitterItemProp.KeyboardIncrement v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseSplitterItem.KeyboardIncrementProperty, box v)
        | BaseSplitterItemProp.Orientation v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseSplitterItem.OrientationProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.BaseSplitterItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
