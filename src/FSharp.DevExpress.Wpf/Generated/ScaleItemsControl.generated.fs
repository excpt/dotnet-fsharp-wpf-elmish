...
type ScaleItemsControlProp =
    | Scale of float

module ScaleItemsControl =
    let scale v : obj = box (ScaleItemsControlProp.Scale v)

    let apply (el: DevExpress.Xpf.Diagram.ScaleItemsControl) (prop: ScaleItemsControlProp) =
        match prop with
        | ScaleItemsControlProp.Scale v -> el.SetValue(DevExpress.Xpf.Diagram.ScaleItemsControl.ScaleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Diagram.ScaleItemsControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
