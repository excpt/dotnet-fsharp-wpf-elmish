...
type XPOSourceBaseWithSessionProp =
    | Base of XPOSourceBaseProp
    | Session of DevExpress.Xpo.Session

module XPOSourceBaseWithSession =
    let session v : obj = box (XPOSourceBaseWithSessionProp.Session v)

    let apply (el: DevExpress.Xpf.Core.DataSources.XPOSourceBaseWithSession) (prop: XPOSourceBaseWithSessionProp) =
        match prop with
        | XPOSourceBaseWithSessionProp.Base p -> XPOSourceBase.apply el p
        | XPOSourceBaseWithSessionProp.Session v -> el.SetValue(DevExpress.Xpf.Core.DataSources.XPOSourceBaseWithSession.SessionProperty, box v)
