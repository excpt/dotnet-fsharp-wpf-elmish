...
type DialogButtonProp =
    | CommandButtonStyle of System.Windows.Style
    | Command of System.Windows.Input.ICommand
    | IsCancel of obj
    | IsDefault of obj
    | DialogUICommandTag of obj
    | DialogResult of obj
    | CommandsSource of System.Collections.IEnumerable
    | Alignment of DevExpress.Mvvm.DialogButtonAlignment

module DialogButton =
    let commandButtonStyle v : obj = box (DialogButtonProp.CommandButtonStyle v)
    let command v : obj = box (DialogButtonProp.Command v)
    let isCancel v : obj = box (DialogButtonProp.IsCancel v)
    let isDefault v : obj = box (DialogButtonProp.IsDefault v)
    let dialogUICommandTag v : obj = box (DialogButtonProp.DialogUICommandTag v)
    let dialogResult v : obj = box (DialogButtonProp.DialogResult v)
    let commandsSource v : obj = box (DialogButtonProp.CommandsSource v)
    let alignment v : obj = box (DialogButtonProp.Alignment v)

    let apply (el: DevExpress.Xpf.Core.DialogButton) (prop: DialogButtonProp) =
        match prop with
        | DialogButtonProp.CommandButtonStyle v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.CommandButtonStyleProperty, box v)
        | DialogButtonProp.Command v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.CommandProperty, box v)
        | DialogButtonProp.IsCancel v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.IsCancelProperty, box v)
        | DialogButtonProp.IsDefault v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.IsDefaultProperty, box v)
        | DialogButtonProp.DialogUICommandTag v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.DialogUICommandTagProperty, box v)
        | DialogButtonProp.DialogResult v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.DialogResultProperty, box v)
        | DialogButtonProp.CommandsSource v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.CommandsSourceProperty, box v)
        | DialogButtonProp.Alignment v -> el.SetValue(DevExpress.Xpf.Core.DialogButton.AlignmentProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DialogButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
