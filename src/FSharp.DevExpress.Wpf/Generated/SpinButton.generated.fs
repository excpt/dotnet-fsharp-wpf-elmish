...
type SpinButtonProp =
    | StartInterval of int

module SpinButton =
    let startInterval v : obj = box (SpinButtonProp.StartInterval v)

    let apply (el: DevExpress.Xpf.Editors.SpinButton) (prop: SpinButtonProp) =
        match prop with
        | SpinButtonProp.StartInterval v -> el.SetValue(DevExpress.Xpf.Editors.SpinButton.StartIntervalProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.SpinButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
