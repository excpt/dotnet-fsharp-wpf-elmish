...
type PushpinTraceControlProp =
    | Item of DevExpress.Xpf.Map.MapItem
    | Visible of bool

module PushpinTraceControl =
    let item v : obj = box (PushpinTraceControlProp.Item v)
    let visible v : obj = box (PushpinTraceControlProp.Visible v)

    let apply (el: DevExpress.Xpf.Map.PushpinTraceControl) (prop: PushpinTraceControlProp) =
        match prop with
        | PushpinTraceControlProp.Item v -> el.SetValue(DevExpress.Xpf.Map.PushpinTraceControl.ItemProperty, box v)
        | PushpinTraceControlProp.Visible v -> el.SetValue(DevExpress.Xpf.Map.PushpinTraceControl.VisibleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.PushpinTraceControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
