...
type PagerControlNumericButtonProp =
    | Base of PagerControlNavigationButtonProp
    | Number of int
    | ShowEllipsis of bool

module PagerControlNumericButton =
    let number v : obj = box (PagerControlNumericButtonProp.Number v)
    let showEllipsis v : obj = box (PagerControlNumericButtonProp.ShowEllipsis v)

    let apply (el: DevExpress.Xpf.Editors.PagerControlNumericButton) (prop: PagerControlNumericButtonProp) =
        match prop with
        | PagerControlNumericButtonProp.Base p -> PagerControlNavigationButton.apply el p
        | PagerControlNumericButtonProp.Number v -> el.SetValue(DevExpress.Xpf.Editors.PagerControlNumericButton.NumberProperty, box v)
        | PagerControlNumericButtonProp.ShowEllipsis v -> el.SetValue(DevExpress.Xpf.Editors.PagerControlNumericButton.ShowEllipsisProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.PagerControlNumericButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
