...
module TileNavItem =
    let itemsSource v : obj = box (TileNavItemProp.ItemsSource v)
    let itemTemplate v : obj = box (TileNavItemProp.ItemTemplate v)
    let itemTemplateSelector v : obj = box (TileNavItemProp.ItemTemplateSelector v)
    let itemStyle v : obj = box (TileNavItemProp.ItemStyle v)

    let apply (el: DevExpress.Xpf.Navigation.TileNavItem) (prop: TileNavItemProp) =
        match prop with
        | TileNavItemProp.Base p -> NavElementBase.apply el p
        | TileNavItemProp.ItemsSource v -> el.SetValue(DevExpress.Xpf.Navigation.TileNavItem.ItemsSourceProperty, box v)
        | TileNavItemProp.ItemTemplate v -> el.SetValue(DevExpress.Xpf.Navigation.TileNavItem.ItemTemplateProperty, box v)
        | TileNavItemProp.ItemTemplateSelector v -> el.SetValue(DevExpress.Xpf.Navigation.TileNavItem.ItemTemplateSelectorProperty, box v)
        | TileNavItemProp.ItemStyle v -> el.SetValue(DevExpress.Xpf.Navigation.TileNavItem.ItemStyleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Navigation.TileNavItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
