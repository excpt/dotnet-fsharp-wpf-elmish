...
type ZAxis3DProp =
    | Base of Axis3DBaseProp
    | NumericScaleOptions of DevExpress.Xpf.Charts.ContinuousNumericScaleOptions

module ZAxis3D =
    let numericScaleOptions v : obj = box (ZAxis3DProp.NumericScaleOptions v)

    let apply (el: DevExpress.Xpf.Charts.ZAxis3D) (prop: ZAxis3DProp) =
        match prop with
        | ZAxis3DProp.Base p -> Axis3DBase.apply el p
        | ZAxis3DProp.NumericScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.ZAxis3D.NumericScaleOptionsProperty, box v)

    let crosshairAxisLabelOptions v : obj = box (Axis3DBaseProp.CrosshairAxisLabelOptions v)
    let crosshairLabelTemplate v : obj = box (Axis3DBaseProp.CrosshairLabelTemplate v)
    ...
    let alwaysShowZeroLevel (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.ZAxis3D.AlwaysShowZeroLevelProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.ZAxis3D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
