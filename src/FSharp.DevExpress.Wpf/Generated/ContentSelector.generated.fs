...
type ContentSelectorProp =
    | SelectedIndex of int
    | SnapperType of DevExpress.Xpf.Core.SnapperType
    | Content1 of System.Windows.UIElement
    | Content2 of System.Windows.UIElement

module ContentSelector =
    let selectedIndex v : obj = box (ContentSelectorProp.SelectedIndex v)
    let snapperType v : obj = box (ContentSelectorProp.SnapperType v)
    let content1 v : obj = box (ContentSelectorProp.Content1 v)
    let content2 v : obj = box (ContentSelectorProp.Content2 v)

    let apply (el: DevExpress.Xpf.Bars.ContentSelector) (prop: ContentSelectorProp) =
        match prop with
        | ContentSelectorProp.SelectedIndex v -> el.SetValue(DevExpress.Xpf.Bars.ContentSelector.SelectedIndexProperty, box v)
        | ContentSelectorProp.SnapperType v -> el.SetValue(DevExpress.Xpf.Bars.ContentSelector.SnapperTypeProperty, box v)
        | ContentSelectorProp.Content1 v -> el.SetValue(DevExpress.Xpf.Bars.ContentSelector.Content1Property, box v)
        | ContentSelectorProp.Content2 v -> el.SetValue(DevExpress.Xpf.Bars.ContentSelector.Content2Property, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.ContentSelector>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
