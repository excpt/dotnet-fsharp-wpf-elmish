...
type PLinqServerModeDataSourceProp =
...
module PLinqServerModeDataSource =
    let defaultSorting v : obj = box (PLinqServerModeDataSourceProp.DefaultSorting v)
    let elementType v : obj = box (PLinqServerModeDataSourceProp.ElementType v)
    let listSource v : obj = box (PLinqServerModeDataSourceProp.ListSource v)

    let apply (el: DevExpress.Xpf.Core.ServerMode.PLinqServerModeDataSource) (prop: PLinqServerModeDataSourceProp) =
        match prop with
        | PLinqServerModeDataSourceProp.Base p -> PLinqDataSourceBase.apply el p
        | PLinqServerModeDataSourceProp.DefaultSorting v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.PLinqServerModeDataSource.DefaultSortingProperty, box v)
        | PLinqServerModeDataSourceProp.ElementType v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.PLinqServerModeDataSource.ElementTypeProperty, box v)
        | PLinqServerModeDataSourceProp.ListSource v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.PLinqServerModeDataSource.ListSourceProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ServerMode.PLinqServerModeDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
