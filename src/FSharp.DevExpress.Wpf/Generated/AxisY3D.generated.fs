...
module AxisY3D =
    let dateTimeScaleOptions v : obj = box (AxisY3DProp.DateTimeScaleOptions v)
    let timeSpanScaleOptions v : obj = box (AxisY3DProp.TimeSpanScaleOptions v)
    let numericScaleOptions v : obj = box (AxisY3DProp.NumericScaleOptions v)

    let apply (el: DevExpress.Xpf.Charts.AxisY3D) (prop: AxisY3DProp) =
        match prop with
        | AxisY3DProp.Base p -> Axis3D.apply el p
        | AxisY3DProp.DateTimeScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.AxisY3D.DateTimeScaleOptionsProperty, box v)
        | AxisY3DProp.TimeSpanScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.AxisY3D.TimeSpanScaleOptionsProperty, box v)
        | AxisY3DProp.NumericScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.AxisY3D.NumericScaleOptionsProperty, box v)
    ...
    let alwaysShowZeroLevel (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.AxisY3D.AlwaysShowZeroLevelProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.AxisY3D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
