...
type HorizontalAppointmentsItemControlProp =
    | Base of AppointmentsItemControlProp

module HorizontalAppointmentsItemControl =

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.HorizontalAppointmentsItemControl) (prop: HorizontalAppointmentsItemControlProp) =
        match prop with
        | HorizontalAppointmentsItemControlProp.Base p -> AppointmentsItemControl.apply el p

    let view v : obj = box (AppointmentsItemControlProp.View v)
    ...
    let scrollOffset (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Scheduler.Drawing.HorizontalAppointmentsItemControl.ScrollOffsetProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.HorizontalAppointmentsItemControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
