...
type SplitterControlProp =
    | Base of BaseSplitterControlProp
    | LayoutItem of DevExpress.Xpf.Docking.BaseLayoutItem

module SplitterControl =
    let layoutItem v : obj = box (SplitterControlProp.LayoutItem v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.SplitterControl) (prop: SplitterControlProp) =
        match prop with
        | SplitterControlProp.Base p -> BaseSplitterControl.apply el p
        | SplitterControlProp.LayoutItem v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.SplitterControl.LayoutItemProperty, box v)

    let isDragDropOver v : obj = box (BaseSplitterControlProp.IsDragDropOver v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.SplitterControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
