...
type VisualDateCellProp =
    | Base of VisualTimeCellBaseProp
    | IsAlternate of bool
    | NormalTemplate of System.Windows.Controls.ControlTemplate
    | AlternateTemplate of System.Windows.Controls.ControlTemplate

module VisualDateCell =
    let isAlternate v : obj = box (VisualDateCellProp.IsAlternate v)
    let normalTemplate v : obj = box (VisualDateCellProp.NormalTemplate v)
    let alternateTemplate v : obj = box (VisualDateCellProp.AlternateTemplate v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.VisualDateCell) (prop: VisualDateCellProp) =
        match prop with
        | VisualDateCellProp.Base p -> VisualTimeCellBase.apply el p
        | VisualDateCellProp.IsAlternate v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.VisualDateCell.IsAlternateProperty, box v)
        | VisualDateCellProp.NormalTemplate v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.VisualDateCell.NormalTemplateProperty, box v)
        | VisualDateCellProp.AlternateTemplate v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.VisualDateCell.AlternateTemplateProperty, box v)

    let backgroundBrushSelector v : obj = box (VisualResourceCellBaseProp.BackgroundBrushSelector v)
    let borderBrushSelector v : obj = box (VisualResourceCellBaseProp.BorderBrushSelector v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.VisualDateCell>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
