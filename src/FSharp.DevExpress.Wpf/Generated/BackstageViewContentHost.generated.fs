...
type BackstageViewContentHostProp =
    | RightContent of obj
    | LeftContent of obj

module BackstageViewContentHost =
    let rightContent v : obj = box (BackstageViewContentHostProp.RightContent v)
    let leftContent v : obj = box (BackstageViewContentHostProp.LeftContent v)

    let apply (el: DevExpress.Xpf.Ribbon.BackstageViewContentHost) (prop: BackstageViewContentHostProp) =
        match prop with
        | BackstageViewContentHostProp.RightContent v -> el.SetValue(DevExpress.Xpf.Ribbon.BackstageViewContentHost.RightContentProperty, box v)
        | BackstageViewContentHostProp.LeftContent v -> el.SetValue(DevExpress.Xpf.Ribbon.BackstageViewContentHost.LeftContentProperty, box v)
    ...
    let animationTime (v: System.TimeSpan) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Ribbon.BackstageViewContentHost.AnimationTimeProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Ribbon.BackstageViewContentHost>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
