...
type ConstantLineTitleProp =
    | Base of AxisElementTitleBaseProp
    | Alignment of DevExpress.Xpf.Charts.ConstantLineTitleAlignment
    | ShowBelowLine of bool

module ConstantLineTitle =
    let alignment v : obj = box (ConstantLineTitleProp.Alignment v)
    let showBelowLine v : obj = box (ConstantLineTitleProp.ShowBelowLine v)

    let apply (el: DevExpress.Xpf.Charts.ConstantLineTitle) (prop: ConstantLineTitleProp) =
        match prop with
        | ConstantLineTitleProp.Base p -> AxisElementTitleBase.apply el p
        | ConstantLineTitleProp.Alignment v -> el.SetValue(DevExpress.Xpf.Charts.ConstantLineTitle.AlignmentProperty, box v)
        | ConstantLineTitleProp.ShowBelowLine v -> el.SetValue(DevExpress.Xpf.Charts.ConstantLineTitle.ShowBelowLineProperty, box v)

    let content v : obj = box (TitleBaseProp.Content v)
    let contentTemplate v : obj = box (TitleBaseProp.ContentTemplate v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.ConstantLineTitle>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
