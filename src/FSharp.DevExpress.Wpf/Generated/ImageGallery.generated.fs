...
type ImageGalleryProp =
    ...
    | MouseWheelScrollStep of float
    | AllowAnimation of bool

module ImageGallery =
    let itemsSource v : obj = box (ImageGalleryProp.ItemsSource v)
    let itemStyle v : obj = box (ImageGalleryProp.ItemStyle v)
    let mouseWheelScrollStep v : obj = box (ImageGalleryProp.MouseWheelScrollStep v)
    let allowAnimation v : obj = box (ImageGalleryProp.AllowAnimation v)

    let apply (el: DevExpress.Xpf.Core.Native.ImageGallery) (prop: ImageGalleryProp) =
        match prop with
        | ImageGalleryProp.ItemsSource v -> el.SetValue(DevExpress.Xpf.Core.Native.ImageGallery.ItemsSourceProperty, box v)
        | ImageGalleryProp.ItemStyle v -> el.SetValue(DevExpress.Xpf.Core.Native.ImageGallery.ItemStyleProperty, box v)
        | ImageGalleryProp.MouseWheelScrollStep v -> el.SetValue(DevExpress.Xpf.Core.Native.ImageGallery.MouseWheelScrollStepProperty, box v)
        | ImageGalleryProp.AllowAnimation v -> el.SetValue(DevExpress.Xpf.Core.Native.ImageGallery.AllowAnimationProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.Native.ImageGallery>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
