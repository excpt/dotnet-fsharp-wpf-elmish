...
type MapZoomTrackbarProp =
    | Position of float
    | ZoomLevel of float
    | MinZoomLevel of int
    | MaxZoomLevel of int
    | Command of System.Windows.Input.ICommand
    | ZoomingStep of float

module MapZoomTrackbar =
    let position v : obj = box (MapZoomTrackbarProp.Position v)
    let zoomLevel v : obj = box (MapZoomTrackbarProp.ZoomLevel v)
    let minZoomLevel v : obj = box (MapZoomTrackbarProp.MinZoomLevel v)
    let maxZoomLevel v : obj = box (MapZoomTrackbarProp.MaxZoomLevel v)
    let command v : obj = box (MapZoomTrackbarProp.Command v)
    let zoomingStep v : obj = box (MapZoomTrackbarProp.ZoomingStep v)

    let apply (el: DevExpress.Xpf.Map.MapZoomTrackbar) (prop: MapZoomTrackbarProp) =
        match prop with
        | MapZoomTrackbarProp.Position v -> el.SetValue(DevExpress.Xpf.Map.MapZoomTrackbar.PositionProperty, box v)
        | MapZoomTrackbarProp.ZoomLevel v -> el.SetValue(DevExpress.Xpf.Map.MapZoomTrackbar.ZoomLevelProperty, box v)
        | MapZoomTrackbarProp.MinZoomLevel v -> el.SetValue(DevExpress.Xpf.Map.MapZoomTrackbar.MinZoomLevelProperty, box v)
        | MapZoomTrackbarProp.MaxZoomLevel v -> el.SetValue(DevExpress.Xpf.Map.MapZoomTrackbar.MaxZoomLevelProperty, box v)
        | MapZoomTrackbarProp.Command v -> el.SetValue(DevExpress.Xpf.Map.MapZoomTrackbar.CommandProperty, box v)
        | MapZoomTrackbarProp.ZoomingStep v -> el.SetValue(DevExpress.Xpf.Map.MapZoomTrackbar.ZoomingStepProperty, box v)
