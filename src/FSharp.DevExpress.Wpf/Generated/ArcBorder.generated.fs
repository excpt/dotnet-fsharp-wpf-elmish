...
type ArcBorderProp =
    ...
    | TopPadding of float
    | Gap of float
    | InnerRadius of float

module ArcBorder =
    let thickness v : obj = box (ArcBorderProp.Thickness v)
    let topPadding v : obj = box (ArcBorderProp.TopPadding v)
    let gap v : obj = box (ArcBorderProp.Gap v)
    let innerRadius v : obj = box (ArcBorderProp.InnerRadius v)

    let apply (el: DevExpress.Xpf.Bars.ArcBorder) (prop: ArcBorderProp) =
        match prop with
        | ArcBorderProp.Thickness v -> el.SetValue(DevExpress.Xpf.Bars.ArcBorder.ThicknessProperty, box v)
        | ArcBorderProp.TopPadding v -> el.SetValue(DevExpress.Xpf.Bars.ArcBorder.TopPaddingProperty, box v)
        | ArcBorderProp.Gap v -> el.SetValue(DevExpress.Xpf.Bars.ArcBorder.GapProperty, box v)
        | ArcBorderProp.InnerRadius v -> el.SetValue(DevExpress.Xpf.Bars.ArcBorder.InnerRadiusProperty, box v)
    ...
    let clipToSectorBounds (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Bars.ArcBorder.ClipToSectorBoundsProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Bars.ArcBorder>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
