...
type PivotGridPopupMenuProp =
    | Base of GridPopupMenuBaseProp

module PivotGridPopupMenu =

    let apply (el: DevExpress.Xpf.PivotGrid.PivotGridPopupMenu) (prop: PivotGridPopupMenuProp) =
        match prop with
        | PivotGridPopupMenuProp.Base p -> GridPopupMenuBase.apply el p
    ...
    let gridMenuType (v: DevExpress.Xpf.PivotGrid.PivotGridMenuType) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.PivotGridPopupMenu.GridMenuTypeProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PivotGrid.PivotGridPopupMenu>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
