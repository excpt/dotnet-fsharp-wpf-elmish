...
type UpDownArrowButtonProp =
    | DownTemplate of System.Windows.Controls.ControlTemplate
    | IsUpOriented of bool
    | UpTemplate of System.Windows.Controls.ControlTemplate

module UpDownArrowButton =
    let downTemplate v : obj = box (UpDownArrowButtonProp.DownTemplate v)
    let isUpOriented v : obj = box (UpDownArrowButtonProp.IsUpOriented v)
    let upTemplate v : obj = box (UpDownArrowButtonProp.UpTemplate v)

    let apply (el: DevExpress.Xpf.Ribbon.UpDownArrowButton) (prop: UpDownArrowButtonProp) =
        match prop with
        | UpDownArrowButtonProp.DownTemplate v -> el.SetValue(DevExpress.Xpf.Ribbon.UpDownArrowButton.DownTemplateProperty, box v)
        | UpDownArrowButtonProp.IsUpOriented v -> el.SetValue(DevExpress.Xpf.Ribbon.UpDownArrowButton.IsUpOrientedProperty, box v)
        | UpDownArrowButtonProp.UpTemplate v -> el.SetValue(DevExpress.Xpf.Ribbon.UpDownArrowButton.UpTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Ribbon.UpDownArrowButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
