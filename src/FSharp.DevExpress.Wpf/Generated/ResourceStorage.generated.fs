...
type ResourceStorageProp =
    | Mappings of DevExpress.Xpf.Scheduler.ResourceMapping
    | ColorSaving of DevExpress.XtraScheduler.ColorSavingType

module ResourceStorage =
    let mappings v : obj = box (ResourceStorageProp.Mappings v)
    let colorSaving v : obj = box (ResourceStorageProp.ColorSaving v)

    let apply (el: DevExpress.Xpf.Scheduler.ResourceStorage) (prop: ResourceStorageProp) =
        match prop with
        | ResourceStorageProp.Mappings v -> el.SetValue(DevExpress.Xpf.Scheduler.ResourceStorage.MappingsProperty, box v)
        | ResourceStorageProp.ColorSaving v -> el.SetValue(DevExpress.Xpf.Scheduler.ResourceStorage.ColorSavingProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.ResourceStorage>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
