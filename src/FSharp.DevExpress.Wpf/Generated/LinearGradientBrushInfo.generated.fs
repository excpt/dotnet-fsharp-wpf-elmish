...
type LinearGradientBrushInfoProp =
    | Container of DevExpress.Xpf.Scheduler.Drawing.GraphicResourceContainer

module LinearGradientBrushInfo =
    let container v : obj = box (LinearGradientBrushInfoProp.Container v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.LinearGradientBrushInfo) (prop: LinearGradientBrushInfoProp) =
        match prop with
        | LinearGradientBrushInfoProp.Container v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.LinearGradientBrushInfo.ContainerProperty, box v)
    ...
    let gradientStopColorId (v: string) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Scheduler.Drawing.LinearGradientBrushInfo.GradientStopColorIdProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.LinearGradientBrushInfo>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
