...
type RadialContextMenuProp =
    | Base of PopupMenuProp
    | Glyph of System.Windows.Media.ImageSource
    | BackButtonGlyph of System.Windows.Media.ImageSource
    | MinSectorCount of int
    | AutoExpand of obj
    | FirstSectorIndex of int
    | GlyphTemplate of System.Windows.DataTemplate
    | BackButtonGlyphTemplate of System.Windows.DataTemplate

module RadialContextMenu =
    let glyph v : obj = box (RadialContextMenuProp.Glyph v)
    let backButtonGlyph v : obj = box (RadialContextMenuProp.BackButtonGlyph v)
    let minSectorCount v : obj = box (RadialContextMenuProp.MinSectorCount v)
    let autoExpand v : obj = box (RadialContextMenuProp.AutoExpand v)
    let firstSectorIndex v : obj = box (RadialContextMenuProp.FirstSectorIndex v)
    let glyphTemplate v : obj = box (RadialContextMenuProp.GlyphTemplate v)
    let backButtonGlyphTemplate v : obj = box (RadialContextMenuProp.BackButtonGlyphTemplate v)

    let apply (el: DevExpress.Xpf.Bars.RadialContextMenu) (prop: RadialContextMenuProp) =
        match prop with
        | RadialContextMenuProp.Base p -> PopupMenu.apply el p
        | RadialContextMenuProp.Glyph v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.GlyphProperty, box v)
        | RadialContextMenuProp.BackButtonGlyph v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.BackButtonGlyphProperty, box v)
        | RadialContextMenuProp.MinSectorCount v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.MinSectorCountProperty, box v)
        | RadialContextMenuProp.AutoExpand v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.AutoExpandProperty, box v)
        | RadialContextMenuProp.FirstSectorIndex v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.FirstSectorIndexProperty, box v)
        | RadialContextMenuProp.GlyphTemplate v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.GlyphTemplateProperty, box v)
        | RadialContextMenuProp.BackButtonGlyphTemplate v -> el.SetValue(DevExpress.Xpf.Bars.RadialContextMenu.BackButtonGlyphTemplateProperty, box v)

    let glyphSize v : obj = box (PopupMenuProp.GlyphSize v)
    let customGlyphSize v : obj = box (PopupMenuProp.CustomGlyphSize v)
    let itemLinksSource v : obj = box (PopupMenuProp.ItemLinksSource v)
    let itemTemplate v : obj = box (PopupMenuProp.ItemTemplate v)
    let itemStyle v : obj = box (PopupMenuProp.ItemStyle v)
    let itemStyleSelector v : obj = box (PopupMenuProp.ItemStyleSelector v)
    let itemTemplateSelector v : obj = box (PopupMenuProp.ItemTemplateSelector v)
    let showKeyGestures v : obj = box (PopupMenuProp.ShowKeyGestures v)
    let itemLinksSourceElementGeneratesUniqueBarItem v : obj = box (PopupMenuProp.ItemLinksSourceElementGeneratesUniqueBarItem v)
    let itemsDisplayMode v : obj = box (PopupMenuBaseProp.ItemsDisplayMode v)
    let stretchRows v : obj = box (PopupMenuBaseProp.StretchRows v)
    let maxRowCount v : obj = box (PopupMenuBaseProp.MaxRowCount v)
    let multiColumn v : obj = box (PopupMenuBaseProp.MultiColumn v)
    let maxColumnHeight v : obj = box (PopupMenuBaseProp.MaxColumnHeight v)
    let expandMode v : obj = box (BarPopupExpandableProp.ExpandMode v)
    let background v : obj = box (BarPopupExpandableProp.Background v)
    let borderBrush v : obj = box (BarPopupExpandableProp.BorderBrush v)
    let borderThickness v : obj = box (BarPopupExpandableProp.BorderThickness v)
    let padding v : obj = box (BarPopupExpandableProp.Padding v)
    let cornerRadius v : obj = box (BarPopupExpandableProp.CornerRadius v)
    let defaultVerticalOffset v : obj = box (BarPopupBaseProp.DefaultVerticalOffset v)
    let ignoreMenuDropAlignment v : obj = box (BarPopupBaseProp.IgnoreMenuDropAlignment v)
    let popupContent v : obj = box (PopupBaseProp.PopupContent v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.RadialContextMenu>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
