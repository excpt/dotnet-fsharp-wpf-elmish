...
type FieldHeadersProp =
    | Base of FieldHeadersBaseProp
    | ContentTemplate of System.Windows.Controls.ControlTemplate
    | LeftOffset of int
    | LeftPixelsOffset of float
    | MeasureMode of DevExpress.Xpf.PivotGrid.Internal.FieldHeadersMeasureMode
    | EmptyTextVAlignment of System.Windows.VerticalAlignment
    | EmptyTextHAlignment of System.Windows.HorizontalAlignment
    | EmptyTextVisibility of System.Windows.Visibility
    | HeadersVAlignment of System.Windows.VerticalAlignment

module FieldHeaders =
    let contentTemplate v : obj = box (FieldHeadersProp.ContentTemplate v)
    let leftOffset v : obj = box (FieldHeadersProp.LeftOffset v)
    let leftPixelsOffset v : obj = box (FieldHeadersProp.LeftPixelsOffset v)
    let measureMode v : obj = box (FieldHeadersProp.MeasureMode v)
    let emptyTextVAlignment v : obj = box (FieldHeadersProp.EmptyTextVAlignment v)
    let emptyTextHAlignment v : obj = box (FieldHeadersProp.EmptyTextHAlignment v)
    let emptyTextVisibility v : obj = box (FieldHeadersProp.EmptyTextVisibility v)
    let headersVAlignment v : obj = box (FieldHeadersProp.HeadersVAlignment v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.FieldHeaders) (prop: FieldHeadersProp) =
        match prop with
        | FieldHeadersProp.Base p -> FieldHeadersBase.apply el p
        | FieldHeadersProp.ContentTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.ContentTemplateProperty, box v)
        | FieldHeadersProp.LeftOffset v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.LeftOffsetProperty, box v)
        | FieldHeadersProp.LeftPixelsOffset v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.LeftPixelsOffsetProperty, box v)
        | FieldHeadersProp.MeasureMode v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.MeasureModeProperty, box v)
        | FieldHeadersProp.EmptyTextVAlignment v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.EmptyTextVAlignmentProperty, box v)
        | FieldHeadersProp.EmptyTextHAlignment v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.EmptyTextHAlignmentProperty, box v)
        | FieldHeadersProp.EmptyTextVisibility v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.EmptyTextVisibilityProperty, box v)
        | FieldHeadersProp.HeadersVAlignment v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeaders.HeadersVAlignmentProperty, box v)

    let area v : obj = box (FieldHeadersBaseProp.Area v)
    let actualArea v : obj = box (FieldHeadersBaseProp.ActualArea v)
    let enableDragDrop v : obj = box (FieldHeadersBaseProp.EnableDragDrop v)
    let enableHeaderMenu v : obj = box (FieldHeadersBaseProp.EnableHeaderMenu v)
    let pivotGrid v : obj = box (FieldHeadersBaseProp.PivotGrid v)
    let showListSource v : obj = box (FieldHeadersBaseProp.ShowListSource v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.FieldHeaders>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
