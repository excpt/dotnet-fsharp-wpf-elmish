...
type MiniMapViewportPanelProp =
    | Viewport of System.Windows.Rect

module MiniMapViewportPanel =
    let viewport v : obj = box (MiniMapViewportPanelProp.Viewport v)

    let apply (el: DevExpress.Xpf.Map.MiniMapViewportPanel) (prop: MiniMapViewportPanelProp) =
        match prop with
        | MiniMapViewportPanelProp.Viewport v -> el.SetValue(DevExpress.Xpf.Map.MiniMapViewportPanel.ViewportProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.MiniMapViewportPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
