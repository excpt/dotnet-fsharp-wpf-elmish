...
type AppBarProp =
    | Base of veItemsControlProp
    | Alignment of DevExpress.Xpf.WindowsUI.AppBarAlignment
    | BackCommand of System.Windows.Input.ICommand
    | ExitCommand of System.Windows.Input.ICommand
    | HideMode of DevExpress.Xpf.WindowsUI.AppBarHideMode
    | IsAnimationEnabled of bool
    | IsBackButtonEnabled of bool
    | IsCompact of bool
    | IsExitButtonEnabled of bool
    | IsOpen of bool
    | IsSticky of bool
    | ItemSpacing of float

module AppBar =
    let alignment v : obj = box (AppBarProp.Alignment v)
    let backCommand v : obj = box (AppBarProp.BackCommand v)
    let exitCommand v : obj = box (AppBarProp.ExitCommand v)
    let hideMode v : obj = box (AppBarProp.HideMode v)
    let isAnimationEnabled v : obj = box (AppBarProp.IsAnimationEnabled v)
    let isBackButtonEnabled v : obj = box (AppBarProp.IsBackButtonEnabled v)
    let isCompact v : obj = box (AppBarProp.IsCompact v)
    let isExitButtonEnabled v : obj = box (AppBarProp.IsExitButtonEnabled v)
    let isOpen v : obj = box (AppBarProp.IsOpen v)
    let isSticky v : obj = box (AppBarProp.IsSticky v)
    let itemSpacing v : obj = box (AppBarProp.ItemSpacing v)

    let apply (el: DevExpress.Xpf.WindowsUI.AppBar) (prop: AppBarProp) =
        match prop with
        | AppBarProp.Base p -> veItemsControl.apply el p
        | AppBarProp.Alignment v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.AlignmentProperty, box v)
        | AppBarProp.BackCommand v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.BackCommandProperty, box v)
        | AppBarProp.ExitCommand v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.ExitCommandProperty, box v)
        | AppBarProp.HideMode v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.HideModeProperty, box v)
        | AppBarProp.IsAnimationEnabled v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.IsAnimationEnabledProperty, box v)
        | AppBarProp.IsBackButtonEnabled v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.IsBackButtonEnabledProperty, box v)
        | AppBarProp.IsCompact v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.IsCompactProperty, box v)
        | AppBarProp.IsExitButtonEnabled v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.IsExitButtonEnabledProperty, box v)
        | AppBarProp.IsOpen v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.IsOpenProperty, box v)
        | AppBarProp.IsSticky v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.IsStickyProperty, box v)
        | AppBarProp.ItemSpacing v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBar.ItemSpacingProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.AppBar>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
