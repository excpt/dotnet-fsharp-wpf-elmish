...
type BaseGridHeaderProp =
    | Base of GridColumnHeaderBaseProp

module BaseGridHeader =

    let apply (el: DevExpress.Xpf.Grid.BaseGridHeader) (prop: BaseGridHeaderProp) =
        match prop with
        | BaseGridHeaderProp.Base p -> GridColumnHeaderBase.apply el p

    let columnPosition v : obj = box (GridColumnHeaderBaseProp.ColumnPosition v)
    let hasTopElement v : obj = box (GridColumnHeaderBaseProp.HasTopElement v)
    let hasBottomElement v : obj = box (GridColumnHeaderBaseProp.HasBottomElement v)
    let hasRightSibling v : obj = box (GridColumnHeaderBaseProp.HasRightSibling v)
    let hasLeftSibling v : obj = box (GridColumnHeaderBaseProp.HasLeftSibling v)
    ...
    let gridColumn (v: DevExpress.Xpf.Grid.BaseColumn) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.GridColumnProperty, box v)) :: node.Props }
    let dropPlaceOrientation (v: System.Windows.Controls.Orientation) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.DropPlaceOrientationProperty, box v)) :: node.Props }
    let dragElementTemplate (v: System.Windows.DataTemplate) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.DragElementTemplateProperty, box v)) :: node.Props }
    let canGroupMergeState (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.CanGroupMergeStateProperty, box v)) :: node.Props }
    let isGroupPanel (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.IsGroupPanelProperty, box v)) :: node.Props }
    let isColumnChooser (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.IsColumnChooserProperty, box v)) :: node.Props }
    let extendedColumnChooserView (v: DevExpress.Xpf.Grid.IExtendedColumnChooserView) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridHeader.ExtendedColumnChooserViewProperty, box v)) :: node.Props }
