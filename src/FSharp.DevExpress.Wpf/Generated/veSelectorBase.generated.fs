...
type veSelectorBaseProp =
    | OnSelectionChanging of DevExpress.Xpf.WindowsUI.Base.SelectionChangingEventHandler

module veSelectorBase =
    let onSelectionChanging v : obj = box (EventProp(box (veSelectorBaseProp.OnSelectionChanging v)))

    let apply (el: DevExpress.Xpf.WindowsUI.Base.veSelectorBase) (prop: veSelectorBaseProp) =
        match prop with
        | veSelectorBaseProp.OnSelectionChanging h -> el.SelectionChanging.AddHandler(h)
