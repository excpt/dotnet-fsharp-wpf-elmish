...
type FieldHeaderProp =
    | Base of FieldHeaderBaseProp
    | ContentTemplate of System.Windows.Controls.ControlTemplate
    | FilterButtonTemplate of System.Windows.Controls.ControlTemplate
    | FilterPopupTemplate of System.Windows.DataTemplate
    | FilterPopupTemplateSelector of System.Windows.Controls.DataTemplateSelector
    | IsPopupOpen of bool
    | IsSelectedInDesignTime of bool

module FieldHeader =
    let contentTemplate v : obj = box (FieldHeaderProp.ContentTemplate v)
    let filterButtonTemplate v : obj = box (FieldHeaderProp.FilterButtonTemplate v)
    let filterPopupTemplate v : obj = box (FieldHeaderProp.FilterPopupTemplate v)
    let filterPopupTemplateSelector v : obj = box (FieldHeaderProp.FilterPopupTemplateSelector v)
    let isPopupOpen v : obj = box (FieldHeaderProp.IsPopupOpen v)
    let isSelectedInDesignTime v : obj = box (FieldHeaderProp.IsSelectedInDesignTime v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.FieldHeader) (prop: FieldHeaderProp) =
        match prop with
        | FieldHeaderProp.Base p -> FieldHeaderBase.apply el p
        | FieldHeaderProp.ContentTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.ContentTemplateProperty, box v)
        | FieldHeaderProp.FilterButtonTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.FilterButtonTemplateProperty, box v)
        | FieldHeaderProp.FilterPopupTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.FilterPopupTemplateProperty, box v)
        | FieldHeaderProp.FilterPopupTemplateSelector v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.FilterPopupTemplateSelectorProperty, box v)
        | FieldHeaderProp.IsPopupOpen v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.IsPopupOpenProperty, box v)
        | FieldHeaderProp.IsSelectedInDesignTime v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.IsSelectedInDesignTimeProperty, box v)

    let dragElementTemplate v : obj = box (FieldHeaderBaseProp.DragElementTemplate v)
    let changeFieldSortOrder v : obj = box (FieldHeaderBaseProp.ChangeFieldSortOrder v)
    let displayText v : obj = box (FieldHeaderBaseProp.DisplayText v)
    ...
    let header (v: DevExpress.Xpf.PivotGrid.Internal.FieldHeader) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.FieldHeader.HeaderProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.FieldHeader>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
