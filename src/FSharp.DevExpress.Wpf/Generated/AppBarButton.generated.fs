...
type AppBarButtonProp =
    | Base of AppBarButtonBaseProp
    | Alignment of DevExpress.Xpf.WindowsUI.AppBarElementAlignment
    | ContentFontFamily of System.Windows.Media.FontFamily
    | ContentFontSize of int
    | IsCompact of bool
    | Label of obj
    | LabelTemplate of System.Windows.DataTemplate
    | LabelTemplateSelector of System.Windows.Controls.DataTemplateSelector

module AppBarButton =
    let alignment v : obj = box (AppBarButtonProp.Alignment v)
    let contentFontFamily v : obj = box (AppBarButtonProp.ContentFontFamily v)
    let contentFontSize v : obj = box (AppBarButtonProp.ContentFontSize v)
    let isCompact v : obj = box (AppBarButtonProp.IsCompact v)
    let label v : obj = box (AppBarButtonProp.Label v)
    let labelTemplate v : obj = box (AppBarButtonProp.LabelTemplate v)
    let labelTemplateSelector v : obj = box (AppBarButtonProp.LabelTemplateSelector v)

    let apply (el: DevExpress.Xpf.WindowsUI.AppBarButton) (prop: AppBarButtonProp) =
        match prop with
        | AppBarButtonProp.Base p -> AppBarButtonBase.apply el p
        | AppBarButtonProp.Alignment v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.AlignmentProperty, box v)
        | AppBarButtonProp.ContentFontFamily v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.ContentFontFamilyProperty, box v)
        | AppBarButtonProp.ContentFontSize v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.ContentFontSizeProperty, box v)
        | AppBarButtonProp.IsCompact v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.IsCompactProperty, box v)
        | AppBarButtonProp.Label v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.LabelProperty, box v)
        | AppBarButtonProp.LabelTemplate v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.LabelTemplateProperty, box v)
        | AppBarButtonProp.LabelTemplateSelector v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButton.LabelTemplateSelectorProperty, box v)

    let flyout v : obj = box (AppBarButtonBaseProp.Flyout v)
    let glyph v : obj = box (CommandButtonProp.Glyph v)
    let allowGlyphTheming v : obj = box (CommandButtonProp.AllowGlyphTheming v)
    let isEllipseEnabled v : obj = box (CommandButtonProp.IsEllipseEnabled v)
    let ellipseDiameter v : obj = box (CommandButtonProp.EllipseDiameter v)
    let glyphWidth v : obj = box (CommandButtonProp.GlyphWidth v)
    let glyphHeight v : obj = box (CommandButtonProp.GlyphHeight v)
    let glyphStretch v : obj = box (CommandButtonProp.GlyphStretch v)
    let strokeThickness v : obj = box (CommandButtonProp.StrokeThickness v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.AppBarButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
