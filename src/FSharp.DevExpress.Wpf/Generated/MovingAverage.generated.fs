...
type MovingAverageProp =
    | Base of IndicatorProp
    | ValueLevel of DevExpress.Xpf.Charts.ValueLevel
    | MovingAverageKind of DevExpress.Xpf.Charts.MovingAverageKind
    | EnvelopePercent of float
    | PointsCount of int

module MovingAverage =
    let valueLevel v : obj = box (MovingAverageProp.ValueLevel v)
    let movingAverageKind v : obj = box (MovingAverageProp.MovingAverageKind v)
    let envelopePercent v : obj = box (MovingAverageProp.EnvelopePercent v)
    let pointsCount v : obj = box (MovingAverageProp.PointsCount v)

    let apply (el: DevExpress.Xpf.Charts.MovingAverage) (prop: MovingAverageProp) =
        match prop with
        | MovingAverageProp.Base p -> Indicator.apply el p
        | MovingAverageProp.ValueLevel v -> el.SetValue(DevExpress.Xpf.Charts.MovingAverage.ValueLevelProperty, box v)
        | MovingAverageProp.MovingAverageKind v -> el.SetValue(DevExpress.Xpf.Charts.MovingAverage.MovingAverageKindProperty, box v)
        | MovingAverageProp.EnvelopePercent v -> el.SetValue(DevExpress.Xpf.Charts.MovingAverage.EnvelopePercentProperty, box v)
        | MovingAverageProp.PointsCount v -> el.SetValue(DevExpress.Xpf.Charts.MovingAverage.PointsCountProperty, box v)
