...
type ColumnPanelProp =
    | ColumnCount of int
    | ColumnOffsets of System.Int32[]
    | RowOffsets of System.Int32[]

module ColumnPanel =
    let columnCount v : obj = box (ColumnPanelProp.ColumnCount v)
    let columnOffsets v : obj = box (ColumnPanelProp.ColumnOffsets v)
    let rowOffsets v : obj = box (ColumnPanelProp.RowOffsets v)

    let apply (el: DevExpress.Xpf.Core.ColumnPanel) (prop: ColumnPanelProp) =
        match prop with
        | ColumnPanelProp.ColumnCount v -> el.SetValue(DevExpress.Xpf.Core.ColumnPanel.ColumnCountProperty, box v)
        | ColumnPanelProp.ColumnOffsets v -> el.SetValue(DevExpress.Xpf.Core.ColumnPanel.ColumnOffsetsProperty, box v)
        | ColumnPanelProp.RowOffsets v -> el.SetValue(DevExpress.Xpf.Core.ColumnPanel.RowOffsetsProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ColumnPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
