...
type FloatingBarPopupProp =
    | Bar of DevExpress.Xpf.Bars.Bar

module FloatingBarPopup =
    let bar v : obj = box (FloatingBarPopupProp.Bar v)

    let apply (el: DevExpress.Xpf.Bars.FloatingBarPopup) (prop: FloatingBarPopupProp) =
        match prop with
        | FloatingBarPopupProp.Bar v -> el.SetValue(DevExpress.Xpf.Bars.FloatingBarPopup.BarProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.FloatingBarPopup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
