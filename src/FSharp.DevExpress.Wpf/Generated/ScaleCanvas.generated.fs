...
type ScaleCanvasProp =
    | Scale of float

module ScaleCanvas =
    let scale v : obj = box (ScaleCanvasProp.Scale v)

    let apply (el: DevExpress.Xpf.Diagram.ScaleCanvas) (prop: ScaleCanvasProp) =
        match prop with
        | ScaleCanvasProp.Scale v -> el.SetValue(DevExpress.Xpf.Diagram.ScaleCanvas.ScaleProperty, box v)
    ...
    let position (v: System.Windows.Point) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Diagram.ScaleCanvas.PositionProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Diagram.ScaleCanvas>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
