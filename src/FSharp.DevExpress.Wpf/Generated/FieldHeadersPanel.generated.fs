...
type FieldHeadersPanelProp =
    | SkipWidth of int
    | MeasureMode of DevExpress.Xpf.PivotGrid.Internal.FieldHeadersMeasureMode

module FieldHeadersPanel =
    let skipWidth v : obj = box (FieldHeadersPanelProp.SkipWidth v)
    let measureMode v : obj = box (FieldHeadersPanelProp.MeasureMode v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.FieldHeadersPanel) (prop: FieldHeadersPanelProp) =
        match prop with
        | FieldHeadersPanelProp.SkipWidth v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeadersPanel.SkipWidthProperty, box v)
        | FieldHeadersPanelProp.MeasureMode v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FieldHeadersPanel.MeasureModeProperty, box v)
    ...
    let index (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.FieldHeadersPanel.IndexProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.FieldHeadersPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
