...
type FloatingContainerControlProp =
    | FloatingMode of DevExpress.Xpf.Core.FloatingMode
    | AdornerTemplate of System.Windows.Controls.ControlTemplate
    | WindowTemplate of System.Windows.Controls.ControlTemplate
    | Owner of System.Windows.FrameworkElement
    | FloatLocation of System.Windows.Point
    | FloatSize of System.Windows.Size
    | IsOpen of bool
    | Content of obj

module FloatingContainerControl =
    let floatingMode v : obj = box (FloatingContainerControlProp.FloatingMode v)
    let adornerTemplate v : obj = box (FloatingContainerControlProp.AdornerTemplate v)
    let windowTemplate v : obj = box (FloatingContainerControlProp.WindowTemplate v)
    let owner v : obj = box (FloatingContainerControlProp.Owner v)
    let floatLocation v : obj = box (FloatingContainerControlProp.FloatLocation v)
    let floatSize v : obj = box (FloatingContainerControlProp.FloatSize v)
    let isOpen v : obj = box (FloatingContainerControlProp.IsOpen v)
    let content v : obj = box (FloatingContainerControlProp.Content v)

    let apply (el: DevExpress.Xpf.Core.FloatingContainerControl) (prop: FloatingContainerControlProp) =
        match prop with
        | FloatingContainerControlProp.FloatingMode v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.FloatingModeProperty, box v)
        | FloatingContainerControlProp.AdornerTemplate v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.AdornerTemplateProperty, box v)
        | FloatingContainerControlProp.WindowTemplate v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.WindowTemplateProperty, box v)
        | FloatingContainerControlProp.Owner v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.OwnerProperty, box v)
        | FloatingContainerControlProp.FloatLocation v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.FloatLocationProperty, box v)
        | FloatingContainerControlProp.FloatSize v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.FloatSizeProperty, box v)
        | FloatingContainerControlProp.IsOpen v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.IsOpenProperty, box v)
        | FloatingContainerControlProp.Content v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainerControl.ContentProperty, box v)
    ...
    let contentOffset (v: System.Windows.Thickness) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.FloatingContainerControl.ContentOffsetProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.FloatingContainerControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
