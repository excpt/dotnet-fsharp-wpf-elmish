...
type FilterCheckedTreeViewProp =
    | IsItemsValid of bool
    | IsItemsLoaded of bool
    | ItemsHeaderTemplate of System.Windows.DataTemplate

module FilterCheckedTreeView =
    let isItemsValid v : obj = box (FilterCheckedTreeViewProp.IsItemsValid v)
    let isItemsLoaded v : obj = box (FilterCheckedTreeViewProp.IsItemsLoaded v)
    let itemsHeaderTemplate v : obj = box (FilterCheckedTreeViewProp.ItemsHeaderTemplate v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.FilterCheckedTreeView) (prop: FilterCheckedTreeViewProp) =
        match prop with
        | FilterCheckedTreeViewProp.IsItemsValid v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FilterCheckedTreeView.IsItemsValidProperty, box v)
        | FilterCheckedTreeViewProp.IsItemsLoaded v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FilterCheckedTreeView.IsItemsLoadedProperty, box v)
        | FilterCheckedTreeViewProp.ItemsHeaderTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.FilterCheckedTreeView.ItemsHeaderTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.FilterCheckedTreeView>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
