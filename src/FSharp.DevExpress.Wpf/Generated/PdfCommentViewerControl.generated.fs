...
type PdfCommentViewerControlProp =
    | TreeViewStyle of System.Windows.Style
    | Settings of DevExpress.Xpf.PdfViewer.PdfCommentViewerSettings
    | HideAfterUse of bool

module PdfCommentViewerControl =
    let treeViewStyle v : obj = box (PdfCommentViewerControlProp.TreeViewStyle v)
    let settings v : obj = box (PdfCommentViewerControlProp.Settings v)
    let hideAfterUse v : obj = box (PdfCommentViewerControlProp.HideAfterUse v)

    let apply (el: DevExpress.Xpf.PdfViewer.PdfCommentViewerControl) (prop: PdfCommentViewerControlProp) =
        match prop with
        | PdfCommentViewerControlProp.TreeViewStyle v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfCommentViewerControl.TreeViewStyleProperty, box v)
        | PdfCommentViewerControlProp.Settings v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfCommentViewerControl.SettingsProperty, box v)
        | PdfCommentViewerControlProp.HideAfterUse v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfCommentViewerControl.HideAfterUseProperty, box v)
    ...
    let allowEditingCurrentEditor (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PdfViewer.PdfCommentViewerControl.AllowEditingCurrentEditorProperty, box v)) :: node.Props }
    let commentViewer (v: DevExpress.Xpf.PdfViewer.PdfCommentViewerControl) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PdfViewer.PdfCommentViewerControl.CommentViewerProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PdfViewer.PdfCommentViewerControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
