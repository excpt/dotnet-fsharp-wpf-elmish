...
module SideBarPanel =
    let activeGroupMinHeight v : obj = box (SideBarPanelProp.ActiveGroupMinHeight v)

    let apply (el: DevExpress.Xpf.NavBar.SideBarPanel) (prop: SideBarPanelProp) =
        match prop with
        | SideBarPanelProp.Base p -> NavBarPositionPanel.apply el p
        | SideBarPanelProp.ActiveGroupMinHeight v -> el.SetValue(DevExpress.Xpf.NavBar.SideBarPanel.ActiveGroupMinHeightProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.NavBar.SideBarPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
