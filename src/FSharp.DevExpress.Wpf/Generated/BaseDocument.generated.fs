...
module BaseDocument =
    let isSelected v : obj = box (BaseDocumentProp.IsSelected v)
    let controlHostTemplate v : obj = box (BaseDocumentProp.ControlHostTemplate v)
    let layoutHostTemplate v : obj = box (BaseDocumentProp.LayoutHostTemplate v)
    let dataHostTemplate v : obj = box (BaseDocumentProp.DataHostTemplate v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.BaseDocument) (prop: BaseDocumentProp) =
        match prop with
        | BaseDocumentProp.Base p -> psvContentControl.apply el p
        | BaseDocumentProp.IsSelected v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseDocument.IsSelectedProperty, box v)
        | BaseDocumentProp.ControlHostTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseDocument.ControlHostTemplateProperty, box v)
        | BaseDocumentProp.LayoutHostTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseDocument.LayoutHostTemplateProperty, box v)
        | BaseDocumentProp.DataHostTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseDocument.DataHostTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.BaseDocument>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
