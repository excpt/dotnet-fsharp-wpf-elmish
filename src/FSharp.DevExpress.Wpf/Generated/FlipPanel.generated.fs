...
type FlipPanelProp =
    | Base of VirtualizingScrollPanelProp
    | ItemCacheMode of DevExpress.Xpf.WindowsUI.ItemCacheMode

module FlipPanel =
    let itemCacheMode v : obj = box (FlipPanelProp.ItemCacheMode v)

    let apply (el: DevExpress.Xpf.WindowsUI.Internal.FlipPanel) (prop: FlipPanelProp) =
        match prop with
        | FlipPanelProp.Base p -> VirtualizingScrollPanel.apply el p
        | FlipPanelProp.ItemCacheMode v -> el.SetValue(DevExpress.Xpf.WindowsUI.Internal.FlipPanel.ItemCacheModeProperty, box v)

    let orientation v : obj = box (VirtualizingScrollPanelProp.Orientation v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.Internal.FlipPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
