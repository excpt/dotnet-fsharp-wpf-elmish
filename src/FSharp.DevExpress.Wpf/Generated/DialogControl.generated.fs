...
type DialogControlProp =
    | DialogContent of System.Windows.FrameworkElement
    | UseContentIndents of bool
    | ShowApplyButton of bool

module DialogControl =
    let dialogContent v : obj = box (DialogControlProp.DialogContent v)
    let useContentIndents v : obj = box (DialogControlProp.UseContentIndents v)
    let showApplyButton v : obj = box (DialogControlProp.ShowApplyButton v)

    let apply (el: DevExpress.Xpf.Core.DialogControl) (prop: DialogControlProp) =
        match prop with
        | DialogControlProp.DialogContent v -> el.SetValue(DevExpress.Xpf.Core.DialogControl.DialogContentProperty, box v)
        | DialogControlProp.UseContentIndents v -> el.SetValue(DevExpress.Xpf.Core.DialogControl.UseContentIndentsProperty, box v)
        | DialogControlProp.ShowApplyButton v -> el.SetValue(DevExpress.Xpf.Core.DialogControl.ShowApplyButtonProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DialogControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
