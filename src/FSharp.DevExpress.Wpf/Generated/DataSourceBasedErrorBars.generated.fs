...
type DataSourceBasedErrorBarsProp =
    | Base of ErrorBarsProp
    | NegativeErrorDataMember of string
    | PositiveErrorDataMember of string

module DataSourceBasedErrorBars =
    let negativeErrorDataMember v : obj = box (DataSourceBasedErrorBarsProp.NegativeErrorDataMember v)
    let positiveErrorDataMember v : obj = box (DataSourceBasedErrorBarsProp.PositiveErrorDataMember v)

    let apply (el: DevExpress.Xpf.Charts.DataSourceBasedErrorBars) (prop: DataSourceBasedErrorBarsProp) =
        match prop with
        | DataSourceBasedErrorBarsProp.Base p -> ErrorBars.apply el p
        | DataSourceBasedErrorBarsProp.NegativeErrorDataMember v -> el.SetValue(DevExpress.Xpf.Charts.DataSourceBasedErrorBars.NegativeErrorDataMemberProperty, box v)
        | DataSourceBasedErrorBarsProp.PositiveErrorDataMember v -> el.SetValue(DevExpress.Xpf.Charts.DataSourceBasedErrorBars.PositiveErrorDataMemberProperty, box v)

    let endStyle v : obj = box (ErrorBarsProp.EndStyle v)
    let direction v : obj = box (ErrorBarsProp.Direction v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.DataSourceBasedErrorBars>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
