...
type ModelControlProp =
    | Base of ChartElementBaseProp
    | Flipped of bool
    | PresentationData of DevExpress.Xpf.Charts.SeriesPointPresentationData

module ModelControl =
    let flipped v : obj = box (ModelControlProp.Flipped v)
    let presentationData v : obj = box (ModelControlProp.PresentationData v)

    let apply (el: DevExpress.Xpf.Charts.ModelControl) (prop: ModelControlProp) =
        match prop with
        | ModelControlProp.Base p -> ChartElementBase.apply el p
        | ModelControlProp.Flipped v -> el.SetValue(DevExpress.Xpf.Charts.ModelControl.FlippedProperty, box v)
        | ModelControlProp.PresentationData v -> el.SetValue(DevExpress.Xpf.Charts.ModelControl.PresentationDataProperty, box v)
