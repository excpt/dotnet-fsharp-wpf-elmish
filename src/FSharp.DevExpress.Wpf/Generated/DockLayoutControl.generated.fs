...
type DockLayoutControlProp =
    | Base of LayoutControlBaseProp
    | AllowItemSizing of bool
    | ItemSizerStyle of System.Windows.Style

module DockLayoutControl =
    let allowItemSizing v : obj = box (DockLayoutControlProp.AllowItemSizing v)
    let itemSizerStyle v : obj = box (DockLayoutControlProp.ItemSizerStyle v)

    let apply (el: DevExpress.Xpf.LayoutControl.DockLayoutControl) (prop: DockLayoutControlProp) =
        match prop with
        | DockLayoutControlProp.Base p -> LayoutControlBase.apply el p
        | DockLayoutControlProp.AllowItemSizing v -> el.SetValue(DevExpress.Xpf.LayoutControl.DockLayoutControl.AllowItemSizingProperty, box v)
        | DockLayoutControlProp.ItemSizerStyle v -> el.SetValue(DevExpress.Xpf.LayoutControl.DockLayoutControl.ItemSizerStyleProperty, box v)
    ...
    let allowHorizontalSizing (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.DockLayoutControl.AllowHorizontalSizingProperty, box v)) :: node.Props }
    let allowVerticalSizing (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.DockLayoutControl.AllowVerticalSizingProperty, box v)) :: node.Props }
    let dock (v: DevExpress.Xpf.LayoutControl.Dock) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.DockLayoutControl.DockProperty, box v)) :: node.Props }
    let useDesiredWidthAsMaxWidth (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.DockLayoutControl.UseDesiredWidthAsMaxWidthProperty, box v)) :: node.Props }
    let useDesiredHeightAsMaxHeight (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.DockLayoutControl.UseDesiredHeightAsMaxHeightProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.LayoutControl.DockLayoutControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
