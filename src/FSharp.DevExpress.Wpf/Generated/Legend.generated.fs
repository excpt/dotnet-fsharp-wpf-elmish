...
type LegendProp =
    | Base of LegendBaseProp
    | UseCheckBoxes of bool
    | MarkerMode of DevExpress.Xpf.Charts.LegendMarkerMode
    | MaxCrosshairContentWidth of float
    | MaxCrosshairContentHeight of float
    | ReverseItems of bool
    | Items of DevExpress.Xpf.Charts.LegendItemList
    | DockTarget of obj

module Legend =
    let useCheckBoxes v : obj = box (LegendProp.UseCheckBoxes v)
    let markerMode v : obj = box (LegendProp.MarkerMode v)
    let maxCrosshairContentWidth v : obj = box (LegendProp.MaxCrosshairContentWidth v)
    let maxCrosshairContentHeight v : obj = box (LegendProp.MaxCrosshairContentHeight v)
    let reverseItems v : obj = box (LegendProp.ReverseItems v)
    let items v : obj = box (LegendProp.Items v)
    let dockTarget v : obj = box (LegendProp.DockTarget v)

    let apply (el: DevExpress.Xpf.Charts.Legend) (prop: LegendProp) =
        match prop with
        | LegendProp.Base p -> LegendBase.apply el p
        | LegendProp.UseCheckBoxes v -> el.SetValue(DevExpress.Xpf.Charts.Legend.UseCheckBoxesProperty, box v)
        | LegendProp.MarkerMode v -> el.SetValue(DevExpress.Xpf.Charts.Legend.MarkerModeProperty, box v)
        | LegendProp.MaxCrosshairContentWidth v -> el.SetValue(DevExpress.Xpf.Charts.Legend.MaxCrosshairContentWidthProperty, box v)
        | LegendProp.MaxCrosshairContentHeight v -> el.SetValue(DevExpress.Xpf.Charts.Legend.MaxCrosshairContentHeightProperty, box v)
        | LegendProp.ReverseItems v -> el.SetValue(DevExpress.Xpf.Charts.Legend.ReverseItemsProperty, box v)
        | LegendProp.Items v -> el.SetValue(DevExpress.Xpf.Charts.Legend.ItemsProperty, box v)
        | LegendProp.DockTarget v -> el.SetValue(DevExpress.Xpf.Charts.Legend.DockTargetProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.Legend>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
