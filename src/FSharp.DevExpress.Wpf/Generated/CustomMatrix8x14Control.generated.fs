...
type CustomMatrix8x14ControlProp =
    | Base of SymbolPresentationControlProp
    | ActiveSegmentTemplate of System.Windows.DataTemplate
    | InactiveSegmentTemplate of System.Windows.DataTemplate

module CustomMatrix8x14Control =
    let activeSegmentTemplate v : obj = box (CustomMatrix8x14ControlProp.ActiveSegmentTemplate v)
    let inactiveSegmentTemplate v : obj = box (CustomMatrix8x14ControlProp.InactiveSegmentTemplate v)

    let apply (el: DevExpress.Xpf.Gauges.CustomMatrix8x14Control) (prop: CustomMatrix8x14ControlProp) =
        match prop with
        | CustomMatrix8x14ControlProp.Base p -> SymbolPresentationControl.apply el p
        | CustomMatrix8x14ControlProp.ActiveSegmentTemplate v -> el.SetValue(DevExpress.Xpf.Gauges.CustomMatrix8x14Control.ActiveSegmentTemplateProperty, box v)
        | CustomMatrix8x14ControlProp.InactiveSegmentTemplate v -> el.SetValue(DevExpress.Xpf.Gauges.CustomMatrix8x14Control.InactiveSegmentTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gauges.CustomMatrix8x14Control>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
