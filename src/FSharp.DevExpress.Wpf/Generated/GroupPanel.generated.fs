...
module GroupPanel =
    let orientation v : obj = box (GroupPanelProp.Orientation v)
    let lastChildFill v : obj = box (GroupPanelProp.LastChildFill v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.GroupPanel) (prop: GroupPanelProp) =
        match prop with
        | GroupPanelProp.Base p -> psvGrid.apply el p
        | GroupPanelProp.Orientation v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.GroupPanel.OrientationProperty, box v)
        | GroupPanelProp.LastChildFill v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.GroupPanel.LastChildFillProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.GroupPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
