...
module DocumentViewer =
    let model v : obj = box (DocumentViewerProp.Model v)

    let apply (el: DevExpress.Xpf.Printing.DocumentViewer) (prop: DocumentViewerProp) =
        match prop with
        | DocumentViewerProp.Model v -> el.SetValue(DevExpress.Xpf.Printing.DocumentViewer.ModelProperty, box v)
    ...
    let documentViewer (v: DevExpress.Xpf.Printing.DocumentPreview) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Printing.DocumentViewer.DocumentViewerProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Printing.DocumentViewer>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
