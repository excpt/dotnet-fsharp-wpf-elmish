...
type DocumentPanelProp =
    | Base of LayoutPanelProp
    | EnableMouseHoverWhenInactive of bool

module DocumentPanel =
    let enableMouseHoverWhenInactive v : obj = box (DocumentPanelProp.EnableMouseHoverWhenInactive v)

    let apply (el: DevExpress.Xpf.Docking.DocumentPanel) (prop: DocumentPanelProp) =
        match prop with
        | DocumentPanelProp.Base p -> LayoutPanel.apply el p
        | DocumentPanelProp.EnableMouseHoverWhenInactive v -> el.SetValue(DevExpress.Xpf.Docking.DocumentPanel.EnableMouseHoverWhenInactiveProperty, box v)
    ...
    let mDILocation (v: System.Windows.Point) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentPanel.MDILocationProperty, box v)) :: node.Props }
    let mDISize (v: System.Windows.Size) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentPanel.MDISizeProperty, box v)) :: node.Props }
    let mDIState (v: DevExpress.Xpf.Docking.MDIState) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentPanel.MDIStateProperty, box v)) :: node.Props }
    let restoreBounds (v: System.Windows.Rect) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.DocumentPanel.RestoreBoundsProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Docking.DocumentPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
