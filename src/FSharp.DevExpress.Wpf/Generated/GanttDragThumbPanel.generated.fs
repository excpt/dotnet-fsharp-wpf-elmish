...
type GanttDragThumbPanelProp =
    | Progress of float

module GanttDragThumbPanel =
    let progress v : obj = box (GanttDragThumbPanelProp.Progress v)

    let apply (el: DevExpress.Xpf.Gantt.Native.GanttDragThumbPanel) (prop: GanttDragThumbPanelProp) =
        match prop with
        | GanttDragThumbPanelProp.Progress v -> el.SetValue(DevExpress.Xpf.Gantt.Native.GanttDragThumbPanel.ProgressProperty, box v)
    ...
    let isProgressThumb (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Gantt.Native.GanttDragThumbPanel.IsProgressThumbProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Gantt.Native.GanttDragThumbPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
