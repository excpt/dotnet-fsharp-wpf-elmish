...
type SharedSizePanelProp =
    | Base of SchedulerPanelBaseProp
    | SharedSizeGroup of string
    | IsBaseSizePanel of bool
    | Orientation of System.Windows.Controls.Orientation
    | NotificationElement of System.Windows.FrameworkElement

module SharedSizePanel =
    let sharedSizeGroup v : obj = box (SharedSizePanelProp.SharedSizeGroup v)
    let isBaseSizePanel v : obj = box (SharedSizePanelProp.IsBaseSizePanel v)
    let orientation v : obj = box (SharedSizePanelProp.Orientation v)
    let notificationElement v : obj = box (SharedSizePanelProp.NotificationElement v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel) (prop: SharedSizePanelProp) =
        match prop with
        | SharedSizePanelProp.Base p -> SchedulerPanelBase.apply el p
        | SharedSizePanelProp.SharedSizeGroup v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel.SharedSizeGroupProperty, box v)
        | SharedSizePanelProp.IsBaseSizePanel v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel.IsBaseSizePanelProperty, box v)
        | SharedSizePanelProp.Orientation v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel.OrientationProperty, box v)
        | SharedSizePanelProp.NotificationElement v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel.NotificationElementProperty, box v)
    ...
    let span (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel.SpanProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.SharedSizePanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
