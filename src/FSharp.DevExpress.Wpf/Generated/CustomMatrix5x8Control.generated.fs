...
type CustomMatrix5x8ControlProp =
...
module CustomMatrix5x8Control =
    let activeSegmentTemplate v : obj = box (CustomMatrix5x8ControlProp.ActiveSegmentTemplate v)
    let inactiveSegmentTemplate v : obj = box (CustomMatrix5x8ControlProp.InactiveSegmentTemplate v)

    let apply (el: DevExpress.Xpf.Gauges.CustomMatrix5x8Control) (prop: CustomMatrix5x8ControlProp) =
        match prop with
        | CustomMatrix5x8ControlProp.Base p -> SymbolPresentationControl.apply el p
        | CustomMatrix5x8ControlProp.ActiveSegmentTemplate v -> el.SetValue(DevExpress.Xpf.Gauges.CustomMatrix5x8Control.ActiveSegmentTemplateProperty, box v)
        | CustomMatrix5x8ControlProp.InactiveSegmentTemplate v -> el.SetValue(DevExpress.Xpf.Gauges.CustomMatrix5x8Control.InactiveSegmentTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gauges.CustomMatrix5x8Control>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
