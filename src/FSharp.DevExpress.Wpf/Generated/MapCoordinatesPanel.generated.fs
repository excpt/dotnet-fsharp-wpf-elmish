...
type MapCoordinatesPanelProp =
    | CoordPoint of DevExpress.Map.CoordPoint

module MapCoordinatesPanel =
    let coordPoint v : obj = box (MapCoordinatesPanelProp.CoordPoint v)

    let apply (el: DevExpress.Xpf.Map.MapCoordinatesPanel) (prop: MapCoordinatesPanelProp) =
        match prop with
        | MapCoordinatesPanelProp.CoordPoint v -> el.SetValue(DevExpress.Xpf.Map.MapCoordinatesPanel.CoordPointProperty, box v)
