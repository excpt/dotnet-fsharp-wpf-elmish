...
type DataBarStyleSettingsProp =
    | FNC1Substitute of string
    | SegmentsInRow of int
    | Type of DevExpress.XtraPrinting.BarCode.DataBarType

module DataBarStyleSettings =
    let fNC1Substitute v : obj = box (DataBarStyleSettingsProp.FNC1Substitute v)
    let segmentsInRow v : obj = box (DataBarStyleSettingsProp.SegmentsInRow v)
    let type v : obj = box (DataBarStyleSettingsProp.Type v)

    let apply (el: DevExpress.Xpf.Editors.DataBarStyleSettings) (prop: DataBarStyleSettingsProp) =
        match prop with
        | DataBarStyleSettingsProp.FNC1Substitute v -> el.SetValue(DevExpress.Xpf.Editors.DataBarStyleSettings.FNC1SubstituteProperty, box v)
        | DataBarStyleSettingsProp.SegmentsInRow v -> el.SetValue(DevExpress.Xpf.Editors.DataBarStyleSettings.SegmentsInRowProperty, box v)
        | DataBarStyleSettingsProp.Type v -> el.SetValue(DevExpress.Xpf.Editors.DataBarStyleSettings.TypeProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.DataBarStyleSettings>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
