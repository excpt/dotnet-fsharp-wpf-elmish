...
type XPViewDataSourceProp =
    | Base of XPDataSourceBaseProp
    | GroupCriteria of DevExpress.Data.Filtering.CriteriaOperator
    | Properties of obj
    | SkipReturnedRecords of int
    | TopReturnedRecords of int

module XPViewDataSource =
    let groupCriteria v : obj = box (XPViewDataSourceProp.GroupCriteria v)
    let properties v : obj = box (XPViewDataSourceProp.Properties v)
    let skipReturnedRecords v : obj = box (XPViewDataSourceProp.SkipReturnedRecords v)
    let topReturnedRecords v : obj = box (XPViewDataSourceProp.TopReturnedRecords v)

    let apply (el: DevExpress.Xpf.Core.DataSources.XPViewDataSource) (prop: XPViewDataSourceProp) =
        match prop with
        | XPViewDataSourceProp.Base p -> XPDataSourceBase.apply el p
        | XPViewDataSourceProp.GroupCriteria v -> el.SetValue(DevExpress.Xpf.Core.DataSources.XPViewDataSource.GroupCriteriaProperty, box v)
        | XPViewDataSourceProp.Properties v -> el.SetValue(DevExpress.Xpf.Core.DataSources.XPViewDataSource.PropertiesProperty, box v)
        | XPViewDataSourceProp.SkipReturnedRecords v -> el.SetValue(DevExpress.Xpf.Core.DataSources.XPViewDataSource.SkipReturnedRecordsProperty, box v)
        | XPViewDataSourceProp.TopReturnedRecords v -> el.SetValue(DevExpress.Xpf.Core.DataSources.XPViewDataSource.TopReturnedRecordsProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DataSources.XPViewDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
