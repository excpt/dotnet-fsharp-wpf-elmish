...
module ResourceMapping =
    let caption v : obj = box (ResourceMappingProp.Caption v)
    let color v : obj = box (ResourceMappingProp.Color v)
    let id v : obj = box (ResourceMappingProp.Id v)
    let image v : obj = box (ResourceMappingProp.Image v)

    let apply (el: DevExpress.Xpf.Scheduler.ResourceMapping) (prop: ResourceMappingProp) =
        match prop with
        | ResourceMappingProp.Caption v -> el.SetValue(DevExpress.Xpf.Scheduler.ResourceMapping.CaptionProperty, box v)
        | ResourceMappingProp.Color v -> el.SetValue(DevExpress.Xpf.Scheduler.ResourceMapping.ColorProperty, box v)
        | ResourceMappingProp.Id v -> el.SetValue(DevExpress.Xpf.Scheduler.ResourceMapping.IdProperty, box v)
        | ResourceMappingProp.Image v -> el.SetValue(DevExpress.Xpf.Scheduler.ResourceMapping.ImageProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.ResourceMapping>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
