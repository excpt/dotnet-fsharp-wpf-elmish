...
module BarControl =
    let barItemDisplayMode v : obj = box (BarControlProp.BarItemDisplayMode v)
    let showBackground v : obj = box (BarControlProp.ShowBackground v)

    let apply (el: DevExpress.Xpf.Bars.BarControl) (prop: BarControlProp) =
        match prop with
        | BarControlProp.Base p -> LinksControl.apply el p
        | BarControlProp.BarItemDisplayMode v -> el.SetValue(DevExpress.Xpf.Bars.BarControl.BarItemDisplayModeProperty, box v)
        | BarControlProp.ShowBackground v -> el.SetValue(DevExpress.Xpf.Bars.BarControl.ShowBackgroundProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.BarControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
