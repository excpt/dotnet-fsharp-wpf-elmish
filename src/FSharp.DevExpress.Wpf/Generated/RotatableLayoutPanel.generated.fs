...
type RotatableLayoutPanelProp =
    | Location of System.Windows.Controls.Dock

module RotatableLayoutPanel =
    let location v : obj = box (RotatableLayoutPanelProp.Location v)

    let apply (el: DevExpress.Xpf.Core.Native.RotatableLayoutPanel) (prop: RotatableLayoutPanelProp) =
        match prop with
        | RotatableLayoutPanelProp.Location v -> el.SetValue(DevExpress.Xpf.Core.Native.RotatableLayoutPanel.LocationProperty, box v)
    ...
    let layoutPosition (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.Native.RotatableLayoutPanel.LayoutPositionProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.Native.RotatableLayoutPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
