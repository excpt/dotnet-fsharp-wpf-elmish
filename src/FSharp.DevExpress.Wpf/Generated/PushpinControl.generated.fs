...
type PushpinControlProp =
    | ItemInfo of DevExpress.Xpf.Map.Native.MapItemInfo
    | State of DevExpress.Xpf.Map.MapPushpinState

module PushpinControl =
    let itemInfo v : obj = box (PushpinControlProp.ItemInfo v)
    let state v : obj = box (PushpinControlProp.State v)

    let apply (el: DevExpress.Xpf.Map.PushpinControl) (prop: PushpinControlProp) =
        match prop with
        | PushpinControlProp.ItemInfo v -> el.SetValue(DevExpress.Xpf.Map.PushpinControl.ItemInfoProperty, box v)
        | PushpinControlProp.State v -> el.SetValue(DevExpress.Xpf.Map.PushpinControl.StateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.PushpinControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
