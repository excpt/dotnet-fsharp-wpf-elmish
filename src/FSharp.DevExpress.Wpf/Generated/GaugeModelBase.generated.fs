...
type GaugeModelBaseProp =
    | Base of GaugeElementProp
    | InnerPadding of System.Windows.Thickness

module GaugeModelBase =
    let innerPadding v : obj = box (GaugeModelBaseProp.InnerPadding v)

    let apply (el: DevExpress.Xpf.Gauges.GaugeModelBase) (prop: GaugeModelBaseProp) =
        match prop with
        | GaugeModelBaseProp.Base p -> GaugeElement.apply el p
        | GaugeModelBaseProp.InnerPadding v -> el.SetValue(DevExpress.Xpf.Gauges.GaugeModelBase.InnerPaddingProperty, box v)
