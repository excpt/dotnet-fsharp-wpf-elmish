...
type BaseCustomizationItemProp =
    | LayoutItem of DevExpress.Xpf.Docking.BaseLayoutItem

module BaseCustomizationItem =
    let layoutItem v : obj = box (BaseCustomizationItemProp.LayoutItem v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.BaseCustomizationItem) (prop: BaseCustomizationItemProp) =
        match prop with
        | BaseCustomizationItemProp.LayoutItem v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.BaseCustomizationItem.LayoutItemProperty, box v)
