...
type UnboundDataSourceProp =
    | Base of SimpleDataSourceBaseProp
    | Properties of obj
    | Count of int

module UnboundDataSource =
    let properties v : obj = box (UnboundDataSourceProp.Properties v)
    let count v : obj = box (UnboundDataSourceProp.Count v)

    let apply (el: DevExpress.Xpf.Core.DataSources.UnboundDataSource) (prop: UnboundDataSourceProp) =
        match prop with
        | UnboundDataSourceProp.Base p -> SimpleDataSourceBase.apply el p
        | UnboundDataSourceProp.Properties v -> el.SetValue(DevExpress.Xpf.Core.DataSources.UnboundDataSource.PropertiesProperty, box v)
        | UnboundDataSourceProp.Count v -> el.SetValue(DevExpress.Xpf.Core.DataSources.UnboundDataSource.CountProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DataSources.UnboundDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
