...
type ODataInstantFeedbackDataSourceProp =
    | Base of WcfInstantFeedbackDataSourceBaseProp
    | KeyExpressions of string
    | OnGetSource of DevExpress.Xpf.Core.ServerMode.GetODataSourceEventHandler
    | OnDismissSource of DevExpress.Xpf.Core.ServerMode.GetODataSourceEventHandler

module ODataInstantFeedbackDataSource =
    let keyExpressions v : obj = box (ODataInstantFeedbackDataSourceProp.KeyExpressions v)
    let onGetSource v : obj = box (EventProp(box (ODataInstantFeedbackDataSourceProp.OnGetSource v)))
    let onDismissSource v : obj = box (EventProp(box (ODataInstantFeedbackDataSourceProp.OnDismissSource v)))

    let apply (el: DevExpress.Xpf.Core.ServerMode.ODataInstantFeedbackDataSource) (prop: ODataInstantFeedbackDataSourceProp) =
        match prop with
        | ODataInstantFeedbackDataSourceProp.Base p -> WcfInstantFeedbackDataSourceBase.apply el p
        | ODataInstantFeedbackDataSourceProp.KeyExpressions v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.ODataInstantFeedbackDataSource.KeyExpressionsProperty, box v)
        | ODataInstantFeedbackDataSourceProp.OnGetSource h -> el.GetSource.AddHandler(h)
        | ODataInstantFeedbackDataSourceProp.OnDismissSource h -> el.DismissSource.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ServerMode.ODataInstantFeedbackDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
