...
type CircularDomainPanelProp =
    | Base of SimplePanelProp
    | Diagram of DevExpress.Xpf.Charts.CircularDiagram2D

module CircularDomainPanel =
    let diagram v : obj = box (CircularDomainPanelProp.Diagram v)

    let apply (el: DevExpress.Xpf.Charts.CircularDomainPanel) (prop: CircularDomainPanelProp) =
        match prop with
        | CircularDomainPanelProp.Base p -> SimplePanel.apply el p
        | CircularDomainPanelProp.Diagram v -> el.SetValue(DevExpress.Xpf.Charts.CircularDomainPanel.DiagramProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.CircularDomainPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
