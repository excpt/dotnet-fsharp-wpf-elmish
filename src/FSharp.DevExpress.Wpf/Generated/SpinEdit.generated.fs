...
type SpinEditProp =
    ...
    | MinValue of obj
    | MaxValue of obj
    | Increment of System.Decimal
    | IsFloatValue of bool
    | AllowRoundOutOfRangeValue of bool

module SpinEdit =
    let value v : obj = box (SpinEditProp.Value v)
    let minValue v : obj = box (SpinEditProp.MinValue v)
    let maxValue v : obj = box (SpinEditProp.MaxValue v)
    let increment v : obj = box (SpinEditProp.Increment v)
    let isFloatValue v : obj = box (SpinEditProp.IsFloatValue v)
    let allowRoundOutOfRangeValue v : obj = box (SpinEditProp.AllowRoundOutOfRangeValue v)

    let apply (el: DevExpress.Xpf.Editors.SpinEdit) (prop: SpinEditProp) =
        match prop with
        | SpinEditProp.Base p -> ButtonEdit.apply el p
        | SpinEditProp.Value v -> el.SetValue(DevExpress.Xpf.Editors.SpinEdit.ValueProperty, box v)
        | SpinEditProp.MinValue v -> el.SetValue(DevExpress.Xpf.Editors.SpinEdit.MinValueProperty, box v)
        | SpinEditProp.MaxValue v -> el.SetValue(DevExpress.Xpf.Editors.SpinEdit.MaxValueProperty, box v)
        | SpinEditProp.Increment v -> el.SetValue(DevExpress.Xpf.Editors.SpinEdit.IncrementProperty, box v)
        | SpinEditProp.IsFloatValue v -> el.SetValue(DevExpress.Xpf.Editors.SpinEdit.IsFloatValueProperty, box v)
        | SpinEditProp.AllowRoundOutOfRangeValue v -> el.SetValue(DevExpress.Xpf.Editors.SpinEdit.AllowRoundOutOfRangeValueProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.SpinEdit>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
