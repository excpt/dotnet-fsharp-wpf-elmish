...
module AxisY2D =
    let dateTimeScaleOptions v : obj = box (AxisY2DProp.DateTimeScaleOptions v)
    let timeSpanScaleOptions v : obj = box (AxisY2DProp.TimeSpanScaleOptions v)
    let numericScaleOptions v : obj = box (AxisY2DProp.NumericScaleOptions v)

    let apply (el: DevExpress.Xpf.Charts.AxisY2D) (prop: AxisY2DProp) =
        match prop with
        | AxisY2DProp.Base p -> Axis2D.apply el p
        | AxisY2DProp.DateTimeScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.AxisY2D.DateTimeScaleOptionsProperty, box v)
        | AxisY2DProp.TimeSpanScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.AxisY2D.TimeSpanScaleOptionsProperty, box v)
        | AxisY2DProp.NumericScaleOptions v -> el.SetValue(DevExpress.Xpf.Charts.AxisY2D.NumericScaleOptionsProperty, box v)
    ...
    let alwaysShowZeroLevel (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.AxisY2D.AlwaysShowZeroLevelProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.AxisY2D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
