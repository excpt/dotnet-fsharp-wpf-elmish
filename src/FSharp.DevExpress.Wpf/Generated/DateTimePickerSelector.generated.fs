...
type DateTimePickerSelectorProp =
    | Base of DXSelectorProp
    | IsAnimated of bool
    | IsExpanded of bool
    | VisibleItemsCount of int
    | UseTransitions of bool

module DateTimePickerSelector =
    let isAnimated v : obj = box (DateTimePickerSelectorProp.IsAnimated v)
    let isExpanded v : obj = box (DateTimePickerSelectorProp.IsExpanded v)
    let visibleItemsCount v : obj = box (DateTimePickerSelectorProp.VisibleItemsCount v)
    let useTransitions v : obj = box (DateTimePickerSelectorProp.UseTransitions v)

    let apply (el: DevExpress.Xpf.Editors.DateTimePickerSelector) (prop: DateTimePickerSelectorProp) =
        match prop with
        | DateTimePickerSelectorProp.Base p -> DXSelector.apply el p
        | DateTimePickerSelectorProp.IsAnimated v -> el.SetValue(DevExpress.Xpf.Editors.DateTimePickerSelector.IsAnimatedProperty, box v)
        | DateTimePickerSelectorProp.IsExpanded v -> el.SetValue(DevExpress.Xpf.Editors.DateTimePickerSelector.IsExpandedProperty, box v)
        | DateTimePickerSelectorProp.VisibleItemsCount v -> el.SetValue(DevExpress.Xpf.Editors.DateTimePickerSelector.VisibleItemsCountProperty, box v)
        | DateTimePickerSelectorProp.UseTransitions v -> el.SetValue(DevExpress.Xpf.Editors.DateTimePickerSelector.UseTransitionsProperty, box v)

    let selectedIndex v : obj = box (DXSelectorProp.SelectedIndex v)
    let selectedItem v : obj = box (DXSelectorProp.SelectedItem v)
    let selectedValue v : obj = box (DXSelectorProp.SelectedValue v)
    let selectedValuePath v : obj = box (DXSelectorProp.SelectedValuePath v)
    let isLooped v : obj = box (DXItemsControlProp.IsLooped v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.DateTimePickerSelector>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
