...
type ImageLayerProp =
    ...
    | DataProvider of DevExpress.Xpf.Map.MapImageDataProviderBase

module ImageLayer =
    let dataProvider v : obj = box (ImageLayerProp.DataProvider v)

    let apply (el: DevExpress.Xpf.Map.ImageLayer) (prop: ImageLayerProp) =
        match prop with
        | ImageLayerProp.Base p -> LayerBase.apply el p
        | ImageLayerProp.DataProvider v -> el.SetValue(DevExpress.Xpf.Map.ImageLayer.DataProviderProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.ImageLayer>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
