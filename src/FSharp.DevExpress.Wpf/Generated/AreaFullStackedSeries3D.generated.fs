...
type AreaFullStackedSeries3DProp =
    | Base of AreaStackedSeries3DProp

module AreaFullStackedSeries3D =

    let apply (el: DevExpress.Xpf.Charts.AreaFullStackedSeries3D) (prop: AreaFullStackedSeries3DProp) =
        match prop with
        | AreaFullStackedSeries3DProp.Base p -> AreaStackedSeries3D.apply el p

    let areaWidth v : obj = box (AreaSeries3DProp.AreaWidth v)
    let material v : obj = box (AreaSeries3DProp.Material v)
    ...
    let percentOptions (v: DevExpress.Xpf.Charts.PercentOptions) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.AreaFullStackedSeries3D.PercentOptionsProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.AreaFullStackedSeries3D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
