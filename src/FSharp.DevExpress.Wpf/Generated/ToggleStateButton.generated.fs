...
type ToggleStateButtonProp =
    | HandleToggle of bool

module ToggleStateButton =
    let handleToggle v : obj = box (ToggleStateButtonProp.HandleToggle v)

    let apply (el: DevExpress.Xpf.Core.ToggleStateButton) (prop: ToggleStateButtonProp) =
        match prop with
        | ToggleStateButtonProp.HandleToggle v -> el.SetValue(DevExpress.Xpf.Core.ToggleStateButton.HandleToggleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ToggleStateButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
