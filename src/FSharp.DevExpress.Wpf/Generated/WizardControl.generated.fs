...
open System
open System.Windows
open DevExpress.Xpf.Controls.Native
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type WizardControlProp =
    | ContentCacheMode of DevExpress.Xpf.Core.TabContentCacheMode
    | ContentHostTemplate of System.Windows.DataTemplate
    | OnSelectionChanged of DevExpress.Xpf.Core.SelectorSelectionChangedEventHandler
    | OnSelectionChanging of DevExpress.Xpf.Core.SelectorSelectionChangingEventHandler

module WizardControl =
    let contentCacheMode v : obj = box (WizardControlProp.ContentCacheMode v)
    let contentHostTemplate v : obj = box (WizardControlProp.ContentHostTemplate v)
    let onSelectionChanged v : obj = box (EventProp(box (WizardControlProp.OnSelectionChanged v)))
    let onSelectionChanging v : obj = box (EventProp(box (WizardControlProp.OnSelectionChanging v)))

    let apply (el: DevExpress.Xpf.Controls.Native.WizardControl) (prop: WizardControlProp) =
        match prop with
        | WizardControlProp.ContentCacheMode v -> el.SetValue(DevExpress.Xpf.Controls.Native.WizardControl.ContentCacheModeProperty, box v)
        | WizardControlProp.ContentHostTemplate v -> el.SetValue(DevExpress.Xpf.Controls.Native.WizardControl.ContentHostTemplateProperty, box v)
        | WizardControlProp.OnSelectionChanged h -> el.SelectionChanged.AddHandler(h)
        | WizardControlProp.OnSelectionChanging h -> el.SelectionChanging.AddHandler(h)
    ...
    let isButtonBack (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Native.WizardControl.IsButtonBackProperty, box v)) :: node.Props }
    let isButtonNext (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Native.WizardControl.IsButtonNextProperty, box v)) :: node.Props }
    let isButtonNavigateTo (v: string) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Native.WizardControl.IsButtonNavigateToProperty, box v)) :: node.Props }
    let isButtonCancel (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Native.WizardControl.IsButtonCancelProperty, box v)) :: node.Props }
    let isButtonFinish (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Native.WizardControl.IsButtonFinishProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Controls.Native.WizardControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
