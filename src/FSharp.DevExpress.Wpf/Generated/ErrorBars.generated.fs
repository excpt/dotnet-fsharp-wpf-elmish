...
type ErrorBarsProp =
    ...
    | EndStyle of DevExpress.Xpf.Charts.ErrorBarEndStyle
    | Direction of DevExpress.Xpf.Charts.ErrorBarDirection

module ErrorBars =
    let endStyle v : obj = box (ErrorBarsProp.EndStyle v)
    let direction v : obj = box (ErrorBarsProp.Direction v)

    let apply (el: DevExpress.Xpf.Charts.ErrorBars) (prop: ErrorBarsProp) =
        match prop with
        | ErrorBarsProp.Base p -> Indicator.apply el p
        | ErrorBarsProp.EndStyle v -> el.SetValue(DevExpress.Xpf.Charts.ErrorBars.EndStyleProperty, box v)
        | ErrorBarsProp.Direction v -> el.SetValue(DevExpress.Xpf.Charts.ErrorBars.DirectionProperty, box v)
