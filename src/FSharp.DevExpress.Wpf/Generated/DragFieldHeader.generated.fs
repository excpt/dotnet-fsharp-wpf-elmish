...
type DragFieldHeaderProp =
    | Base of FieldHeaderProp
    | BestFitWCorrection of int

module DragFieldHeader =
    let bestFitWCorrection v : obj = box (DragFieldHeaderProp.BestFitWCorrection v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.DragFieldHeader) (prop: DragFieldHeaderProp) =
        match prop with
        | DragFieldHeaderProp.Base p -> FieldHeader.apply el p
        | DragFieldHeaderProp.BestFitWCorrection v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.DragFieldHeader.BestFitWCorrectionProperty, box v)
    ...
    let canHide (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.DragFieldHeader.CanHideProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.DragFieldHeader>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
