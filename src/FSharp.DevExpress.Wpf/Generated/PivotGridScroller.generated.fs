...
module PivotGridScroller =
    let scrollBarMode v : obj = box (PivotGridScrollerProp.ScrollBarMode v)
    let cellBackground v : obj = box (PivotGridScrollerProp.CellBackground v)
    let valueBackground v : obj = box (PivotGridScrollerProp.ValueBackground v)
    let resizingIndicatorTemplate v : obj = box (PivotGridScrollerProp.ResizingIndicatorTemplate v)
    let focusedCellBorderTemplate v : obj = box (PivotGridScrollerProp.FocusedCellBorderTemplate v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller) (prop: PivotGridScrollerProp) =
        match prop with
        | PivotGridScrollerProp.ScrollBarMode v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.ScrollBarModeProperty, box v)
        | PivotGridScrollerProp.CellBackground v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.CellBackgroundProperty, box v)
        | PivotGridScrollerProp.ValueBackground v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.ValueBackgroundProperty, box v)
        | PivotGridScrollerProp.ResizingIndicatorTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.ResizingIndicatorTemplateProperty, box v)
        | PivotGridScrollerProp.FocusedCellBorderTemplate v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.FocusedCellBorderTemplateProperty, box v)
    ...
    let scrollPart (v: DevExpress.Xpf.PivotGrid.Internal.IScrollPartInitializer) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.ScrollPartProperty, box v)) :: node.Props }
    let pivotGrid (v: DevExpress.Xpf.PivotGrid.PivotGridControl) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller.PivotGridProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.PivotGridScroller>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
