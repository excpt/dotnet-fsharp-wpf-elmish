...
type TimeRegionDecorationPanelProp =
    ...
    | AllowSplitRegion of bool

module TimeRegionDecorationPanel =
    let itemTemplate v : obj = box (TimeRegionDecorationPanelProp.ItemTemplate v)
    let itemTemplateSelector v : obj = box (TimeRegionDecorationPanelProp.ItemTemplateSelector v)
    let allowSplitRegion v : obj = box (TimeRegionDecorationPanelProp.AllowSplitRegion v)

    let apply (el: DevExpress.Xpf.Scheduling.Panels.TimeRegionDecorationPanel) (prop: TimeRegionDecorationPanelProp) =
        match prop with
        | TimeRegionDecorationPanelProp.Base p -> TimeRegionDecorationPanelBase.apply el p
        | TimeRegionDecorationPanelProp.ItemTemplate v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRegionDecorationPanel.ItemTemplateProperty, box v)
        | TimeRegionDecorationPanelProp.ItemTemplateSelector v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRegionDecorationPanel.ItemTemplateSelectorProperty, box v)
        | TimeRegionDecorationPanelProp.AllowSplitRegion v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.TimeRegionDecorationPanel.AllowSplitRegionProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduling.Panels.TimeRegionDecorationPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
