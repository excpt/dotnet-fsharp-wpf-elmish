...
type XPOSourceBaseProp =
    | Base of DataSourceWithDataBaseProp
    | ObjectType of System.Type

module XPOSourceBase =
    let objectType v : obj = box (XPOSourceBaseProp.ObjectType v)

    let apply (el: DevExpress.Xpf.Core.DataSources.XPOSourceBase) (prop: XPOSourceBaseProp) =
        match prop with
        | XPOSourceBaseProp.Base p -> DataSourceWithDataBase.apply el p
        | XPOSourceBaseProp.ObjectType v -> el.SetValue(DevExpress.Xpf.Core.DataSources.XPOSourceBase.ObjectTypeProperty, box v)
