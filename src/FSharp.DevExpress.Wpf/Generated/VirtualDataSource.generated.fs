...
type VirtualDataSourceProp =
    | Base of SimpleDataSourceBaseProp
    | RowsCount of int
    | PropertiesCount of int

module VirtualDataSource =
    let rowsCount v : obj = box (VirtualDataSourceProp.RowsCount v)
    let propertiesCount v : obj = box (VirtualDataSourceProp.PropertiesCount v)

    let apply (el: DevExpress.Xpf.Core.DataSources.VirtualDataSource) (prop: VirtualDataSourceProp) =
        match prop with
        | VirtualDataSourceProp.Base p -> SimpleDataSourceBase.apply el p
        | VirtualDataSourceProp.RowsCount v -> el.SetValue(DevExpress.Xpf.Core.DataSources.VirtualDataSource.RowsCountProperty, box v)
        | VirtualDataSourceProp.PropertiesCount v -> el.SetValue(DevExpress.Xpf.Core.DataSources.VirtualDataSource.PropertiesCountProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DataSources.VirtualDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
