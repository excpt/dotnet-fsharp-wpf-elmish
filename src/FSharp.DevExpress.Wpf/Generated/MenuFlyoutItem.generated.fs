...
module MenuFlyoutItem =
    let staysOpenOnClick v : obj = box (MenuFlyoutItemProp.StaysOpenOnClick v)
    let command v : obj = box (MenuFlyoutItemProp.Command v)
    let commandParameter v : obj = box (MenuFlyoutItemProp.CommandParameter v)
    let commandTarget v : obj = box (MenuFlyoutItemProp.CommandTarget v)

    let apply (el: DevExpress.Xpf.WindowsUI.MenuFlyoutItem) (prop: MenuFlyoutItemProp) =
        match prop with
        | MenuFlyoutItemProp.Base p -> ClickableBase.apply el p
        | MenuFlyoutItemProp.StaysOpenOnClick v -> el.SetValue(DevExpress.Xpf.WindowsUI.MenuFlyoutItem.StaysOpenOnClickProperty, box v)
        | MenuFlyoutItemProp.Command v -> el.SetValue(DevExpress.Xpf.WindowsUI.MenuFlyoutItem.CommandProperty, box v)
        | MenuFlyoutItemProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.WindowsUI.MenuFlyoutItem.CommandParameterProperty, box v)
        | MenuFlyoutItemProp.CommandTarget v -> el.SetValue(DevExpress.Xpf.WindowsUI.MenuFlyoutItem.CommandTargetProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.MenuFlyoutItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
