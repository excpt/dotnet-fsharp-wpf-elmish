...
type AppBarButtonBaseProp =
    | Base of CommandButtonProp
    | Flyout of DevExpress.Xpf.WindowsUI.Internal.Flyout.FlyoutBase

module AppBarButtonBase =
    let flyout v : obj = box (AppBarButtonBaseProp.Flyout v)

    let apply (el: DevExpress.Xpf.WindowsUI.AppBarButtonBase) (prop: AppBarButtonBaseProp) =
        match prop with
        | AppBarButtonBaseProp.Base p -> CommandButton.apply el p
        | AppBarButtonBaseProp.Flyout v -> el.SetValue(DevExpress.Xpf.WindowsUI.AppBarButtonBase.FlyoutProperty, box v)
