...
type PLinqInstantFeedbackDataSourceProp =
    | Base of PLinqDataSourceBaseProp
    | DefaultSorting of string
    | ListSource of System.ComponentModel.IListSource
    | OnGetEnumerable of DevExpress.Xpf.Core.ServerMode.GetEnumerableEventHandler
    | OnDismissEnumerable of DevExpress.Xpf.Core.ServerMode.GetEnumerableEventHandler

module PLinqInstantFeedbackDataSource =
    let defaultSorting v : obj = box (PLinqInstantFeedbackDataSourceProp.DefaultSorting v)
    let listSource v : obj = box (PLinqInstantFeedbackDataSourceProp.ListSource v)
    let onGetEnumerable v : obj = box (EventProp(box (PLinqInstantFeedbackDataSourceProp.OnGetEnumerable v)))
    let onDismissEnumerable v : obj = box (EventProp(box (PLinqInstantFeedbackDataSourceProp.OnDismissEnumerable v)))

    let apply (el: DevExpress.Xpf.Core.ServerMode.PLinqInstantFeedbackDataSource) (prop: PLinqInstantFeedbackDataSourceProp) =
        match prop with
        | PLinqInstantFeedbackDataSourceProp.Base p -> PLinqDataSourceBase.apply el p
        | PLinqInstantFeedbackDataSourceProp.DefaultSorting v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.PLinqInstantFeedbackDataSource.DefaultSortingProperty, box v)
        | PLinqInstantFeedbackDataSourceProp.ListSource v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.PLinqInstantFeedbackDataSource.ListSourceProperty, box v)
        | PLinqInstantFeedbackDataSourceProp.OnGetEnumerable h -> el.GetEnumerable.AddHandler(h)
        | PLinqInstantFeedbackDataSourceProp.OnDismissEnumerable h -> el.DismissEnumerable.AddHandler(h)

    let itemsSource v : obj = box (ItemsSourceDataSourceBaseProp.ItemsSource v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ServerMode.PLinqInstantFeedbackDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
