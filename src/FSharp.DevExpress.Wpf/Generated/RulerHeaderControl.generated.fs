...
type RulerHeaderControlProp =
    | Caption of string
    | Interval of DevExpress.XtraScheduler.TimeInterval

module RulerHeaderControl =
    let caption v : obj = box (RulerHeaderControlProp.Caption v)
    let interval v : obj = box (RulerHeaderControlProp.Interval v)

    let apply (el: DevExpress.Xpf.Scheduler.RulerHeaderControl) (prop: RulerHeaderControlProp) =
        match prop with
        | RulerHeaderControlProp.Caption v -> el.SetValue(DevExpress.Xpf.Scheduler.RulerHeaderControl.CaptionProperty, box v)
        | RulerHeaderControlProp.Interval v -> el.SetValue(DevExpress.Xpf.Scheduler.RulerHeaderControl.IntervalProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.RulerHeaderControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
