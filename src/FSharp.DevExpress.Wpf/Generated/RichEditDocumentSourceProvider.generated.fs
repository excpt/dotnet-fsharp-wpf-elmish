...
type RichEditDocumentSourceProviderProp =
    | Base of DXDesignTimeControlProp
    | Source of obj

module RichEditDocumentSourceProvider =
    let source v : obj = box (RichEditDocumentSourceProviderProp.Source v)

    let apply (el: DevExpress.Xpf.RichEdit.RichEditDocumentSourceProvider) (prop: RichEditDocumentSourceProviderProp) =
        match prop with
        | RichEditDocumentSourceProviderProp.Base p -> DXDesignTimeControl.apply el p
        | RichEditDocumentSourceProviderProp.Source v -> el.SetValue(DevExpress.Xpf.RichEdit.RichEditDocumentSourceProvider.SourceProperty, box v)
