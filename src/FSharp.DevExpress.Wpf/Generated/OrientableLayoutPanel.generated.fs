...
type OrientableLayoutPanelProp =
    | Orientation of System.Windows.Controls.Orientation
    | HorizontalLayout of DevExpress.Xpf.Core.Native.OrientableLayout
    | VerticalLayout of DevExpress.Xpf.Core.Native.OrientableLayout

module OrientableLayoutPanel =
    let orientation v : obj = box (OrientableLayoutPanelProp.Orientation v)
    let horizontalLayout v : obj = box (OrientableLayoutPanelProp.HorizontalLayout v)
    let verticalLayout v : obj = box (OrientableLayoutPanelProp.VerticalLayout v)

    let apply (el: DevExpress.Xpf.Core.Native.OrientableLayoutPanel) (prop: OrientableLayoutPanelProp) =
        match prop with
        | OrientableLayoutPanelProp.Orientation v -> el.SetValue(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.OrientationProperty, box v)
        | OrientableLayoutPanelProp.HorizontalLayout v -> el.SetValue(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.HorizontalLayoutProperty, box v)
        | OrientableLayoutPanelProp.VerticalLayout v -> el.SetValue(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.VerticalLayoutProperty, box v)
    ...
    let hRow (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.HRowProperty, box v)) :: node.Props }
    let hColumn (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.HColumnProperty, box v)) :: node.Props }
    let vRow (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.VRowProperty, box v)) :: node.Props }
    let vColumn (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.Native.OrientableLayoutPanel.VColumnProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.Native.OrientableLayoutPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
