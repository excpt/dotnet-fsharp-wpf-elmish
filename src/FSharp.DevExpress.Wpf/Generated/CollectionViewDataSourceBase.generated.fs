...
type CollectionViewDataSourceBaseProp =
    | Base of EnumerableDataSourceBaseProp
    | CollectionViewType of System.Type
    | Culture of System.Globalization.CultureInfo
    | GroupDescriptions of obj
    | SortDescriptions of System.ComponentModel.SortDescriptionCollection

module CollectionViewDataSourceBase =
    let collectionViewType v : obj = box (CollectionViewDataSourceBaseProp.CollectionViewType v)
    let culture v : obj = box (CollectionViewDataSourceBaseProp.Culture v)
    let groupDescriptions v : obj = box (CollectionViewDataSourceBaseProp.GroupDescriptions v)
    let sortDescriptions v : obj = box (CollectionViewDataSourceBaseProp.SortDescriptions v)

    let apply (el: DevExpress.Xpf.Core.DataSources.CollectionViewDataSourceBase) (prop: CollectionViewDataSourceBaseProp) =
        match prop with
        | CollectionViewDataSourceBaseProp.Base p -> EnumerableDataSourceBase.apply el p
        | CollectionViewDataSourceBaseProp.CollectionViewType v -> el.SetValue(DevExpress.Xpf.Core.DataSources.CollectionViewDataSourceBase.CollectionViewTypeProperty, box v)
        | CollectionViewDataSourceBaseProp.Culture v -> el.SetValue(DevExpress.Xpf.Core.DataSources.CollectionViewDataSourceBase.CultureProperty, box v)
        | CollectionViewDataSourceBaseProp.GroupDescriptions v -> el.SetValue(DevExpress.Xpf.Core.DataSources.CollectionViewDataSourceBase.GroupDescriptionsProperty, box v)
        | CollectionViewDataSourceBaseProp.SortDescriptions v -> el.SetValue(DevExpress.Xpf.Core.DataSources.CollectionViewDataSourceBase.SortDescriptionsProperty, box v)
