...
type WcfInstantFeedbackDataSourceProp =
    | Base of WcfInstantFeedbackDataSourceBaseProp
    | UseExtendedDataQuery of bool
    | OnGetSource of DevExpress.Xpf.Core.ServerMode.GetWcfSourceEventHandler
    | OnDismissSource of DevExpress.Xpf.Core.ServerMode.GetWcfSourceEventHandler

module WcfInstantFeedbackDataSource =
    let useExtendedDataQuery v : obj = box (WcfInstantFeedbackDataSourceProp.UseExtendedDataQuery v)
    let onGetSource v : obj = box (EventProp(box (WcfInstantFeedbackDataSourceProp.OnGetSource v)))
    let onDismissSource v : obj = box (EventProp(box (WcfInstantFeedbackDataSourceProp.OnDismissSource v)))

    let apply (el: DevExpress.Xpf.Core.ServerMode.WcfInstantFeedbackDataSource) (prop: WcfInstantFeedbackDataSourceProp) =
        match prop with
        | WcfInstantFeedbackDataSourceProp.Base p -> WcfInstantFeedbackDataSourceBase.apply el p
        | WcfInstantFeedbackDataSourceProp.UseExtendedDataQuery v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.WcfInstantFeedbackDataSource.UseExtendedDataQueryProperty, box v)
        | WcfInstantFeedbackDataSourceProp.OnGetSource h -> el.GetSource.AddHandler(h)
        | WcfInstantFeedbackDataSourceProp.OnDismissSource h -> el.DismissSource.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ServerMode.WcfInstantFeedbackDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
