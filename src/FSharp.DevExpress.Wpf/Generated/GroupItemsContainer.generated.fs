...
type GroupItemsContainerProp =
    | Base of XPFContentControlProp
    | NavBarViewKind of DevExpress.Xpf.NavBar.NavBarViewKind
    | ItemsCount of int
    | DisplaySource of DevExpress.Xpf.NavBar.DisplaySource
    | NavBarGroupContent of obj
    | GroupPosition of DevExpress.Xpf.NavBar.GroupPosition
    | AnimationProgress of float
    | ShowBorder of bool

module GroupItemsContainer =
    let navBarViewKind v : obj = box (GroupItemsContainerProp.NavBarViewKind v)
    let itemsCount v : obj = box (GroupItemsContainerProp.ItemsCount v)
    let displaySource v : obj = box (GroupItemsContainerProp.DisplaySource v)
    let navBarGroupContent v : obj = box (GroupItemsContainerProp.NavBarGroupContent v)
    let groupPosition v : obj = box (GroupItemsContainerProp.GroupPosition v)
    let animationProgress v : obj = box (GroupItemsContainerProp.AnimationProgress v)
    let showBorder v : obj = box (GroupItemsContainerProp.ShowBorder v)

    let apply (el: DevExpress.Xpf.NavBar.GroupItemsContainer) (prop: GroupItemsContainerProp) =
        match prop with
        | GroupItemsContainerProp.Base p -> XPFContentControl.apply el p
        | GroupItemsContainerProp.NavBarViewKind v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.NavBarViewKindProperty, box v)
        | GroupItemsContainerProp.ItemsCount v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.ItemsCountProperty, box v)
        | GroupItemsContainerProp.DisplaySource v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.DisplaySourceProperty, box v)
        | GroupItemsContainerProp.NavBarGroupContent v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.NavBarGroupContentProperty, box v)
        | GroupItemsContainerProp.GroupPosition v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.GroupPositionProperty, box v)
        | GroupItemsContainerProp.AnimationProgress v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.AnimationProgressProperty, box v)
        | GroupItemsContainerProp.ShowBorder v -> el.SetValue(DevExpress.Xpf.NavBar.GroupItemsContainer.ShowBorderProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.NavBar.GroupItemsContainer>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
