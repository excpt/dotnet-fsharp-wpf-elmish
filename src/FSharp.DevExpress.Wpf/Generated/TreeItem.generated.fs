...
type TreeItemProp =
    | Base of BaseCustomizationItemProp
    | TreeItemState of DevExpress.Xpf.Docking.VisualElements.TreeItemState

module TreeItem =
    let treeItemState v : obj = box (TreeItemProp.TreeItemState v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.TreeItem) (prop: TreeItemProp) =
        match prop with
        | TreeItemProp.Base p -> BaseCustomizationItem.apply el p
        | TreeItemProp.TreeItemState v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.TreeItem.TreeItemStateProperty, box v)

    let layoutItem v : obj = box (BaseCustomizationItemProp.LayoutItem v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.TreeItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
