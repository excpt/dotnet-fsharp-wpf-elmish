...
module PdfPagesSelector =
    let isSearchControlVisible v : obj = box (PdfPagesSelectorProp.IsSearchControlVisible v)
    let searchParameter v : obj = box (PdfPagesSelectorProp.SearchParameter v)

    let apply (el: DevExpress.Xpf.PdfViewer.PdfPagesSelector) (prop: PdfPagesSelectorProp) =
        match prop with
        | PdfPagesSelectorProp.Base p -> DocumentViewerItemsControl.apply el p
        | PdfPagesSelectorProp.IsSearchControlVisible v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfPagesSelector.IsSearchControlVisibleProperty, box v)
        | PdfPagesSelectorProp.SearchParameter v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfPagesSelector.SearchParameterProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PdfViewer.PdfPagesSelector>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
