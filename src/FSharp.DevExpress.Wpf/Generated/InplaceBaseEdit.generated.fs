...
type InplaceBaseEditProp =
    | Base of ChromeProp
    | EditValue of obj
    | EditCoreStyle of System.Windows.Style

module InplaceBaseEdit =
    let editValue v : obj = box (InplaceBaseEditProp.EditValue v)
    let editCoreStyle v : obj = box (InplaceBaseEditProp.EditCoreStyle v)

    let apply (el: DevExpress.Xpf.Editors.InplaceBaseEdit) (prop: InplaceBaseEditProp) =
        match prop with
        | InplaceBaseEditProp.Base p -> Chrome.apply el p
        | InplaceBaseEditProp.EditValue v -> el.SetValue(DevExpress.Xpf.Editors.InplaceBaseEdit.EditValueProperty, box v)
        | InplaceBaseEditProp.EditCoreStyle v -> el.SetValue(DevExpress.Xpf.Editors.InplaceBaseEdit.EditCoreStyleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.InplaceBaseEdit>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
