...
open System
open System.Windows
open DevExpress.Xpf.Printing.Parameters
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type ParameterLayoutRootProp =
    | Base of ParameterLayoutGroupBaseProp
    | GroupStyle of System.Windows.Style
    | ParameterItemStyle of System.Windows.Style
    | SeparatorStyle of System.Windows.Style

module ParameterLayoutRoot =
    let groupStyle v : obj = box (ParameterLayoutRootProp.GroupStyle v)
    let parameterItemStyle v : obj = box (ParameterLayoutRootProp.ParameterItemStyle v)
    let separatorStyle v : obj = box (ParameterLayoutRootProp.SeparatorStyle v)

    let apply (el: DevExpress.Xpf.Printing.Parameters.ParameterLayoutRoot) (prop: ParameterLayoutRootProp) =
        match prop with
        | ParameterLayoutRootProp.Base p -> ParameterLayoutGroupBase.apply el p
        | ParameterLayoutRootProp.GroupStyle v -> el.SetValue(DevExpress.Xpf.Printing.Parameters.ParameterLayoutRoot.GroupStyleProperty, box v)
        | ParameterLayoutRootProp.ParameterItemStyle v -> el.SetValue(DevExpress.Xpf.Printing.Parameters.ParameterLayoutRoot.ParameterItemStyleProperty, box v)
        | ParameterLayoutRootProp.SeparatorStyle v -> el.SetValue(DevExpress.Xpf.Printing.Parameters.ParameterLayoutRoot.SeparatorStyleProperty, box v)

    let itemsSource v : obj = box (ParameterLayoutGroupBaseProp.ItemsSource v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Printing.Parameters.ParameterLayoutRoot>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
