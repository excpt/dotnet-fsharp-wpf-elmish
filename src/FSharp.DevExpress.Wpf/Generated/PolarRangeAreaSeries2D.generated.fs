...
type PolarRangeAreaSeries2DProp =
    | Base of CircularRangeAreaSeries2DProp

module PolarRangeAreaSeries2D =

    let apply (el: DevExpress.Xpf.Charts.PolarRangeAreaSeries2D) (prop: PolarRangeAreaSeries2DProp) =
        match prop with
        | PolarRangeAreaSeries2DProp.Base p -> CircularRangeAreaSeries2D.apply el p
    ...
    let dateTimeValue2 (v: System.DateTime) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.PolarRangeAreaSeries2D.DateTimeValue2Property, box v)) :: node.Props }
    let timeSpanValue2 (v: System.TimeSpan) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.PolarRangeAreaSeries2D.TimeSpanValue2Property, box v)) :: node.Props }
    let value2 (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.PolarRangeAreaSeries2D.Value2Property, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.PolarRangeAreaSeries2D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
