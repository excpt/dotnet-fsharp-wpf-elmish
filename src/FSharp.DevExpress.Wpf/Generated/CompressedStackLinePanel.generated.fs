...
type CompressedStackLinePanelProp =
    | CompressedIndex of int
    | Orientation of System.Windows.Controls.Orientation
    | LinesBrush of System.Windows.Media.Brush
    | LinesWidth of float

module CompressedStackLinePanel =
    let compressedIndex v : obj = box (CompressedStackLinePanelProp.CompressedIndex v)
    let orientation v : obj = box (CompressedStackLinePanelProp.Orientation v)
    let linesBrush v : obj = box (CompressedStackLinePanelProp.LinesBrush v)
    let linesWidth v : obj = box (CompressedStackLinePanelProp.LinesWidth v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.CompressedStackLinePanel) (prop: CompressedStackLinePanelProp) =
        match prop with
        | CompressedStackLinePanelProp.CompressedIndex v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.CompressedStackLinePanel.CompressedIndexProperty, box v)
        | CompressedStackLinePanelProp.Orientation v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.CompressedStackLinePanel.OrientationProperty, box v)
        | CompressedStackLinePanelProp.LinesBrush v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.CompressedStackLinePanel.LinesBrushProperty, box v)
        | CompressedStackLinePanelProp.LinesWidth v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.CompressedStackLinePanel.LinesWidthProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.CompressedStackLinePanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
