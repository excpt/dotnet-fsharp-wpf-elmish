...
type TimescaleControlProp =
    | Timescales of DevExpress.Xpf.Gantt.TimescaleRulerCollection
    | StartDate of System.DateTime
    | TimescaleRulerStyle of System.Windows.Style
    | TimescaleRulerStyleSelector of System.Windows.Controls.StyleSelector

module TimescaleControl =
    let timescales v : obj = box (TimescaleControlProp.Timescales v)
    let startDate v : obj = box (TimescaleControlProp.StartDate v)
    let timescaleRulerStyle v : obj = box (TimescaleControlProp.TimescaleRulerStyle v)
    let timescaleRulerStyleSelector v : obj = box (TimescaleControlProp.TimescaleRulerStyleSelector v)

    let apply (el: DevExpress.Xpf.Gantt.TimescaleControl) (prop: TimescaleControlProp) =
        match prop with
        | TimescaleControlProp.Timescales v -> el.SetValue(DevExpress.Xpf.Gantt.TimescaleControl.TimescalesProperty, box v)
        | TimescaleControlProp.StartDate v -> el.SetValue(DevExpress.Xpf.Gantt.TimescaleControl.StartDateProperty, box v)
        | TimescaleControlProp.TimescaleRulerStyle v -> el.SetValue(DevExpress.Xpf.Gantt.TimescaleControl.TimescaleRulerStyleProperty, box v)
        | TimescaleControlProp.TimescaleRulerStyleSelector v -> el.SetValue(DevExpress.Xpf.Gantt.TimescaleControl.TimescaleRulerStyleSelectorProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gantt.TimescaleControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
