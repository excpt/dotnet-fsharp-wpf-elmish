...
type PivotFieldListControlProp =
    | Base of FieldListControlBaseProp
    | ShowBorder of bool
    | LabelVisibility of System.Windows.Visibility
    | EmptyTextVisibility of System.Windows.Visibility
    | Area of DevExpress.Xpf.PivotGrid.FieldListArea
    | Orientation of DevExpress.Xpf.PivotGrid.FieldListOrientation
    | ShowHeaderMenu of bool

module PivotFieldListControl =
    let showBorder v : obj = box (PivotFieldListControlProp.ShowBorder v)
    let labelVisibility v : obj = box (PivotFieldListControlProp.LabelVisibility v)
    let emptyTextVisibility v : obj = box (PivotFieldListControlProp.EmptyTextVisibility v)
    let area v : obj = box (PivotFieldListControlProp.Area v)
    let orientation v : obj = box (PivotFieldListControlProp.Orientation v)
    let showHeaderMenu v : obj = box (PivotFieldListControlProp.ShowHeaderMenu v)

    let apply (el: DevExpress.Xpf.PivotGrid.PivotFieldListControl) (prop: PivotFieldListControlProp) =
        match prop with
        | PivotFieldListControlProp.Base p -> FieldListControlBase.apply el p
        | PivotFieldListControlProp.ShowBorder v -> el.SetValue(DevExpress.Xpf.PivotGrid.PivotFieldListControl.ShowBorderProperty, box v)
        | PivotFieldListControlProp.LabelVisibility v -> el.SetValue(DevExpress.Xpf.PivotGrid.PivotFieldListControl.LabelVisibilityProperty, box v)
        | PivotFieldListControlProp.EmptyTextVisibility v -> el.SetValue(DevExpress.Xpf.PivotGrid.PivotFieldListControl.EmptyTextVisibilityProperty, box v)
        | PivotFieldListControlProp.Area v -> el.SetValue(DevExpress.Xpf.PivotGrid.PivotFieldListControl.AreaProperty, box v)
        | PivotFieldListControlProp.Orientation v -> el.SetValue(DevExpress.Xpf.PivotGrid.PivotFieldListControl.OrientationProperty, box v)
        | PivotFieldListControlProp.ShowHeaderMenu v -> el.SetValue(DevExpress.Xpf.PivotGrid.PivotFieldListControl.ShowHeaderMenuProperty, box v)

    let groupFields v : obj = box (FieldListControlBaseProp.GroupFields v)
    let includeVisibleFields v : obj = box (FieldListControlBaseProp.IncludeVisibleFields v)
    let owner v : obj = box (ColumnChooserControlBaseProp.Owner v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.PivotFieldListControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
