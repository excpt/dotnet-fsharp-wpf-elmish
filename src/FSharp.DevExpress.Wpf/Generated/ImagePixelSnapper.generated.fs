...
type ImagePixelSnapperProp =
    | Image of System.Windows.Controls.Image

module ImagePixelSnapper =
    let image v : obj = box (ImagePixelSnapperProp.Image v)

    let apply (el: DevExpress.Xpf.Core.ImagePixelSnapper) (prop: ImagePixelSnapperProp) =
        match prop with
        | ImagePixelSnapperProp.Image v -> el.SetValue(DevExpress.Xpf.Core.ImagePixelSnapper.ImageProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ImagePixelSnapper>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
