...
type CircularSeries2DProp =
    | Base of CircularSeriesBase2DProp
    | MarkerModel of DevExpress.Xpf.Charts.Marker2DModel
    | MarkerSize of int

module CircularSeries2D =
    let markerModel v : obj = box (CircularSeries2DProp.MarkerModel v)
    let markerSize v : obj = box (CircularSeries2DProp.MarkerSize v)

    let apply (el: DevExpress.Xpf.Charts.CircularSeries2D) (prop: CircularSeries2DProp) =
        match prop with
        | CircularSeries2DProp.Base p -> CircularSeriesBase2D.apply el p
        | CircularSeries2DProp.MarkerModel v -> el.SetValue(DevExpress.Xpf.Charts.CircularSeries2D.MarkerModelProperty, box v)
        | CircularSeries2DProp.MarkerSize v -> el.SetValue(DevExpress.Xpf.Charts.CircularSeries2D.MarkerSizeProperty, box v)
    ...
    let angle (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.CircularSeries2D.AngleProperty, box v)) :: node.Props }
