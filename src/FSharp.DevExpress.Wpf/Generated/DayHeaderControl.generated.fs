...
type DayHeaderControlProp =
    | Base of AutoTextFormatControlBaseProp
    | DateFormats of DevExpress.Xpf.Scheduler.Drawing.ObservableStringCollection
    | Date of System.DateTime

module DayHeaderControl =
    let dateFormats v : obj = box (DayHeaderControlProp.DateFormats v)
    let date v : obj = box (DayHeaderControlProp.Date v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.DayHeaderControl) (prop: DayHeaderControlProp) =
        match prop with
        | DayHeaderControlProp.Base p -> AutoTextFormatControlBase.apply el p
        | DayHeaderControlProp.DateFormats v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.DayHeaderControl.DateFormatsProperty, box v)
        | DayHeaderControlProp.Date v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.DayHeaderControl.DateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.DayHeaderControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
