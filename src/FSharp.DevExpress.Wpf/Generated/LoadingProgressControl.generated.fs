...
type LoadingProgressControlProp =
    | Message of string
    | Progress of float
    | TotalProgress of float

module LoadingProgressControl =
    let message v : obj = box (LoadingProgressControlProp.Message v)
    let progress v : obj = box (LoadingProgressControlProp.Progress v)
    let totalProgress v : obj = box (LoadingProgressControlProp.TotalProgress v)

    let apply (el: DevExpress.Xpf.PdfViewer.LoadingProgressControl) (prop: LoadingProgressControlProp) =
        match prop with
        | LoadingProgressControlProp.Message v -> el.SetValue(DevExpress.Xpf.PdfViewer.LoadingProgressControl.MessageProperty, box v)
        | LoadingProgressControlProp.Progress v -> el.SetValue(DevExpress.Xpf.PdfViewer.LoadingProgressControl.ProgressProperty, box v)
        | LoadingProgressControlProp.TotalProgress v -> el.SetValue(DevExpress.Xpf.PdfViewer.LoadingProgressControl.TotalProgressProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PdfViewer.LoadingProgressControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
