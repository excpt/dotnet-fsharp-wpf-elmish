...
type TrackBarEditTickItemProp =
    ...
    | TickLength of float
    | TickContentDistance of float
    | TickOpacity of float

module TrackBarEditTickItem =
    let placement v : obj = box (TrackBarEditTickItemProp.Placement v)
    let displayMode v : obj = box (TrackBarEditTickItemProp.DisplayMode v)
    let tickLength v : obj = box (TrackBarEditTickItemProp.TickLength v)
    let tickContentDistance v : obj = box (TrackBarEditTickItemProp.TickContentDistance v)
    let tickOpacity v : obj = box (TrackBarEditTickItemProp.TickOpacity v)

    let apply (el: DevExpress.Xpf.Editors.TrackBarEditTickItem) (prop: TrackBarEditTickItemProp) =
        match prop with
        | TrackBarEditTickItemProp.Placement v -> el.SetValue(DevExpress.Xpf.Editors.TrackBarEditTickItem.PlacementProperty, box v)
        | TrackBarEditTickItemProp.DisplayMode v -> el.SetValue(DevExpress.Xpf.Editors.TrackBarEditTickItem.DisplayModeProperty, box v)
        | TrackBarEditTickItemProp.TickLength v -> el.SetValue(DevExpress.Xpf.Editors.TrackBarEditTickItem.TickLengthProperty, box v)
        | TrackBarEditTickItemProp.TickContentDistance v -> el.SetValue(DevExpress.Xpf.Editors.TrackBarEditTickItem.TickContentDistanceProperty, box v)
        | TrackBarEditTickItemProp.TickOpacity v -> el.SetValue(DevExpress.Xpf.Editors.TrackBarEditTickItem.TickOpacityProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.TrackBarEditTickItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
