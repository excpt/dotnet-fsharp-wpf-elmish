...
type PageSelectorProp =
    | Base of DocumentViewerItemsControlProp
    | IsSearchControlVisible of bool
    | SearchParameter of DevExpress.Xpf.DocumentViewer.TextSearchParameter

module PageSelector =
    let isSearchControlVisible v : obj = box (PageSelectorProp.IsSearchControlVisible v)
    let searchParameter v : obj = box (PageSelectorProp.SearchParameter v)

    let apply (el: DevExpress.Xpf.Printing.PreviewControl.Native.PageSelector) (prop: PageSelectorProp) =
        match prop with
        | PageSelectorProp.Base p -> DocumentViewerItemsControl.apply el p
        | PageSelectorProp.IsSearchControlVisible v -> el.SetValue(DevExpress.Xpf.Printing.PreviewControl.Native.PageSelector.IsSearchControlVisibleProperty, box v)
        | PageSelectorProp.SearchParameter v -> el.SetValue(DevExpress.Xpf.Printing.PreviewControl.Native.PageSelector.SearchParameterProperty, box v)

    let pageDisplayMode v : obj = box (DocumentViewerItemsControlProp.PageDisplayMode v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Printing.PreviewControl.Native.PageSelector>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
