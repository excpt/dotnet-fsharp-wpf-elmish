...
type SchedulerControlBorderProp =
    | Base of DXSchedulerThemesLoaderProp
...
module SchedulerControlBorder =
    let visibleBorderTemplate v : obj = box (SchedulerControlBorderProp.VisibleBorderTemplate v)
    let invisibleBorderTemplate v : obj = box (SchedulerControlBorderProp.InvisibleBorderTemplate v)
    let showBorder v : obj = box (SchedulerControlBorderProp.ShowBorder v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.SchedulerControlBorder) (prop: SchedulerControlBorderProp) =
        match prop with
        | SchedulerControlBorderProp.Base p -> DXSchedulerThemesLoader.apply el p
        | SchedulerControlBorderProp.VisibleBorderTemplate v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SchedulerControlBorder.VisibleBorderTemplateProperty, box v)
        | SchedulerControlBorderProp.InvisibleBorderTemplate v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SchedulerControlBorder.InvisibleBorderTemplateProperty, box v)
        | SchedulerControlBorderProp.ShowBorder v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SchedulerControlBorder.ShowBorderProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.SchedulerControlBorder>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
