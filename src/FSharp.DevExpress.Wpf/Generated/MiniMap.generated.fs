...
type MiniMapProp =
    | Base of MapElementProp
    | Alignment of DevExpress.Xpf.Map.MiniMapAlignment
    | Behavior of DevExpress.Xpf.Map.MiniMapBehavior
    | EnableZooming of bool
    | EnableScrolling of bool
    | SetMapCenterOnClick of bool
    | ViewportTemplate of System.Windows.DataTemplate

module MiniMap =
    let alignment v : obj = box (MiniMapProp.Alignment v)
    let behavior v : obj = box (MiniMapProp.Behavior v)
    let enableZooming v : obj = box (MiniMapProp.EnableZooming v)
    let enableScrolling v : obj = box (MiniMapProp.EnableScrolling v)
    let setMapCenterOnClick v : obj = box (MiniMapProp.SetMapCenterOnClick v)
    let viewportTemplate v : obj = box (MiniMapProp.ViewportTemplate v)

    let apply (el: DevExpress.Xpf.Map.MiniMap) (prop: MiniMapProp) =
        match prop with
        | MiniMapProp.Base p -> MapElement.apply el p
        | MiniMapProp.Alignment v -> el.SetValue(DevExpress.Xpf.Map.MiniMap.AlignmentProperty, box v)
        | MiniMapProp.Behavior v -> el.SetValue(DevExpress.Xpf.Map.MiniMap.BehaviorProperty, box v)
        | MiniMapProp.EnableZooming v -> el.SetValue(DevExpress.Xpf.Map.MiniMap.EnableZoomingProperty, box v)
        | MiniMapProp.EnableScrolling v -> el.SetValue(DevExpress.Xpf.Map.MiniMap.EnableScrollingProperty, box v)
        | MiniMapProp.SetMapCenterOnClick v -> el.SetValue(DevExpress.Xpf.Map.MiniMap.SetMapCenterOnClickProperty, box v)
        | MiniMapProp.ViewportTemplate v -> el.SetValue(DevExpress.Xpf.Map.MiniMap.ViewportTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.MiniMap>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
