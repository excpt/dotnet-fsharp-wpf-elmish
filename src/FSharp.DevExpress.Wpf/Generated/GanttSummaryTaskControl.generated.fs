...
type GanttSummaryTaskControlProp =
    | Base of GanttTaskControlBaseProp
    | BracketBrush of System.Windows.Media.Brush
    | BracketThickness of System.Windows.Thickness
    | BracketHeight of float
    | BracketIndent of System.Windows.Thickness

module GanttSummaryTaskControl =
    let bracketBrush v : obj = box (GanttSummaryTaskControlProp.BracketBrush v)
    let bracketThickness v : obj = box (GanttSummaryTaskControlProp.BracketThickness v)
    let bracketHeight v : obj = box (GanttSummaryTaskControlProp.BracketHeight v)
    let bracketIndent v : obj = box (GanttSummaryTaskControlProp.BracketIndent v)

    let apply (el: DevExpress.Xpf.Gantt.GanttSummaryTaskControl) (prop: GanttSummaryTaskControlProp) =
        match prop with
        | GanttSummaryTaskControlProp.Base p -> GanttTaskControlBase.apply el p
        | GanttSummaryTaskControlProp.BracketBrush v -> el.SetValue(DevExpress.Xpf.Gantt.GanttSummaryTaskControl.BracketBrushProperty, box v)
        | GanttSummaryTaskControlProp.BracketThickness v -> el.SetValue(DevExpress.Xpf.Gantt.GanttSummaryTaskControl.BracketThicknessProperty, box v)
        | GanttSummaryTaskControlProp.BracketHeight v -> el.SetValue(DevExpress.Xpf.Gantt.GanttSummaryTaskControl.BracketHeightProperty, box v)
        | GanttSummaryTaskControlProp.BracketIndent v -> el.SetValue(DevExpress.Xpf.Gantt.GanttSummaryTaskControl.BracketIndentProperty, box v)

    let progressBackground v : obj = box (GanttTaskControlBaseProp.ProgressBackground v)
    let cornerRadius v : obj = box (GanttTaskControlBaseProp.CornerRadius v)
    let userStyle v : obj = box (GanttItemControlBaseProp.UserStyle v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gantt.GanttSummaryTaskControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
