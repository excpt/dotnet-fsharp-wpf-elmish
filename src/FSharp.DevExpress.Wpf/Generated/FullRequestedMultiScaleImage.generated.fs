...
type FullRequestedMultiScaleImageProp =
    | Base of MultiScaleImageBaseProp
    | Image of DevExpress.Xpf.Map.FullRequestedImage
    | ContextProvider of DevExpress.Map.Native.IFullRequestedContextProvider

module FullRequestedMultiScaleImage =
    let image v : obj = box (FullRequestedMultiScaleImageProp.Image v)
    let contextProvider v : obj = box (FullRequestedMultiScaleImageProp.ContextProvider v)

    let apply (el: DevExpress.Xpf.Map.FullRequestedMultiScaleImage) (prop: FullRequestedMultiScaleImageProp) =
        match prop with
        | FullRequestedMultiScaleImageProp.Base p -> MultiScaleImageBase.apply el p
        | FullRequestedMultiScaleImageProp.Image v -> el.SetValue(DevExpress.Xpf.Map.FullRequestedMultiScaleImage.ImageProperty, box v)
        | FullRequestedMultiScaleImageProp.ContextProvider v -> el.SetValue(DevExpress.Xpf.Map.FullRequestedMultiScaleImage.ContextProviderProperty, box v)

    let useSprings v : obj = box (MultiScaleImageBaseProp.UseSprings v)
    let source v : obj = box (MultiScaleImageBaseProp.Source v)
    let layer v : obj = box (MultiScaleImageBaseProp.Layer v)
