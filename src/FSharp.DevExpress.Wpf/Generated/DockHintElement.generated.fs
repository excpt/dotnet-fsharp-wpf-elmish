...
type DockHintElementProp =
    | Base of psvControlProp
    | ShowShadow of bool

module DockHintElement =
    let showShadow v : obj = box (DockHintElementProp.ShowShadow v)

    let apply (el: DevExpress.Xpf.Docking.Platform.DockHintElement) (prop: DockHintElementProp) =
        match prop with
        | DockHintElementProp.Base p -> psvControl.apply el p
        | DockHintElementProp.ShowShadow v -> el.SetValue(DevExpress.Xpf.Docking.Platform.DockHintElement.ShowShadowProperty, box v)
    ...
    let dockType (v: DevExpress.Xpf.Layout.Core.DockType) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.Platform.DockHintElement.DockTypeProperty, box v)) :: node.Props }
