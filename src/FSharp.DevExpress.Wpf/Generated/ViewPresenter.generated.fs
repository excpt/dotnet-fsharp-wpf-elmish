...
open System
open System.Windows
open DevExpress.Xpf.Core.MvvmSample.Helpers
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type ViewPresenterProp =
    | Content of obj
    | DefaultStoryboard of System.Windows.Media.Animation.Storyboard
    | Storyboards of DevExpress.Xpf.Core.MvvmSample.Helpers.StoryboardCollection
    | Storyboard of string
    | StoryboardSelector of obj
    | OldContentTranslateX of float
    | NewContentTranslateX of float

module ViewPresenter =
    let content v : obj = box (ViewPresenterProp.Content v)
    let defaultStoryboard v : obj = box (ViewPresenterProp.DefaultStoryboard v)
    let storyboards v : obj = box (ViewPresenterProp.Storyboards v)
    let storyboard v : obj = box (ViewPresenterProp.Storyboard v)
    let storyboardSelector v : obj = box (ViewPresenterProp.StoryboardSelector v)
    let oldContentTranslateX v : obj = box (ViewPresenterProp.OldContentTranslateX v)
    let newContentTranslateX v : obj = box (ViewPresenterProp.NewContentTranslateX v)

    let apply (el: DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter) (prop: ViewPresenterProp) =
        match prop with
        | ViewPresenterProp.Content v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.ContentProperty, box v)
        | ViewPresenterProp.DefaultStoryboard v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.DefaultStoryboardProperty, box v)
        | ViewPresenterProp.Storyboards v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.StoryboardsProperty, box v)
        | ViewPresenterProp.Storyboard v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.StoryboardProperty, box v)
        | ViewPresenterProp.StoryboardSelector v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.StoryboardSelectorProperty, box v)
        | ViewPresenterProp.OldContentTranslateX v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.OldContentTranslateXProperty, box v)
        | ViewPresenterProp.NewContentTranslateX v -> el.SetValue(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.NewContentTranslateXProperty, box v)
    ...
    let storyboardName (v: string) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter.StoryboardNameProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.MvvmSample.Helpers.ViewPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
