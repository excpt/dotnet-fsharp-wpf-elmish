...
type DiagramShapeProp =
    | Base of DiagramItemProp
    | Shape of DevExpress.Diagram.Core.ShapeDescription
    | Content of string
    | ContentBackground of System.Windows.Media.Brush
    | CanChangeParameter of bool
    | Parameters of System.Windows.Media.DoubleCollection

module DiagramShape =
    let shape v : obj = box (DiagramShapeProp.Shape v)
    let content v : obj = box (DiagramShapeProp.Content v)
    let contentBackground v : obj = box (DiagramShapeProp.ContentBackground v)
    let canChangeParameter v : obj = box (DiagramShapeProp.CanChangeParameter v)
    let parameters v : obj = box (DiagramShapeProp.Parameters v)

    let apply (el: DevExpress.Xpf.Diagram.DiagramShape) (prop: DiagramShapeProp) =
        match prop with
        | DiagramShapeProp.Base p -> DiagramItem.apply el p
        | DiagramShapeProp.Shape v -> el.SetValue(DevExpress.Xpf.Diagram.DiagramShape.ShapeProperty, box v)
        | DiagramShapeProp.Content v -> el.SetValue(DevExpress.Xpf.Diagram.DiagramShape.ContentProperty, box v)
        | DiagramShapeProp.ContentBackground v -> el.SetValue(DevExpress.Xpf.Diagram.DiagramShape.ContentBackgroundProperty, box v)
        | DiagramShapeProp.CanChangeParameter v -> el.SetValue(DevExpress.Xpf.Diagram.DiagramShape.CanChangeParameterProperty, box v)
        | DiagramShapeProp.Parameters v -> el.SetValue(DevExpress.Xpf.Diagram.DiagramShape.ParametersProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Diagram.DiagramShape>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
