...
type MapToggleButtonProp =
    | Base of MapButtonBaseProp
    | IsActive of bool
    | UndoCommand of System.Windows.Input.ICommand
    | UndoCommandParameter of obj

module MapToggleButton =
    let isActive v : obj = box (MapToggleButtonProp.IsActive v)
    let undoCommand v : obj = box (MapToggleButtonProp.UndoCommand v)
    let undoCommandParameter v : obj = box (MapToggleButtonProp.UndoCommandParameter v)

    let apply (el: DevExpress.Xpf.Map.MapToggleButton) (prop: MapToggleButtonProp) =
        match prop with
        | MapToggleButtonProp.Base p -> MapButtonBase.apply el p
        | MapToggleButtonProp.IsActive v -> el.SetValue(DevExpress.Xpf.Map.MapToggleButton.IsActiveProperty, box v)
        | MapToggleButtonProp.UndoCommand v -> el.SetValue(DevExpress.Xpf.Map.MapToggleButton.UndoCommandProperty, box v)
        | MapToggleButtonProp.UndoCommandParameter v -> el.SetValue(DevExpress.Xpf.Map.MapToggleButton.UndoCommandParameterProperty, box v)

    let command v : obj = box (MapButtonBaseProp.Command v)
    let commandParameter v : obj = box (MapButtonBaseProp.CommandParameter v)
