...
type PLinqInstantFeedbackDataSourceBaseProp =
    | Base of ListSourceDataSourceBaseProp
    | DefaultSorting of string

module PLinqInstantFeedbackDataSourceBase =
    let defaultSorting v : obj = box (PLinqInstantFeedbackDataSourceBaseProp.DefaultSorting v)

    let apply (el: DevExpress.Xpf.Core.DataSources.PLinqInstantFeedbackDataSourceBase) (prop: PLinqInstantFeedbackDataSourceBaseProp) =
        match prop with
        | PLinqInstantFeedbackDataSourceBaseProp.Base p -> ListSourceDataSourceBase.apply el p
        | PLinqInstantFeedbackDataSourceBaseProp.DefaultSorting v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PLinqInstantFeedbackDataSourceBase.DefaultSortingProperty, box v)
