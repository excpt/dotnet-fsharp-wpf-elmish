...
type ChartTextElementProp =
    | Base of ChartElementProp
    | Visible of bool
    | ElementTemplate of System.Windows.DataTemplate

module ChartTextElement =
    let visible v : obj = box (ChartTextElementProp.Visible v)
    let elementTemplate v : obj = box (ChartTextElementProp.ElementTemplate v)

    let apply (el: DevExpress.Xpf.Charts.ChartTextElement) (prop: ChartTextElementProp) =
        match prop with
        | ChartTextElementProp.Base p -> ChartElement.apply el p
        | ChartTextElementProp.Visible v -> el.SetValue(DevExpress.Xpf.Charts.ChartTextElement.VisibleProperty, box v)
        | ChartTextElementProp.ElementTemplate v -> el.SetValue(DevExpress.Xpf.Charts.ChartTextElement.ElementTemplateProperty, box v)
