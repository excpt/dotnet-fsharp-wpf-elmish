...
type CalendarItemProp =
    | Base of CalendarItemBaseProp
    | IsSelected of bool

module CalendarItem =
    let isSelected v : obj = box (CalendarItemProp.IsSelected v)

    let apply (el: DevExpress.Xpf.Editors.RangeControl.CalendarItem) (prop: CalendarItemProp) =
        match prop with
        | CalendarItemProp.Base p -> CalendarItemBase.apply el p
        | CalendarItemProp.IsSelected v -> el.SetValue(DevExpress.Xpf.Editors.RangeControl.CalendarItem.IsSelectedProperty, box v)

    let text v : obj = box (CalendarItemBaseProp.Text v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.RangeControl.CalendarItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
