...
type CrosshairLinesRenderControlProp =
    | LineItems of obj

module CrosshairLinesRenderControl =
    let lineItems v : obj = box (CrosshairLinesRenderControlProp.LineItems v)

    let apply (el: DevExpress.Xpf.Charts.CrosshairLinesRenderControl) (prop: CrosshairLinesRenderControlProp) =
        match prop with
        | CrosshairLinesRenderControlProp.LineItems v -> el.SetValue(DevExpress.Xpf.Charts.CrosshairLinesRenderControl.LineItemsProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.CrosshairLinesRenderControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
