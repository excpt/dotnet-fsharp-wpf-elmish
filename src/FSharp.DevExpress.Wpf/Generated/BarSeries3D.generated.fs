...
type BarSeries3DProp =
    | Base of XYSeries3DProp
    | Model of DevExpress.Xpf.Charts.Bar3DModel
    | BarWidth of float

module BarSeries3D =
    let model v : obj = box (BarSeries3DProp.Model v)
    let barWidth v : obj = box (BarSeries3DProp.BarWidth v)

    let apply (el: DevExpress.Xpf.Charts.BarSeries3D) (prop: BarSeries3DProp) =
        match prop with
        | BarSeries3DProp.Base p -> XYSeries3D.apply el p
        | BarSeries3DProp.Model v -> el.SetValue(DevExpress.Xpf.Charts.BarSeries3D.ModelProperty, box v)
        | BarSeries3DProp.BarWidth v -> el.SetValue(DevExpress.Xpf.Charts.BarSeries3D.BarWidthProperty, box v)
    ...
    let transform (v: System.Windows.Media.Media3D.Transform3D) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.BarSeries3D.TransformProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.BarSeries3D>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
