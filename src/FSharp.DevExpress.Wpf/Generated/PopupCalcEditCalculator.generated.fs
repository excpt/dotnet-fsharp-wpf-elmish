...
type PopupCalcEditCalculatorProp =
    | Base of CalculatorProp

module PopupCalcEditCalculator =

    let apply (el: DevExpress.Xpf.Editors.PopupCalcEditCalculator) (prop: PopupCalcEditCalculatorProp) =
        match prop with
        | PopupCalcEditCalculatorProp.Base p -> Calculator.apply el p
    ...
    let isMemoryIndicator (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Editors.PopupCalcEditCalculator.IsMemoryIndicatorProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Editors.PopupCalcEditCalculator>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
