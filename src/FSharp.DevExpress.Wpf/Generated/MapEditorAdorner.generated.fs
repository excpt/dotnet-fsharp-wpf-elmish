...
type MapEditorAdornerProp =
    | CanRotate of bool
    | CanTransform of bool
    | ShowEditRect of bool
    | AdditionalPoint of obj

module MapEditorAdorner =
    let canRotate v : obj = box (MapEditorAdornerProp.CanRotate v)
    let canTransform v : obj = box (MapEditorAdornerProp.CanTransform v)
    let showEditRect v : obj = box (MapEditorAdornerProp.ShowEditRect v)
    let additionalPoint v : obj = box (MapEditorAdornerProp.AdditionalPoint v)

    let apply (el: DevExpress.Xpf.Map.MapEditorAdorner) (prop: MapEditorAdornerProp) =
        match prop with
        | MapEditorAdornerProp.CanRotate v -> el.SetValue(DevExpress.Xpf.Map.MapEditorAdorner.CanRotateProperty, box v)
        | MapEditorAdornerProp.CanTransform v -> el.SetValue(DevExpress.Xpf.Map.MapEditorAdorner.CanTransformProperty, box v)
        | MapEditorAdornerProp.ShowEditRect v -> el.SetValue(DevExpress.Xpf.Map.MapEditorAdorner.ShowEditRectProperty, box v)
        | MapEditorAdornerProp.AdditionalPoint v -> el.SetValue(DevExpress.Xpf.Map.MapEditorAdorner.AdditionalPointProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.MapEditorAdorner>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
