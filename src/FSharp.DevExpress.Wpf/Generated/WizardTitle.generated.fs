...
type WizardTitleProp =
    | Title of string

module WizardTitle =
    let title v : obj = box (WizardTitleProp.Title v)

    let apply (el: DevExpress.Xpf.DataAccess.DataSourceWizard.Native.WizardTitle) (prop: WizardTitleProp) =
        match prop with
        | WizardTitleProp.Title v -> el.SetValue(DevExpress.Xpf.DataAccess.DataSourceWizard.Native.WizardTitle.TitleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.DataAccess.DataSourceWizard.Native.WizardTitle>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
