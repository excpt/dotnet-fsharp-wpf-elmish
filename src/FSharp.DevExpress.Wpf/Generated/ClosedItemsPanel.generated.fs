...
type ClosedItemsPanelProp =
    | Base of BarContainerControlProp
    | Dock of System.Windows.Controls.Dock

module ClosedItemsPanel =
    let dock v : obj = box (ClosedItemsPanelProp.Dock v)

    let apply (el: DevExpress.Xpf.Docking.ClosedItemsPanel) (prop: ClosedItemsPanelProp) =
        match prop with
        | ClosedItemsPanelProp.Base p -> BarContainerControl.apply el p
        | ClosedItemsPanelProp.Dock v -> el.SetValue(DevExpress.Xpf.Docking.ClosedItemsPanel.DockProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.ClosedItemsPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
