...
module ScrollBox =
    ...
    let apply (el: DevExpress.Xpf.LayoutControl.ScrollBox) (prop: ScrollBoxProp) =
        match prop with
        | ScrollBoxProp.Base p -> LayoutControlBase.apply el p
    ...
    let left (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.ScrollBox.LeftProperty, box v)) :: node.Props }
    let top (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.LayoutControl.ScrollBox.TopProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.LayoutControl.ScrollBox>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
