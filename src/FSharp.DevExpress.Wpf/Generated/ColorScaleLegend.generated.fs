...
type ColorScaleLegendProp =
    | Base of ColorLegendProp
    | EnableGradientScale of bool

module ColorScaleLegend =
    let enableGradientScale v : obj = box (ColorScaleLegendProp.EnableGradientScale v)

    let apply (el: DevExpress.Xpf.Map.ColorScaleLegend) (prop: ColorScaleLegendProp) =
        match prop with
        | ColorScaleLegendProp.Base p -> ColorLegend.apply el p
        | ColorScaleLegendProp.EnableGradientScale v -> el.SetValue(DevExpress.Xpf.Map.ColorScaleLegend.EnableGradientScaleProperty, box v)

    let customItems v : obj = box (ColorLegendProp.CustomItems v)
    let itemTemplate v : obj = box (ColorLegendProp.ItemTemplate v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.ColorScaleLegend>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
