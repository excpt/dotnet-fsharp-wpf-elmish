...
open System
open System.Windows
open DevExpress.Xpf.Grid.Hierarchy
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type CardsHierarchyPanelProp =
    | Base of HierarchyPanelProp
    | CardMargin of System.Windows.Thickness
    | ContainerIndent of float
    | FixedSize of float
    | SeparatorThickness of float
    | MaxCardCountInRow of int
    | CardAlignment of DevExpress.Xpf.Core.Alignment

module CardsHierarchyPanel =
    let cardMargin v : obj = box (CardsHierarchyPanelProp.CardMargin v)
    let containerIndent v : obj = box (CardsHierarchyPanelProp.ContainerIndent v)
    let fixedSize v : obj = box (CardsHierarchyPanelProp.FixedSize v)
    let separatorThickness v : obj = box (CardsHierarchyPanelProp.SeparatorThickness v)
    let maxCardCountInRow v : obj = box (CardsHierarchyPanelProp.MaxCardCountInRow v)
    let cardAlignment v : obj = box (CardsHierarchyPanelProp.CardAlignment v)

    let apply (el: DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel) (prop: CardsHierarchyPanelProp) =
        match prop with
        | CardsHierarchyPanelProp.Base p -> HierarchyPanel.apply el p
        | CardsHierarchyPanelProp.CardMargin v -> el.SetValue(DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel.CardMarginProperty, box v)
        | CardsHierarchyPanelProp.ContainerIndent v -> el.SetValue(DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel.ContainerIndentProperty, box v)
        | CardsHierarchyPanelProp.FixedSize v -> el.SetValue(DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel.FixedSizeProperty, box v)
        | CardsHierarchyPanelProp.SeparatorThickness v -> el.SetValue(DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel.SeparatorThicknessProperty, box v)
        | CardsHierarchyPanelProp.MaxCardCountInRow v -> el.SetValue(DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel.MaxCardCountInRowProperty, box v)
        | CardsHierarchyPanelProp.CardAlignment v -> el.SetValue(DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel.CardAlignmentProperty, box v)

    let itemsContainer v : obj = box (HierarchyPanelProp.ItemsContainer v)
    let orientation v : obj = box (HierarchyPanelProp.Orientation v)
    let dataPresenter v : obj = box (HierarchyPanelProp.DataPresenter v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Grid.Hierarchy.CardsHierarchyPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
