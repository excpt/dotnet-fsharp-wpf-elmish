...
type PdfPageControlProp =
    | Base of PageControlProp
    | Pages of obj
    | IsSelected of bool

module PdfPageControl =
    let pages v : obj = box (PdfPageControlProp.Pages v)
    let isSelected v : obj = box (PdfPageControlProp.IsSelected v)

    let apply (el: DevExpress.Xpf.PdfViewer.PdfPageControl) (prop: PdfPageControlProp) =
        match prop with
        | PdfPageControlProp.Base p -> PageControl.apply el p
        | PdfPageControlProp.Pages v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfPageControl.PagesProperty, box v)
        | PdfPageControlProp.IsSelected v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfPageControl.IsSelectedProperty, box v)

    let pageDisplayMode v : obj = box (PageControlProp.PageDisplayMode v)
    let maxPageCount v : obj = box (PageControlProp.MaxPageCount v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PdfViewer.PdfPageControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
