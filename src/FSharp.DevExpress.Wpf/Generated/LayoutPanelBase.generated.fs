...
type LayoutPanelBaseProp =
    | Base of ScrollControlProp
    | ItemSpacing of float
    | Padding of System.Windows.Thickness

module LayoutPanelBase =
    let itemSpacing v : obj = box (LayoutPanelBaseProp.ItemSpacing v)
    let padding v : obj = box (LayoutPanelBaseProp.Padding v)

    let apply (el: DevExpress.Xpf.Controls.Primitives.LayoutPanelBase) (prop: LayoutPanelBaseProp) =
        match prop with
        | LayoutPanelBaseProp.Base p -> ScrollControl.apply el p
        | LayoutPanelBaseProp.ItemSpacing v -> el.SetValue(DevExpress.Xpf.Controls.Primitives.LayoutPanelBase.ItemSpacingProperty, box v)
        | LayoutPanelBaseProp.Padding v -> el.SetValue(DevExpress.Xpf.Controls.Primitives.LayoutPanelBase.PaddingProperty, box v)
