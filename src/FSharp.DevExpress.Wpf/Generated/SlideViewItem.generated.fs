...
type SlideViewItemProp =
    | Base of veHeaderedContentControlProp
    | Command of System.Windows.Input.ICommand
    | CommandParameter of obj
    | CommandTarget of System.Windows.IInputElement
    | IsHeaderInteractive of bool
    | InteractiveHeaderTemplate of System.Windows.DataTemplate

module SlideViewItem =
    let command v : obj = box (SlideViewItemProp.Command v)
    let commandParameter v : obj = box (SlideViewItemProp.CommandParameter v)
    let commandTarget v : obj = box (SlideViewItemProp.CommandTarget v)
    let isHeaderInteractive v : obj = box (SlideViewItemProp.IsHeaderInteractive v)
    let interactiveHeaderTemplate v : obj = box (SlideViewItemProp.InteractiveHeaderTemplate v)

    let apply (el: DevExpress.Xpf.WindowsUI.SlideViewItem) (prop: SlideViewItemProp) =
        match prop with
        | SlideViewItemProp.Base p -> veHeaderedContentControl.apply el p
        | SlideViewItemProp.Command v -> el.SetValue(DevExpress.Xpf.WindowsUI.SlideViewItem.CommandProperty, box v)
        | SlideViewItemProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.WindowsUI.SlideViewItem.CommandParameterProperty, box v)
        | SlideViewItemProp.CommandTarget v -> el.SetValue(DevExpress.Xpf.WindowsUI.SlideViewItem.CommandTargetProperty, box v)
        | SlideViewItemProp.IsHeaderInteractive v -> el.SetValue(DevExpress.Xpf.WindowsUI.SlideViewItem.IsHeaderInteractiveProperty, box v)
        | SlideViewItemProp.InteractiveHeaderTemplate v -> el.SetValue(DevExpress.Xpf.WindowsUI.SlideViewItem.InteractiveHeaderTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.SlideViewItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
