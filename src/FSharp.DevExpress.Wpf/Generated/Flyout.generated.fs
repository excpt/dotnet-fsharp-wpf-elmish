...
type FlyoutProp =
    ...
    | CloseOnCommandExecute of bool

module Flyout =
    let content v : obj = box (FlyoutProp.Content v)
    let contentTemplate v : obj = box (FlyoutProp.ContentTemplate v)
    let contentTemplateSelector v : obj = box (FlyoutProp.ContentTemplateSelector v)
    let command v : obj = box (FlyoutProp.Command v)
    let commandParameter v : obj = box (FlyoutProp.CommandParameter v)
    let commandTarget v : obj = box (FlyoutProp.CommandTarget v)
    let closeOnCommandExecute v : obj = box (FlyoutProp.CloseOnCommandExecute v)

    let apply (el: DevExpress.Xpf.WindowsUI.Flyout) (prop: FlyoutProp) =
        match prop with
        | FlyoutProp.Base p -> FlyoutBase.apply el p
        | FlyoutProp.Content v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.ContentProperty, box v)
        | FlyoutProp.ContentTemplate v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.ContentTemplateProperty, box v)
        | FlyoutProp.ContentTemplateSelector v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.ContentTemplateSelectorProperty, box v)
        | FlyoutProp.Command v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.CommandProperty, box v)
        | FlyoutProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.CommandParameterProperty, box v)
        | FlyoutProp.CommandTarget v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.CommandTargetProperty, box v)
        | FlyoutProp.CloseOnCommandExecute v -> el.SetValue(DevExpress.Xpf.WindowsUI.Flyout.CloseOnCommandExecuteProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.Flyout>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
