...
type MultiScaleImageProp =
    | Base of MultiScaleTileImageProp
    | TileCacheNamePrefix of string

module MultiScaleImage =
    let tileCacheNamePrefix v : obj = box (MultiScaleImageProp.TileCacheNamePrefix v)

    let apply (el: DevExpress.Xpf.Map.MultiScaleImage) (prop: MultiScaleImageProp) =
        match prop with
        | MultiScaleImageProp.Base p -> MultiScaleTileImage.apply el p
        | MultiScaleImageProp.TileCacheNamePrefix v -> el.SetValue(DevExpress.Xpf.Map.MultiScaleImage.TileCacheNamePrefixProperty, box v)

    let cacheOptions v : obj = box (MultiScaleTileImageProp.CacheOptions v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Map.MultiScaleImage>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
