...
type SchedulerItemsControlProp =
    | Base of XPFItemsControlProp

module SchedulerItemsControl =

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.SchedulerItemsControl) (prop: SchedulerItemsControlProp) =
        match prop with
        | SchedulerItemsControlProp.Base p -> XPFItemsControl.apply el p
    ...
    let elementPosition (v: DevExpress.Xpf.Scheduler.Drawing.ElementPosition) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Scheduler.Drawing.SchedulerItemsControl.ElementPositionProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.SchedulerItemsControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
