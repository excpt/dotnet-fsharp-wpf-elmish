...
type CardsPanelProp =
    ...
    | Owner of DevExpress.Xpf.Core.ICardsPanelOwner

module CardsPanel =
    let fixedSize v : obj = box (CardsPanelProp.FixedSize v)
    let maxCardCountInRow v : obj = box (CardsPanelProp.MaxCardCountInRow v)
    let cardAlignment v : obj = box (CardsPanelProp.CardAlignment v)
    let separatorThickness v : obj = box (CardsPanelProp.SeparatorThickness v)
    let cardMargin v : obj = box (CardsPanelProp.CardMargin v)
    let padding v : obj = box (CardsPanelProp.Padding v)
    let owner v : obj = box (CardsPanelProp.Owner v)

    let apply (el: DevExpress.Xpf.Core.CardsPanel) (prop: CardsPanelProp) =
        match prop with
        | CardsPanelProp.Base p -> OrderPanelBase.apply el p
        | CardsPanelProp.FixedSize v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.FixedSizeProperty, box v)
        | CardsPanelProp.MaxCardCountInRow v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.MaxCardCountInRowProperty, box v)
        | CardsPanelProp.CardAlignment v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.CardAlignmentProperty, box v)
        | CardsPanelProp.SeparatorThickness v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.SeparatorThicknessProperty, box v)
        | CardsPanelProp.CardMargin v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.CardMarginProperty, box v)
        | CardsPanelProp.Padding v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.PaddingProperty, box v)
        | CardsPanelProp.Owner v -> el.SetValue(DevExpress.Xpf.Core.CardsPanel.OwnerProperty, box v)

    let orientation v : obj = box (OrderPanelBaseProp.Orientation v)
    let arrangeAccordingToVisibleIndex v : obj = box (OrderPanelBaseProp.ArrangeAccordingToVisibleIndex v)
    ...
    let cardRow (v: int) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.CardsPanel.CardRowProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.CardsPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
