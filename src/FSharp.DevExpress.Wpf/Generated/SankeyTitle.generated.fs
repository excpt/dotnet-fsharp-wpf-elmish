...
type SankeyTitleProp =
    | Base of SankeyTextElementProp
    | Visible of obj
    | Dock of System.Windows.Controls.Dock

module SankeyTitle =
    let visible v : obj = box (SankeyTitleProp.Visible v)
    let dock v : obj = box (SankeyTitleProp.Dock v)

    let apply (el: DevExpress.Xpf.Charts.Sankey.SankeyTitle) (prop: SankeyTitleProp) =
        match prop with
        | SankeyTitleProp.Base p -> SankeyTextElement.apply el p
        | SankeyTitleProp.Visible v -> el.SetValue(DevExpress.Xpf.Charts.Sankey.SankeyTitle.VisibleProperty, box v)
        | SankeyTitleProp.Dock v -> el.SetValue(DevExpress.Xpf.Charts.Sankey.SankeyTitle.DockProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.Sankey.SankeyTitle>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
