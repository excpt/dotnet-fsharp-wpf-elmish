...
type BarSeries2DProp =
    | Base of BarSeries2DBaseProp
    | Model of DevExpress.Xpf.Charts.Bar2DModel

module BarSeries2D =
    let model v : obj = box (BarSeries2DProp.Model v)

    let apply (el: DevExpress.Xpf.Charts.BarSeries2D) (prop: BarSeries2DProp) =
        match prop with
        | BarSeries2DProp.Base p -> BarSeries2DBase.apply el p
        | BarSeries2DProp.Model v -> el.SetValue(DevExpress.Xpf.Charts.BarSeries2D.ModelProperty, box v)
    ...
    let showZeroValueLabels (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.BarSeries2D.ShowZeroValueLabelsProperty, box v)) :: node.Props }
