...
type NavigationPanePanelProp =
    | Base of NavigationPanePanelBaseProp
    | Header of System.Windows.FrameworkElement
    | ActiveGroup of System.Windows.FrameworkElement
    | Splitter of System.Windows.FrameworkElement
    | GroupsControl of DevExpress.Xpf.NavBar.NavPaneGroupButtonPanelControl
    | OverflowPanel of System.Windows.FrameworkElement

module NavigationPanePanel =
    let header v : obj = box (NavigationPanePanelProp.Header v)
    let activeGroup v : obj = box (NavigationPanePanelProp.ActiveGroup v)
    let splitter v : obj = box (NavigationPanePanelProp.Splitter v)
    let groupsControl v : obj = box (NavigationPanePanelProp.GroupsControl v)
    let overflowPanel v : obj = box (NavigationPanePanelProp.OverflowPanel v)

    let apply (el: DevExpress.Xpf.NavBar.NavigationPanePanel) (prop: NavigationPanePanelProp) =
        match prop with
        | NavigationPanePanelProp.Base p -> NavigationPanePanelBase.apply el p
        | NavigationPanePanelProp.Header v -> el.SetValue(DevExpress.Xpf.NavBar.NavigationPanePanel.HeaderProperty, box v)
        | NavigationPanePanelProp.ActiveGroup v -> el.SetValue(DevExpress.Xpf.NavBar.NavigationPanePanel.ActiveGroupProperty, box v)
        | NavigationPanePanelProp.Splitter v -> el.SetValue(DevExpress.Xpf.NavBar.NavigationPanePanel.SplitterProperty, box v)
        | NavigationPanePanelProp.GroupsControl v -> el.SetValue(DevExpress.Xpf.NavBar.NavigationPanePanel.GroupsControlProperty, box v)
        | NavigationPanePanelProp.OverflowPanel v -> el.SetValue(DevExpress.Xpf.NavBar.NavigationPanePanel.OverflowPanelProperty, box v)

    let orientation v : obj = box (NavigationPanePanelBaseProp.Orientation v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.NavBar.NavigationPanePanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
