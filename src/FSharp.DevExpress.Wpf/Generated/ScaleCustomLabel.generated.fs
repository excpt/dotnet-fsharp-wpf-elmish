...
type ScaleCustomLabelProp =
    | Base of ScaleCustomElementProp
    | Offset of float
    | Value of float

module ScaleCustomLabel =
    let offset v : obj = box (ScaleCustomLabelProp.Offset v)
    let value v : obj = box (ScaleCustomLabelProp.Value v)

    let apply (el: DevExpress.Xpf.Gauges.ScaleCustomLabel) (prop: ScaleCustomLabelProp) =
        match prop with
        | ScaleCustomLabelProp.Base p -> ScaleCustomElement.apply el p
        | ScaleCustomLabelProp.Offset v -> el.SetValue(DevExpress.Xpf.Gauges.ScaleCustomLabel.OffsetProperty, box v)
        | ScaleCustomLabelProp.Value v -> el.SetValue(DevExpress.Xpf.Gauges.ScaleCustomLabel.ValueProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gauges.ScaleCustomLabel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
