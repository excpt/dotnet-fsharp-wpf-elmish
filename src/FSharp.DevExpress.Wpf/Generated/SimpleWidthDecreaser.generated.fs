...
type SimpleWidthDecreaserProp =
    | Element of System.Windows.FrameworkElement
    | MainElement of System.Windows.FrameworkElement
    | DesiredWidth of float
    | MainElementWidth of float

module SimpleWidthDecreaser =
    let element v : obj = box (SimpleWidthDecreaserProp.Element v)
    let mainElement v : obj = box (SimpleWidthDecreaserProp.MainElement v)
    let desiredWidth v : obj = box (SimpleWidthDecreaserProp.DesiredWidth v)
    let mainElementWidth v : obj = box (SimpleWidthDecreaserProp.MainElementWidth v)

    let apply (el: DevExpress.Xpf.PivotGrid.Internal.SimpleWidthDecreaser) (prop: SimpleWidthDecreaserProp) =
        match prop with
        | SimpleWidthDecreaserProp.Element v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.SimpleWidthDecreaser.ElementProperty, box v)
        | SimpleWidthDecreaserProp.MainElement v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.SimpleWidthDecreaser.MainElementProperty, box v)
        | SimpleWidthDecreaserProp.DesiredWidth v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.SimpleWidthDecreaser.DesiredWidthProperty, box v)
        | SimpleWidthDecreaserProp.MainElementWidth v -> el.SetValue(DevExpress.Xpf.PivotGrid.Internal.SimpleWidthDecreaser.MainElementWidthProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.Internal.SimpleWidthDecreaser>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
