...
type LinqServerModeDataSourceProp =
...
module LinqServerModeDataSource =
    let defaultSorting v : obj = box (LinqServerModeDataSourceProp.DefaultSorting v)
    let elementType v : obj = box (LinqServerModeDataSourceProp.ElementType v)
    let keyExpression v : obj = box (LinqServerModeDataSourceProp.KeyExpression v)
    let queryableSource v : obj = box (LinqServerModeDataSourceProp.QueryableSource v)
    let onExceptionThrown v : obj = box (EventProp(box (LinqServerModeDataSourceProp.OnExceptionThrown v)))
    let onInconsistencyDetected v : obj = box (EventProp(box (LinqServerModeDataSourceProp.OnInconsistencyDetected v)))

    let apply (el: DevExpress.Xpf.Core.ServerMode.LinqServerModeDataSource) (prop: LinqServerModeDataSourceProp) =
        match prop with
        | LinqServerModeDataSourceProp.Base p -> ListSourceDataSourceBase.apply el p
        | LinqServerModeDataSourceProp.DefaultSorting v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.LinqServerModeDataSource.DefaultSortingProperty, box v)
        | LinqServerModeDataSourceProp.ElementType v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.LinqServerModeDataSource.ElementTypeProperty, box v)
        | LinqServerModeDataSourceProp.KeyExpression v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.LinqServerModeDataSource.KeyExpressionProperty, box v)
        | LinqServerModeDataSourceProp.QueryableSource v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.LinqServerModeDataSource.QueryableSourceProperty, box v)
        | LinqServerModeDataSourceProp.OnExceptionThrown h -> el.ExceptionThrown.AddHandler(h)
        | LinqServerModeDataSourceProp.OnInconsistencyDetected h -> el.InconsistencyDetected.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.ServerMode.LinqServerModeDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
