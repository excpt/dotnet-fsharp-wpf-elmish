...
type GlyphSideControlProp =
    | ItemPosition of System.Windows.VerticalAlignment

module GlyphSideControl =
    let itemPosition v : obj = box (GlyphSideControlProp.ItemPosition v)

    let apply (el: DevExpress.Xpf.Bars.GlyphSideControl) (prop: GlyphSideControlProp) =
        match prop with
        | GlyphSideControlProp.ItemPosition v -> el.SetValue(DevExpress.Xpf.Bars.GlyphSideControl.ItemPositionProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.GlyphSideControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
