...
type ConnectorPointItemsControlProp =
    | Base of ScaleItemsControlProp

module ConnectorPointItemsControl =

    let apply (el: DevExpress.Xpf.Diagram.ConnectorPointItemsControl) (prop: ConnectorPointItemsControlProp) =
        match prop with
        | ConnectorPointItemsControlProp.Base p -> ScaleItemsControl.apply el p

    let scale v : obj = box (ScaleItemsControlProp.Scale v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Diagram.ConnectorPointItemsControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
