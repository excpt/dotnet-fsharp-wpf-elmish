...
type PivotOlapDataSourceProp =
    | Base of DXDesignTimeControlProp
    | Server of string
    | Catalog of string
    | Cube of string
    | ConnectionTimeout of int
    | LocaleIdentifier of int
    | QueryTimeout of int
    | UserId of string
    | Password of string
    | Provider of string

module PivotOlapDataSource =
    let server v : obj = box (PivotOlapDataSourceProp.Server v)
    let catalog v : obj = box (PivotOlapDataSourceProp.Catalog v)
    let cube v : obj = box (PivotOlapDataSourceProp.Cube v)
    let connectionTimeout v : obj = box (PivotOlapDataSourceProp.ConnectionTimeout v)
    let localeIdentifier v : obj = box (PivotOlapDataSourceProp.LocaleIdentifier v)
    let queryTimeout v : obj = box (PivotOlapDataSourceProp.QueryTimeout v)
    let userId v : obj = box (PivotOlapDataSourceProp.UserId v)
    let password v : obj = box (PivotOlapDataSourceProp.Password v)
    let provider v : obj = box (PivotOlapDataSourceProp.Provider v)

    let apply (el: DevExpress.Xpf.Core.DataSources.PivotOlapDataSource) (prop: PivotOlapDataSourceProp) =
        match prop with
        | PivotOlapDataSourceProp.Base p -> DXDesignTimeControl.apply el p
        | PivotOlapDataSourceProp.Server v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.ServerProperty, box v)
        | PivotOlapDataSourceProp.Catalog v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.CatalogProperty, box v)
        | PivotOlapDataSourceProp.Cube v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.CubeProperty, box v)
        | PivotOlapDataSourceProp.ConnectionTimeout v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.ConnectionTimeoutProperty, box v)
        | PivotOlapDataSourceProp.LocaleIdentifier v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.LocaleIdentifierProperty, box v)
        | PivotOlapDataSourceProp.QueryTimeout v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.QueryTimeoutProperty, box v)
        | PivotOlapDataSourceProp.UserId v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.UserIdProperty, box v)
        | PivotOlapDataSourceProp.Password v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.PasswordProperty, box v)
        | PivotOlapDataSourceProp.Provider v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PivotOlapDataSource.ProviderProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DataSources.PivotOlapDataSource>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
