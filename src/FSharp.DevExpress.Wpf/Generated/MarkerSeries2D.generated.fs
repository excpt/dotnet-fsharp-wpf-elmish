...
type MarkerSeries2DProp =
    | Base of XYSeries2DProp

module MarkerSeries2D =

    let apply (el: DevExpress.Xpf.Charts.MarkerSeries2D) (prop: MarkerSeries2DProp) =
        match prop with
        | MarkerSeries2DProp.Base p -> XYSeries2D.apply el p
    ...
    let angle (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.MarkerSeries2D.AngleProperty, box v)) :: node.Props }
