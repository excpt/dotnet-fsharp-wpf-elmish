...
type ThemeableTickmarkControlProp =
    | Base of PresentationControlProp
    | ThemeBrush of System.Windows.Media.Brush

module ThemeableTickmarkControl =
    let themeBrush v : obj = box (ThemeableTickmarkControlProp.ThemeBrush v)

    let apply (el: DevExpress.Xpf.Gauges.ThemeableTickmarkControl) (prop: ThemeableTickmarkControlProp) =
        match prop with
        | ThemeableTickmarkControlProp.Base p -> PresentationControl.apply el p
        | ThemeableTickmarkControlProp.ThemeBrush v -> el.SetValue(DevExpress.Xpf.Gauges.ThemeableTickmarkControl.ThemeBrushProperty, box v)
