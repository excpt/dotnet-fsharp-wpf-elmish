...
type ErrorRetryPanelProp =
    | RetryCommand of System.Windows.Input.ICommand
    | CopyCommand of System.Windows.Input.ICommand

module ErrorRetryPanel =
    let retryCommand v : obj = box (ErrorRetryPanelProp.RetryCommand v)
    let copyCommand v : obj = box (ErrorRetryPanelProp.CopyCommand v)

    let apply (el: DevExpress.Xpf.Scheduling.Visual.ErrorRetryPanel) (prop: ErrorRetryPanelProp) =
        match prop with
        | ErrorRetryPanelProp.RetryCommand v -> el.SetValue(DevExpress.Xpf.Scheduling.Visual.ErrorRetryPanel.RetryCommandProperty, box v)
        | ErrorRetryPanelProp.CopyCommand v -> el.SetValue(DevExpress.Xpf.Scheduling.Visual.ErrorRetryPanel.CopyCommandProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduling.Visual.ErrorRetryPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
