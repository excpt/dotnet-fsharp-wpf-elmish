...
type DockHintButtonProp =
    | Base of psvControlProp
    | IsHot of bool
    | IsAvailable of bool

module DockHintButton =
    let isHot v : obj = box (DockHintButtonProp.IsHot v)
    let isAvailable v : obj = box (DockHintButtonProp.IsAvailable v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.DockHintButton) (prop: DockHintButtonProp) =
        match prop with
        | DockHintButtonProp.Base p -> psvControl.apply el p
        | DockHintButtonProp.IsHot v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DockHintButton.IsHotProperty, box v)
        | DockHintButtonProp.IsAvailable v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DockHintButton.IsAvailableProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.DockHintButton>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
