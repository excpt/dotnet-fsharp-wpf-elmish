...
type TimeRulerFormProp =
    | Title of string

module TimeRulerForm =
    let title v : obj = box (TimeRulerFormProp.Title v)

    let apply (el: DevExpress.Xpf.Scheduler.UI.TimeRulerForm) (prop: TimeRulerFormProp) =
        match prop with
        | TimeRulerFormProp.Title v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.TimeRulerForm.TitleProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.UI.TimeRulerForm>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
