...
open System
open System.Windows
open DevExpress.Xpf.Scheduler.UI
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type AppointmentInplaceEditorBaseProp =
    | Subject of string
    | Description of string
    | Location of string
    | Label of DevExpress.XtraScheduler.IAppointmentLabel
    | Status of DevExpress.XtraScheduler.IAppointmentStatus

module AppointmentInplaceEditorBase =
    let subject v : obj = box (AppointmentInplaceEditorBaseProp.Subject v)
    let description v : obj = box (AppointmentInplaceEditorBaseProp.Description v)
    let location v : obj = box (AppointmentInplaceEditorBaseProp.Location v)
    let label v : obj = box (AppointmentInplaceEditorBaseProp.Label v)
    let status v : obj = box (AppointmentInplaceEditorBaseProp.Status v)

    let apply (el: DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase) (prop: AppointmentInplaceEditorBaseProp) =
        match prop with
        | AppointmentInplaceEditorBaseProp.Subject v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase.SubjectProperty, box v)
        | AppointmentInplaceEditorBaseProp.Description v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase.DescriptionProperty, box v)
        | AppointmentInplaceEditorBaseProp.Location v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase.LocationProperty, box v)
        | AppointmentInplaceEditorBaseProp.Label v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase.LabelProperty, box v)
        | AppointmentInplaceEditorBaseProp.Status v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase.StatusProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.UI.AppointmentInplaceEditorBase>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
