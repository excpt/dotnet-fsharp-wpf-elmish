...
type FilterColumnProviderProp =
    | CustomColumns of obj
    | Owner of DevExpress.Xpf.PropertyGrid.PropertyGridControl

module FilterColumnProvider =
    let customColumns v : obj = box (FilterColumnProviderProp.CustomColumns v)
    let owner v : obj = box (FilterColumnProviderProp.Owner v)

    let apply (el: DevExpress.Xpf.PropertyGrid.FilterColumnProvider) (prop: FilterColumnProviderProp) =
        match prop with
        | FilterColumnProviderProp.CustomColumns v -> el.SetValue(DevExpress.Xpf.PropertyGrid.FilterColumnProvider.CustomColumnsProperty, box v)
        | FilterColumnProviderProp.Owner v -> el.SetValue(DevExpress.Xpf.PropertyGrid.FilterColumnProvider.OwnerProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PropertyGrid.FilterColumnProvider>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
