...
type PdfPresenterControlProp =
    | Base of DocumentPresenterControlProp
    | AllowCurrentPageHighlighting of bool
    | EnableCaretAnimation of bool

module PdfPresenterControl =
    let allowCurrentPageHighlighting v : obj = box (PdfPresenterControlProp.AllowCurrentPageHighlighting v)
    let enableCaretAnimation v : obj = box (PdfPresenterControlProp.EnableCaretAnimation v)

    let apply (el: DevExpress.Xpf.PdfViewer.PdfPresenterControl) (prop: PdfPresenterControlProp) =
        match prop with
        | PdfPresenterControlProp.Base p -> DocumentPresenterControl.apply el p
        | PdfPresenterControlProp.AllowCurrentPageHighlighting v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfPresenterControl.AllowCurrentPageHighlightingProperty, box v)
        | PdfPresenterControlProp.EnableCaretAnimation v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfPresenterControl.EnableCaretAnimationProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PdfViewer.PdfPresenterControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
