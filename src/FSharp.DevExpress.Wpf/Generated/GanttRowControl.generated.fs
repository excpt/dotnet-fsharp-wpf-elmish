...
type GanttRowControlProp =
    | UserStyle of System.Windows.Style
    | IsSelected of bool
    | SelectedBorderThickness of System.Windows.Thickness
    | SelectedBorderBrush of System.Windows.Media.Brush

module GanttRowControl =
    let userStyle v : obj = box (GanttRowControlProp.UserStyle v)
    let isSelected v : obj = box (GanttRowControlProp.IsSelected v)
    let selectedBorderThickness v : obj = box (GanttRowControlProp.SelectedBorderThickness v)
    let selectedBorderBrush v : obj = box (GanttRowControlProp.SelectedBorderBrush v)

    let apply (el: DevExpress.Xpf.Gantt.GanttRowControl) (prop: GanttRowControlProp) =
        match prop with
        | GanttRowControlProp.UserStyle v -> el.SetValue(DevExpress.Xpf.Gantt.GanttRowControl.UserStyleProperty, box v)
        | GanttRowControlProp.IsSelected v -> el.SetValue(DevExpress.Xpf.Gantt.GanttRowControl.IsSelectedProperty, box v)
        | GanttRowControlProp.SelectedBorderThickness v -> el.SetValue(DevExpress.Xpf.Gantt.GanttRowControl.SelectedBorderThicknessProperty, box v)
        | GanttRowControlProp.SelectedBorderBrush v -> el.SetValue(DevExpress.Xpf.Gantt.GanttRowControl.SelectedBorderBrushProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gantt.GanttRowControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
