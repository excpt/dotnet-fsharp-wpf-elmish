...
type TabOrientablePanelProp =
    | Base of OrientableLayoutPanelProp
    | Indent of System.Windows.Thickness
    | ViewInfo of DevExpress.Xpf.Core.Native.TabViewInfo

module TabOrientablePanel =
    let indent v : obj = box (TabOrientablePanelProp.Indent v)
    let viewInfo v : obj = box (TabOrientablePanelProp.ViewInfo v)

    let apply (el: DevExpress.Xpf.Core.Native.TabOrientablePanel) (prop: TabOrientablePanelProp) =
        match prop with
        | TabOrientablePanelProp.Base p -> OrientableLayoutPanel.apply el p
        | TabOrientablePanelProp.Indent v -> el.SetValue(DevExpress.Xpf.Core.Native.TabOrientablePanel.IndentProperty, box v)
        | TabOrientablePanelProp.ViewInfo v -> el.SetValue(DevExpress.Xpf.Core.Native.TabOrientablePanel.ViewInfoProperty, box v)
    ...
    let childMargin (v: System.Windows.Thickness) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.Native.TabOrientablePanel.ChildMarginProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.Native.TabOrientablePanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
