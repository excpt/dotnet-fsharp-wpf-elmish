...
open System
open System.Windows
open DevExpress.Xpf.PivotGrid.Printing
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type PivotPrintHeaderAreaProp =
    | Base of FieldHeadersProp
    | HasBottomBorder of bool
    | HasTopBorder of bool

module PivotPrintHeaderArea =
    let hasBottomBorder v : obj = box (PivotPrintHeaderAreaProp.HasBottomBorder v)
    let hasTopBorder v : obj = box (PivotPrintHeaderAreaProp.HasTopBorder v)

    let apply (el: DevExpress.Xpf.PivotGrid.Printing.PivotPrintHeaderArea) (prop: PivotPrintHeaderAreaProp) =
        match prop with
        | PivotPrintHeaderAreaProp.Base p -> FieldHeaders.apply el p
        | PivotPrintHeaderAreaProp.HasBottomBorder v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintHeaderArea.HasBottomBorderProperty, box v)
        | PivotPrintHeaderAreaProp.HasTopBorder v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintHeaderArea.HasTopBorderProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.Printing.PivotPrintHeaderArea>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
