...
type GridPopupMenuBaseProp =
    | Base of CustomizablePopupMenuBaseProp

module GridPopupMenuBase =

    let apply (el: DevExpress.Xpf.Bars.GridPopupMenuBase) (prop: GridPopupMenuBaseProp) =
        match prop with
        | GridPopupMenuBaseProp.Base p -> CustomizablePopupMenuBase.apply el p
    ...
    let gridMenuInfo (v: DevExpress.Xpf.Bars.GridMenuInfoBase) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Bars.GridPopupMenuBase.GridMenuInfoProperty, box v)) :: node.Props }
