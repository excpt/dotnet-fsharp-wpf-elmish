...
type CheckEditProp =
    ...
    | CheckedGlyph of System.Windows.Media.ImageSource
    | UncheckedGlyph of System.Windows.Media.ImageSource
    | IndeterminateGlyph of System.Windows.Media.ImageSource
    | GlyphTemplate of System.Windows.DataTemplate
    | CheckBoxWidth of float
    | CheckBoxHeight of float
...
module CheckEdit =
    let isChecked v : obj = box (CheckEditProp.IsChecked v)
    let isThreeState v : obj = box (CheckEditProp.IsThreeState v)
    let reverseCheckOrder v : obj = box (CheckEditProp.ReverseCheckOrder v)
    let clickMode v : obj = box (CheckEditProp.ClickMode v)
    let content v : obj = box (CheckEditProp.Content v)
    let contentTemplate v : obj = box (CheckEditProp.ContentTemplate v)
    let command v : obj = box (CheckEditProp.Command v)
    let commandParameter v : obj = box (CheckEditProp.CommandParameter v)
    let commandTarget v : obj = box (CheckEditProp.CommandTarget v)
    let contentTemplateSelector v : obj = box (CheckEditProp.ContentTemplateSelector v)
    let checkedGlyph v : obj = box (CheckEditProp.CheckedGlyph v)
    let uncheckedGlyph v : obj = box (CheckEditProp.UncheckedGlyph v)
    let indeterminateGlyph v : obj = box (CheckEditProp.IndeterminateGlyph v)
    let glyphTemplate v : obj = box (CheckEditProp.GlyphTemplate v)
    let checkBoxWidth v : obj = box (CheckEditProp.CheckBoxWidth v)
    let checkBoxHeight v : obj = box (CheckEditProp.CheckBoxHeight v)
    let onChecked v : obj = box (EventProp(box (CheckEditProp.OnChecked v)))
    let onUnchecked v : obj = box (EventProp(box (CheckEditProp.OnUnchecked v)))
    let onIndeterminate v : obj = box (EventProp(box (CheckEditProp.OnIndeterminate v)))

    let apply (el: DevExpress.Xpf.Editors.CheckEdit) (prop: CheckEditProp) =
        match prop with
        | CheckEditProp.Base p -> BaseEdit.apply el p
        | CheckEditProp.IsChecked v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.IsCheckedProperty, box v)
        | CheckEditProp.IsThreeState v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.IsThreeStateProperty, box v)
        | CheckEditProp.ReverseCheckOrder v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.ReverseCheckOrderProperty, box v)
        | CheckEditProp.ClickMode v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.ClickModeProperty, box v)
        | CheckEditProp.Content v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.ContentProperty, box v)
        | CheckEditProp.ContentTemplate v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.ContentTemplateProperty, box v)
        | CheckEditProp.Command v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.CommandProperty, box v)
        | CheckEditProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.CommandParameterProperty, box v)
        | CheckEditProp.CommandTarget v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.CommandTargetProperty, box v)
        | CheckEditProp.ContentTemplateSelector v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.ContentTemplateSelectorProperty, box v)
        | CheckEditProp.CheckedGlyph v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.CheckedGlyphProperty, box v)
        | CheckEditProp.UncheckedGlyph v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.UncheckedGlyphProperty, box v)
        | CheckEditProp.IndeterminateGlyph v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.IndeterminateGlyphProperty, box v)
        | CheckEditProp.GlyphTemplate v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.GlyphTemplateProperty, box v)
        | CheckEditProp.CheckBoxWidth v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.CheckBoxWidthProperty, box v)
        | CheckEditProp.CheckBoxHeight v -> el.SetValue(DevExpress.Xpf.Editors.CheckEdit.CheckBoxHeightProperty, box v)
        | CheckEditProp.OnChecked h -> el.Checked.AddHandler(h)
        | CheckEditProp.OnUnchecked h -> el.Unchecked.AddHandler(h)
        | CheckEditProp.OnIndeterminate h -> el.Indeterminate.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.CheckEdit>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
