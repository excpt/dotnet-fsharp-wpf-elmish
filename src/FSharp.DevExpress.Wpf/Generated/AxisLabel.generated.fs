...
type AxisLabelProp =
    ...
    | Formatter of DevExpress.Xpf.Charts.IAxisLabelFormatter

module AxisLabel =
    let staggered v : obj = box (AxisLabelProp.Staggered v)
    let beginText v : obj = box (AxisLabelProp.BeginText v)
    let endText v : obj = box (AxisLabelProp.EndText v)
    let angle v : obj = box (AxisLabelProp.Angle v)
    let textPattern v : obj = box (AxisLabelProp.TextPattern v)
    let formatter v : obj = box (AxisLabelProp.Formatter v)

    let apply (el: DevExpress.Xpf.Charts.AxisLabel) (prop: AxisLabelProp) =
        match prop with
        | AxisLabelProp.Base p -> ChartTextElement.apply el p
        | AxisLabelProp.Staggered v -> el.SetValue(DevExpress.Xpf.Charts.AxisLabel.StaggeredProperty, box v)
        | AxisLabelProp.BeginText v -> el.SetValue(DevExpress.Xpf.Charts.AxisLabel.BeginTextProperty, box v)
        | AxisLabelProp.EndText v -> el.SetValue(DevExpress.Xpf.Charts.AxisLabel.EndTextProperty, box v)
        | AxisLabelProp.Angle v -> el.SetValue(DevExpress.Xpf.Charts.AxisLabel.AngleProperty, box v)
        | AxisLabelProp.TextPattern v -> el.SetValue(DevExpress.Xpf.Charts.AxisLabel.TextPatternProperty, box v)
        | AxisLabelProp.Formatter v -> el.SetValue(DevExpress.Xpf.Charts.AxisLabel.FormatterProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.AxisLabel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
