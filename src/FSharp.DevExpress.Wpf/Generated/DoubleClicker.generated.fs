...
type DoubleClickerProp =
    | CommandParameter of obj
    | Command of System.Windows.Input.ICommand

module DoubleClicker =
    let commandParameter v : obj = box (DoubleClickerProp.CommandParameter v)
    let command v : obj = box (DoubleClickerProp.Command v)

    let apply (el: DevExpress.Xpf.Core.DoubleClicker) (prop: DoubleClickerProp) =
        match prop with
        | DoubleClickerProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.Core.DoubleClicker.CommandParameterProperty, box v)
        | DoubleClickerProp.Command v -> el.SetValue(DevExpress.Xpf.Core.DoubleClicker.CommandProperty, box v)
    ...
    let isDoubleClickArea (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.DoubleClicker.IsDoubleClickAreaProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.DoubleClicker>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
