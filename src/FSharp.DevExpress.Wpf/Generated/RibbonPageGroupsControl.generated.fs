...
type RibbonPageGroupsControlProp =
    | Page of DevExpress.Xpf.Ribbon.RibbonPage
    | HasVisibleGroups of bool
    | IsInSimplifiedRibbon of bool

module RibbonPageGroupsControl =
    let page v : obj = box (RibbonPageGroupsControlProp.Page v)
    let hasVisibleGroups v : obj = box (RibbonPageGroupsControlProp.HasVisibleGroups v)
    let isInSimplifiedRibbon v : obj = box (RibbonPageGroupsControlProp.IsInSimplifiedRibbon v)

    let apply (el: DevExpress.Xpf.Ribbon.RibbonPageGroupsControl) (prop: RibbonPageGroupsControlProp) =
        match prop with
        | RibbonPageGroupsControlProp.Page v -> el.SetValue(DevExpress.Xpf.Ribbon.RibbonPageGroupsControl.PageProperty, box v)
        | RibbonPageGroupsControlProp.HasVisibleGroups v -> el.SetValue(DevExpress.Xpf.Ribbon.RibbonPageGroupsControl.HasVisibleGroupsProperty, box v)
        | RibbonPageGroupsControlProp.IsInSimplifiedRibbon v -> el.SetValue(DevExpress.Xpf.Ribbon.RibbonPageGroupsControl.IsInSimplifiedRibbonProperty, box v)
    ...
    let ribbonPageGroupsControl (v: DevExpress.Xpf.Ribbon.RibbonPageGroupsControl) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Ribbon.RibbonPageGroupsControl.RibbonPageGroupsControlProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Ribbon.RibbonPageGroupsControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
