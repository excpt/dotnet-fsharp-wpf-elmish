...
type BaseItemsPanelProp =
    | Orientation of System.Windows.Controls.Orientation
    | ItemSpacing of float

module BaseItemsPanel =
    let orientation v : obj = box (BaseItemsPanelProp.Orientation v)
    let itemSpacing v : obj = box (BaseItemsPanelProp.ItemSpacing v)

    let apply (el: DevExpress.Xpf.WindowsUI.Internal.BaseItemsPanel) (prop: BaseItemsPanelProp) =
        match prop with
        | BaseItemsPanelProp.Orientation v -> el.SetValue(DevExpress.Xpf.WindowsUI.Internal.BaseItemsPanel.OrientationProperty, box v)
        | BaseItemsPanelProp.ItemSpacing v -> el.SetValue(DevExpress.Xpf.WindowsUI.Internal.BaseItemsPanel.ItemSpacingProperty, box v)
