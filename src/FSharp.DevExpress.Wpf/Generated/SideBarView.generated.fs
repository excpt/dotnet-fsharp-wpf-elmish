...
type SideBarViewProp =
    | Base of NavBarViewBaseProp
    | ActiveGroupMinHeight of float
    | SideBarPanel of DevExpress.Xpf.NavBar.SideBarPanel

module SideBarView =
    let activeGroupMinHeight v : obj = box (SideBarViewProp.ActiveGroupMinHeight v)
    let sideBarPanel v : obj = box (SideBarViewProp.SideBarPanel v)

    let apply (el: DevExpress.Xpf.NavBar.SideBarView) (prop: SideBarViewProp) =
        match prop with
        | SideBarViewProp.Base p -> NavBarViewBase.apply el p
        | SideBarViewProp.ActiveGroupMinHeight v -> el.SetValue(DevExpress.Xpf.NavBar.SideBarView.ActiveGroupMinHeightProperty, box v)
        | SideBarViewProp.SideBarPanel v -> el.SetValue(DevExpress.Xpf.NavBar.SideBarView.SideBarPanelProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.NavBar.SideBarView>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
