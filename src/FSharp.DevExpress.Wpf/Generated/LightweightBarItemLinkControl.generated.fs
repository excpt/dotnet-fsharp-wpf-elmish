...
type LightweightBarItemLinkControlProp =
    | IsInSimplifiedRibbon of bool
    | OnClick of System.Windows.RoutedEventHandler

module LightweightBarItemLinkControl =
    let isInSimplifiedRibbon v : obj = box (LightweightBarItemLinkControlProp.IsInSimplifiedRibbon v)
    let onClick v : obj = box (EventProp(box (LightweightBarItemLinkControlProp.OnClick v)))

    let apply (el: DevExpress.Xpf.Bars.LightweightBarItemLinkControl) (prop: LightweightBarItemLinkControlProp) =
        match prop with
        | LightweightBarItemLinkControlProp.IsInSimplifiedRibbon v -> el.SetValue(DevExpress.Xpf.Bars.LightweightBarItemLinkControl.IsInSimplifiedRibbonProperty, box v)
        | LightweightBarItemLinkControlProp.OnClick h -> el.Click.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.LightweightBarItemLinkControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
