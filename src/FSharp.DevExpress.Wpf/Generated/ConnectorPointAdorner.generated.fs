...
type ConnectorPointAdornerProp =
    | PointIndex of int
    | Connector of DevExpress.Diagram.Core.IDiagramConnector

module ConnectorPointAdorner =
    let pointIndex v : obj = box (ConnectorPointAdornerProp.PointIndex v)
    let connector v : obj = box (ConnectorPointAdornerProp.Connector v)

    let apply (el: DevExpress.Xpf.Diagram.ConnectorPointAdorner) (prop: ConnectorPointAdornerProp) =
        match prop with
        | ConnectorPointAdornerProp.PointIndex v -> el.SetValue(DevExpress.Xpf.Diagram.ConnectorPointAdorner.PointIndexProperty, box v)
        | ConnectorPointAdornerProp.Connector v -> el.SetValue(DevExpress.Xpf.Diagram.ConnectorPointAdorner.ConnectorProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Diagram.ConnectorPointAdorner>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
