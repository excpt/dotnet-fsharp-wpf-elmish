...
type AutoHideGroupProp =
    | Base of LayoutGroupProp
    | DockType of System.Windows.Controls.Dock
    | AutoHideSpeed of int

module AutoHideGroup =
    let dockType v : obj = box (AutoHideGroupProp.DockType v)
    let autoHideSpeed v : obj = box (AutoHideGroupProp.AutoHideSpeed v)

    let apply (el: DevExpress.Xpf.Docking.AutoHideGroup) (prop: AutoHideGroupProp) =
        match prop with
        | AutoHideGroupProp.Base p -> LayoutGroup.apply el p
        | AutoHideGroupProp.DockType v -> el.SetValue(DevExpress.Xpf.Docking.AutoHideGroup.DockTypeProperty, box v)
        | AutoHideGroupProp.AutoHideSpeed v -> el.SetValue(DevExpress.Xpf.Docking.AutoHideGroup.AutoHideSpeedProperty, box v)
    ...
    let autoHideSize (v: System.Windows.Size) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.AutoHideGroup.AutoHideSizeProperty, box v)) :: node.Props }
    let autoHideType (v: DevExpress.Xpf.Docking.AutoHideType) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.AutoHideGroup.AutoHideTypeProperty, box v)) :: node.Props }
    let sizeToContent (v: System.Windows.SizeToContent) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.AutoHideGroup.SizeToContentProperty, box v)) :: node.Props }
    let isAutoHideCenter (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.AutoHideGroup.IsAutoHideCenterProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Docking.AutoHideGroup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
