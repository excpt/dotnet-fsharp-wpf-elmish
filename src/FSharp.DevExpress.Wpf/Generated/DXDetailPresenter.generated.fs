...
type DXDetailPresenterProp =
    | Base of DXContentPresenterProp
    | IsContentVisible of bool
    | DetailTemplate of System.Windows.DataTemplate
    | DetailContent of obj

module DXDetailPresenter =
    let isContentVisible v : obj = box (DXDetailPresenterProp.IsContentVisible v)
    let detailTemplate v : obj = box (DXDetailPresenterProp.DetailTemplate v)
    let detailContent v : obj = box (DXDetailPresenterProp.DetailContent v)

    let apply (el: DevExpress.Xpf.Grid.DXDetailPresenter) (prop: DXDetailPresenterProp) =
        match prop with
        | DXDetailPresenterProp.Base p -> DXContentPresenter.apply el p
        | DXDetailPresenterProp.IsContentVisible v -> el.SetValue(DevExpress.Xpf.Grid.DXDetailPresenter.IsContentVisibleProperty, box v)
        | DXDetailPresenterProp.DetailTemplate v -> el.SetValue(DevExpress.Xpf.Grid.DXDetailPresenter.DetailTemplateProperty, box v)
        | DXDetailPresenterProp.DetailContent v -> el.SetValue(DevExpress.Xpf.Grid.DXDetailPresenter.DetailContentProperty, box v)
    ...
    let isDetailVisible (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.DXDetailPresenter.IsDetailVisibleProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Grid.DXDetailPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
