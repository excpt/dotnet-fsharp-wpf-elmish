...
type TransferControlProp =
    | CurrentContent of obj
    | PreviousContent of obj
    | ControlTemplate of System.Windows.Controls.ControlTemplate
    | PreviousControlTemplate of System.Windows.Controls.ControlTemplate
    | SpeedRatio of float
    | OnContentChanged of System.Windows.RoutedEventHandler

module TransferControl =
    let currentContent v : obj = box (TransferControlProp.CurrentContent v)
    let previousContent v : obj = box (TransferControlProp.PreviousContent v)
    let controlTemplate v : obj = box (TransferControlProp.ControlTemplate v)
    let previousControlTemplate v : obj = box (TransferControlProp.PreviousControlTemplate v)
    let speedRatio v : obj = box (TransferControlProp.SpeedRatio v)
    let onContentChanged v : obj = box (EventProp(box (TransferControlProp.OnContentChanged v)))

    let apply (el: DevExpress.Xpf.Core.TransferControl) (prop: TransferControlProp) =
        match prop with
        | TransferControlProp.CurrentContent v -> el.SetValue(DevExpress.Xpf.Core.TransferControl.CurrentContentProperty, box v)
        | TransferControlProp.PreviousContent v -> el.SetValue(DevExpress.Xpf.Core.TransferControl.PreviousContentProperty, box v)
        | TransferControlProp.ControlTemplate v -> el.SetValue(DevExpress.Xpf.Core.TransferControl.ControlTemplateProperty, box v)
        | TransferControlProp.PreviousControlTemplate v -> el.SetValue(DevExpress.Xpf.Core.TransferControl.PreviousControlTemplateProperty, box v)
        | TransferControlProp.SpeedRatio v -> el.SetValue(DevExpress.Xpf.Core.TransferControl.SpeedRatioProperty, box v)
        | TransferControlProp.OnContentChanged h -> el.ContentChanged.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.TransferControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
