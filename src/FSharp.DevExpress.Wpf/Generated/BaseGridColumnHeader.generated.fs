...
type BaseGridColumnHeaderProp =
    | Base of BaseGridHeaderProp
    | ColumnFilterPopupStyle of System.Windows.Style
    | ShowFilterButtonOnHover of bool

module BaseGridColumnHeader =
    let columnFilterPopupStyle v : obj = box (BaseGridColumnHeaderProp.ColumnFilterPopupStyle v)
    let showFilterButtonOnHover v : obj = box (BaseGridColumnHeaderProp.ShowFilterButtonOnHover v)

    let apply (el: DevExpress.Xpf.Grid.BaseGridColumnHeader) (prop: BaseGridColumnHeaderProp) =
        match prop with
        | BaseGridColumnHeaderProp.Base p -> BaseGridHeader.apply el p
        | BaseGridColumnHeaderProp.ColumnFilterPopupStyle v -> el.SetValue(DevExpress.Xpf.Grid.BaseGridColumnHeader.ColumnFilterPopupStyleProperty, box v)
        | BaseGridColumnHeaderProp.ShowFilterButtonOnHover v -> el.SetValue(DevExpress.Xpf.Grid.BaseGridColumnHeader.ShowFilterButtonOnHoverProperty, box v)
    ...
    let correctDragIndicatorLocation (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.BaseGridColumnHeader.CorrectDragIndicatorLocationProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Grid.BaseGridColumnHeader>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
