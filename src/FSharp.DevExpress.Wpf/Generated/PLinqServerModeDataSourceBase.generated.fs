...
type PLinqServerModeDataSourceBaseProp =
    | Base of ListSourceDataSourceBaseProp
    | DefaultSorting of string

module PLinqServerModeDataSourceBase =
    let defaultSorting v : obj = box (PLinqServerModeDataSourceBaseProp.DefaultSorting v)

    let apply (el: DevExpress.Xpf.Core.DataSources.PLinqServerModeDataSourceBase) (prop: PLinqServerModeDataSourceBaseProp) =
        match prop with
        | PLinqServerModeDataSourceBaseProp.Base p -> ListSourceDataSourceBase.apply el p
        | PLinqServerModeDataSourceBaseProp.DefaultSorting v -> el.SetValue(DevExpress.Xpf.Core.DataSources.PLinqServerModeDataSourceBase.DefaultSortingProperty, box v)
