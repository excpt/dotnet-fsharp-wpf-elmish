...
type GroupPaneProp =
    | Base of psvContentControlProp
    | NoBorderTemplate of System.Windows.DataTemplate
    | GroupTemplate of System.Windows.DataTemplate
    | GroupBoxTemplate of System.Windows.DataTemplate
    | TabbedTemplate of System.Windows.DataTemplate

module GroupPane =
    let noBorderTemplate v : obj = box (GroupPaneProp.NoBorderTemplate v)
    let groupTemplate v : obj = box (GroupPaneProp.GroupTemplate v)
    let groupBoxTemplate v : obj = box (GroupPaneProp.GroupBoxTemplate v)
    let tabbedTemplate v : obj = box (GroupPaneProp.TabbedTemplate v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.GroupPane) (prop: GroupPaneProp) =
        match prop with
        | GroupPaneProp.Base p -> psvContentControl.apply el p
        | GroupPaneProp.NoBorderTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.GroupPane.NoBorderTemplateProperty, box v)
        | GroupPaneProp.GroupTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.GroupPane.GroupTemplateProperty, box v)
        | GroupPaneProp.GroupBoxTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.GroupPane.GroupBoxTemplateProperty, box v)
        | GroupPaneProp.TabbedTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.GroupPane.TabbedTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.GroupPane>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
