...
type NavigationPageProp =
    | TopAppBar of DevExpress.Xpf.WindowsUI.AppBar
    | BottomAppBar of DevExpress.Xpf.WindowsUI.AppBar

module NavigationPage =
    let topAppBar v : obj = box (NavigationPageProp.TopAppBar v)
    let bottomAppBar v : obj = box (NavigationPageProp.BottomAppBar v)

    let apply (el: DevExpress.Xpf.WindowsUI.NavigationPage) (prop: NavigationPageProp) =
        match prop with
        | NavigationPageProp.TopAppBar v -> el.SetValue(DevExpress.Xpf.WindowsUI.NavigationPage.TopAppBarProperty, box v)
        | NavigationPageProp.BottomAppBar v -> el.SetValue(DevExpress.Xpf.WindowsUI.NavigationPage.BottomAppBarProperty, box v)
    ...
    let navigationCacheMode (v: DevExpress.Xpf.WindowsUI.NavigationCacheMode) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.WindowsUI.NavigationPage.NavigationCacheModeProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.WindowsUI.NavigationPage>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
