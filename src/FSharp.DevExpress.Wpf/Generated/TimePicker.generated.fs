...
type TimePickerProp =
    ...
    | OnDateTimeChanged of DevExpress.Xpf.Editors.TimePickerDateTimeChangedEventHandler

module TimePicker =
    let dateTime v : obj = box (TimePickerProp.DateTime v)
    let minValue v : obj = box (TimePickerProp.MinValue v)
    let maxValue v : obj = box (TimePickerProp.MaxValue v)
    let visibleMaskParts v : obj = box (TimePickerProp.VisibleMaskParts v)
    let onDateTimeChanged v : obj = box (EventProp(box (TimePickerProp.OnDateTimeChanged v)))

    let apply (el: DevExpress.Xpf.Editors.TimePicker) (prop: TimePickerProp) =
        match prop with
        | TimePickerProp.DateTime v -> el.SetValue(DevExpress.Xpf.Editors.TimePicker.DateTimeProperty, box v)
        | TimePickerProp.MinValue v -> el.SetValue(DevExpress.Xpf.Editors.TimePicker.MinValueProperty, box v)
        | TimePickerProp.MaxValue v -> el.SetValue(DevExpress.Xpf.Editors.TimePicker.MaxValueProperty, box v)
        | TimePickerProp.VisibleMaskParts v -> el.SetValue(DevExpress.Xpf.Editors.TimePicker.VisibleMaskPartsProperty, box v)
        | TimePickerProp.OnDateTimeChanged h -> el.DateTimeChanged.AddHandler(h)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Editors.TimePicker>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
