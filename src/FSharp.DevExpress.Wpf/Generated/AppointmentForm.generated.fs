...
type AppointmentFormProp =
    | Title of string
    | IsReadOnly of bool
    | CanChangeAllDay of bool

module AppointmentForm =
    let title v : obj = box (AppointmentFormProp.Title v)
    let isReadOnly v : obj = box (AppointmentFormProp.IsReadOnly v)
    let canChangeAllDay v : obj = box (AppointmentFormProp.CanChangeAllDay v)

    let apply (el: DevExpress.Xpf.Scheduler.UI.AppointmentForm) (prop: AppointmentFormProp) =
        match prop with
        | AppointmentFormProp.Title v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentForm.TitleProperty, box v)
        | AppointmentFormProp.IsReadOnly v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentForm.IsReadOnlyProperty, box v)
        | AppointmentFormProp.CanChangeAllDay v -> el.SetValue(DevExpress.Xpf.Scheduler.UI.AppointmentForm.CanChangeAllDayProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.UI.AppointmentForm>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
