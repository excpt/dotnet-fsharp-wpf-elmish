...
type AutoHideTrayProp =
    | Base of psvItemsControlProp
    | CheckInterval of System.TimeSpan
    | ViewStyle of DevExpress.Xpf.Docking.DockingViewStyle

module AutoHideTray =
    let checkInterval v : obj = box (AutoHideTrayProp.CheckInterval v)
    let viewStyle v : obj = box (AutoHideTrayProp.ViewStyle v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.AutoHideTray) (prop: AutoHideTrayProp) =
        match prop with
        | AutoHideTrayProp.Base p -> psvItemsControl.apply el p
        | AutoHideTrayProp.CheckInterval v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.AutoHideTray.CheckIntervalProperty, box v)
        | AutoHideTrayProp.ViewStyle v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.AutoHideTray.ViewStyleProperty, box v)
    ...
    let orientation (v: System.Windows.Controls.Orientation) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.VisualElements.AutoHideTray.OrientationProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.AutoHideTray>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
