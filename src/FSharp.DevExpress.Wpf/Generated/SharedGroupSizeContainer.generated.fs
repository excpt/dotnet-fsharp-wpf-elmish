...
type SharedGroupSizeContainerProp =
    | MeasuredSize of System.Windows.Size

module SharedGroupSizeContainer =
    let measuredSize v : obj = box (SharedGroupSizeContainerProp.MeasuredSize v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.SharedGroupSizeContainer) (prop: SharedGroupSizeContainerProp) =
        match prop with
        | SharedGroupSizeContainerProp.MeasuredSize v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.SharedGroupSizeContainer.MeasuredSizeProperty, box v)
    ...
    let sharedGroupStates (v: DevExpress.Xpf.Scheduler.Drawing.SharedGroupStateCollection) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Scheduler.Drawing.SharedGroupSizeContainer.SharedGroupStatesProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.SharedGroupSizeContainer>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
