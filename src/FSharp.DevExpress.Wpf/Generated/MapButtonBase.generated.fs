...
type MapButtonBaseProp =
    | Command of System.Windows.Input.ICommand
    | CommandParameter of obj

module MapButtonBase =
    let command v : obj = box (MapButtonBaseProp.Command v)
    let commandParameter v : obj = box (MapButtonBaseProp.CommandParameter v)

    let apply (el: DevExpress.Xpf.Map.MapButtonBase) (prop: MapButtonBaseProp) =
        match prop with
        | MapButtonBaseProp.Command v -> el.SetValue(DevExpress.Xpf.Map.MapButtonBase.CommandProperty, box v)
        | MapButtonBaseProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.Map.MapButtonBase.CommandParameterProperty, box v)
