...
type SelectionGeometryControlProp =
    | Base of SelectionControlProp
    | Geometry of obj

module SelectionGeometryControl =
    let geometry v : obj = box (SelectionGeometryControlProp.Geometry v)

    let apply (el: DevExpress.Xpf.Charts.SelectionGeometryControl) (prop: SelectionGeometryControlProp) =
        match prop with
        | SelectionGeometryControlProp.Base p -> SelectionControl.apply el p
        | SelectionGeometryControlProp.Geometry v -> el.SetValue(DevExpress.Xpf.Charts.SelectionGeometryControl.GeometryProperty, box v)

    let isSelected v : obj = box (SelectionControlProp.IsSelected v)
    let isHighlighted v : obj = box (SelectionControlProp.IsHighlighted v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.SelectionGeometryControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
