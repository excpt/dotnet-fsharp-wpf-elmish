...
open System
open System.Windows
open DevExpress.Xpf.Printing.PreviewControl.Native
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type SearchControlContainerProp =
    | Base of SearchControlContainerProp
    | SearchParameter of DevExpress.Xpf.DocumentViewer.TextSearchParameter
    | WholeWord of bool
    | IsCaseSensitive of bool
    | SearchText of string
    | IsSearchControlVisible of bool

module SearchControlContainer =
    let searchParameter v : obj = box (SearchControlContainerProp.SearchParameter v)
    let wholeWord v : obj = box (SearchControlContainerProp.WholeWord v)
    let isCaseSensitive v : obj = box (SearchControlContainerProp.IsCaseSensitive v)
    let searchText v : obj = box (SearchControlContainerProp.SearchText v)
    let isSearchControlVisible v : obj = box (SearchControlContainerProp.IsSearchControlVisible v)

    let apply (el: DevExpress.Xpf.Printing.PreviewControl.Native.SearchControlContainer) (prop: SearchControlContainerProp) =
        match prop with
        | SearchControlContainerProp.Base p -> SearchControlContainer.apply el p
        | SearchControlContainerProp.SearchParameter v -> el.SetValue(DevExpress.Xpf.DocumentViewer.SearchControlContainer.SearchParameterProperty, box v)
        | SearchControlContainerProp.WholeWord v -> el.SetValue(DevExpress.Xpf.DocumentViewer.SearchControlContainer.WholeWordProperty, box v)
        | SearchControlContainerProp.IsCaseSensitive v -> el.SetValue(DevExpress.Xpf.DocumentViewer.SearchControlContainer.IsCaseSensitiveProperty, box v)
        | SearchControlContainerProp.SearchText v -> el.SetValue(DevExpress.Xpf.DocumentViewer.SearchControlContainer.SearchTextProperty, box v)
        | SearchControlContainerProp.IsSearchControlVisible v -> el.SetValue(DevExpress.Xpf.DocumentViewer.SearchControlContainer.IsSearchControlVisibleProperty, box v)
    ...
    let actualSearchContainer (v: DevExpress.Xpf.DocumentViewer.SearchControlContainer) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.DocumentViewer.SearchControlContainer.ActualSearchContainerProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Printing.PreviewControl.Native.SearchControlContainer>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
