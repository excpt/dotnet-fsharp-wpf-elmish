...
type FocusRectPresenterProp =
    | View of DevExpress.Xpf.Grid.DataViewBase
    | SizeExpansion of int
    | ChildTemplate of System.Windows.Controls.ControlTemplate

module FocusRectPresenter =
    let view v : obj = box (FocusRectPresenterProp.View v)
    let sizeExpansion v : obj = box (FocusRectPresenterProp.SizeExpansion v)
    let childTemplate v : obj = box (FocusRectPresenterProp.ChildTemplate v)

    let apply (el: DevExpress.Xpf.Grid.FocusRectPresenter) (prop: FocusRectPresenterProp) =
        match prop with
        | FocusRectPresenterProp.View v -> el.SetValue(DevExpress.Xpf.Grid.FocusRectPresenter.ViewProperty, box v)
        | FocusRectPresenterProp.SizeExpansion v -> el.SetValue(DevExpress.Xpf.Grid.FocusRectPresenter.SizeExpansionProperty, box v)
        | FocusRectPresenterProp.ChildTemplate v -> el.SetValue(DevExpress.Xpf.Grid.FocusRectPresenter.ChildTemplateProperty, box v)
    ...
    let isHorizontalScrollHost (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.FocusRectPresenter.IsHorizontalScrollHostProperty, box v)) :: node.Props }
    let isVerticalScrollHost (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Grid.FocusRectPresenter.IsVerticalScrollHostProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Grid.FocusRectPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
