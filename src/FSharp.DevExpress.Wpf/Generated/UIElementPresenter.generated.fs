...
type UIElementPresenterProp =
    | Base of psvDecoratorProp
    | UIElement of System.Windows.UIElement

module UIElementPresenter =
    let uIElement v : obj = box (UIElementPresenterProp.UIElement v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.UIElementPresenter) (prop: UIElementPresenterProp) =
        match prop with
        | UIElementPresenterProp.Base p -> psvDecorator.apply el p
        | UIElementPresenterProp.UIElement v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.UIElementPresenter.UIElementProperty, box v)

    let child v : obj = box (psvDecoratorProp.Child v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.UIElementPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
