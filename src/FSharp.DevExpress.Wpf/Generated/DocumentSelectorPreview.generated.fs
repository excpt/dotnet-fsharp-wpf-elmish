...
type DocumentSelectorPreviewProp =
    | Base of psvControlProp
    | Target of DevExpress.Xpf.Docking.LayoutPanel
    | DocumentBorderTemplate of System.Windows.DataTemplate
    | PanelBorderTemplate of System.Windows.DataTemplate

module DocumentSelectorPreview =
    let target v : obj = box (DocumentSelectorPreviewProp.Target v)
    let documentBorderTemplate v : obj = box (DocumentSelectorPreviewProp.DocumentBorderTemplate v)
    let panelBorderTemplate v : obj = box (DocumentSelectorPreviewProp.PanelBorderTemplate v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.DocumentSelectorPreview) (prop: DocumentSelectorPreviewProp) =
        match prop with
        | DocumentSelectorPreviewProp.Base p -> psvControl.apply el p
        | DocumentSelectorPreviewProp.Target v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelectorPreview.TargetProperty, box v)
        | DocumentSelectorPreviewProp.DocumentBorderTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelectorPreview.DocumentBorderTemplateProperty, box v)
        | DocumentSelectorPreviewProp.PanelBorderTemplate v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelectorPreview.PanelBorderTemplateProperty, box v)
    ...
    let isInPreview (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Docking.VisualElements.DocumentSelectorPreview.IsInPreviewProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.DocumentSelectorPreview>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
