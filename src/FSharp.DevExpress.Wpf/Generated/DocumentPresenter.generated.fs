...
type DocumentPresenterProp =
    | Document of string

module DocumentPresenter =
    let document v : obj = box (DocumentPresenterProp.Document v)

    let apply (el: DevExpress.Xpf.Core.DocumentPresenter) (prop: DocumentPresenterProp) =
        match prop with
        | DocumentPresenterProp.Document v -> el.SetValue(DevExpress.Xpf.Core.DocumentPresenter.DocumentProperty, box v)
    ...
    let navigateUri (v: string) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.DocumentPresenter.NavigateUriProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Core.DocumentPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
