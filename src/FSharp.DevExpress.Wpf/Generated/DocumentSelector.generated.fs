...
type DocumentSelectorProp =
    | Base of psvControlProp
    | PanelIndex of int
    | DocumentIndex of int
    | PanelsCaption of string
    | DocumentsCaption of string
    | BindableDocumentIndex of int
    | BindablePanelIndex of int

module DocumentSelector =
    let panelIndex v : obj = box (DocumentSelectorProp.PanelIndex v)
    let documentIndex v : obj = box (DocumentSelectorProp.DocumentIndex v)
    let panelsCaption v : obj = box (DocumentSelectorProp.PanelsCaption v)
    let documentsCaption v : obj = box (DocumentSelectorProp.DocumentsCaption v)
    let bindableDocumentIndex v : obj = box (DocumentSelectorProp.BindableDocumentIndex v)
    let bindablePanelIndex v : obj = box (DocumentSelectorProp.BindablePanelIndex v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.DocumentSelector) (prop: DocumentSelectorProp) =
        match prop with
        | DocumentSelectorProp.Base p -> psvControl.apply el p
        | DocumentSelectorProp.PanelIndex v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelector.PanelIndexProperty, box v)
        | DocumentSelectorProp.DocumentIndex v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelector.DocumentIndexProperty, box v)
        | DocumentSelectorProp.PanelsCaption v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelector.PanelsCaptionProperty, box v)
        | DocumentSelectorProp.DocumentsCaption v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelector.DocumentsCaptionProperty, box v)
        | DocumentSelectorProp.BindableDocumentIndex v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelector.BindableDocumentIndexProperty, box v)
        | DocumentSelectorProp.BindablePanelIndex v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.DocumentSelector.BindablePanelIndexProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.DocumentSelector>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
