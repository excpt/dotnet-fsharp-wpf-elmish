...
type InstantFeedbackDataSourceBaseProp =
    | Base of ListSourceDataSourceBaseProp
    | AreSourceRowsThreadSafe of bool
    | DefaultSorting of string
    | KeyExpression of string
    | QueryableSource of System.Linq.IQueryable
    | OnGetQueryable of DevExpress.Xpf.Core.ServerMode.GetQueryableEventHandler
    | OnDismissQueryable of DevExpress.Xpf.Core.ServerMode.GetQueryableEventHandler

module InstantFeedbackDataSourceBase =
    let areSourceRowsThreadSafe v : obj = box (InstantFeedbackDataSourceBaseProp.AreSourceRowsThreadSafe v)
    let defaultSorting v : obj = box (InstantFeedbackDataSourceBaseProp.DefaultSorting v)
    let keyExpression v : obj = box (InstantFeedbackDataSourceBaseProp.KeyExpression v)
    let queryableSource v : obj = box (InstantFeedbackDataSourceBaseProp.QueryableSource v)
    let onGetQueryable v : obj = box (EventProp(box (InstantFeedbackDataSourceBaseProp.OnGetQueryable v)))
    let onDismissQueryable v : obj = box (EventProp(box (InstantFeedbackDataSourceBaseProp.OnDismissQueryable v)))

    let apply (el: DevExpress.Xpf.Core.ServerMode.InstantFeedbackDataSourceBase) (prop: InstantFeedbackDataSourceBaseProp) =
        match prop with
        | InstantFeedbackDataSourceBaseProp.Base p -> ListSourceDataSourceBase.apply el p
        | InstantFeedbackDataSourceBaseProp.AreSourceRowsThreadSafe v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.InstantFeedbackDataSourceBase.AreSourceRowsThreadSafeProperty, box v)
        | InstantFeedbackDataSourceBaseProp.DefaultSorting v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.InstantFeedbackDataSourceBase.DefaultSortingProperty, box v)
        | InstantFeedbackDataSourceBaseProp.KeyExpression v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.InstantFeedbackDataSourceBase.KeyExpressionProperty, box v)
        | InstantFeedbackDataSourceBaseProp.QueryableSource v -> el.SetValue(DevExpress.Xpf.Core.ServerMode.InstantFeedbackDataSourceBase.QueryableSourceProperty, box v)
        | InstantFeedbackDataSourceBaseProp.OnGetQueryable h -> el.GetQueryable.AddHandler(h)
        | InstantFeedbackDataSourceBaseProp.OnDismissQueryable h -> el.DismissQueryable.AddHandler(h)
