...
type FloatingContainerProp =
    | Base of BaseFloatingContainerProp
    | ContainerStartupLocation of System.Windows.WindowStartupLocation
    | ContentTemplate of System.Windows.DataTemplate
    | AllowMoving of bool
    | AllowSizing of bool
    | AllowShowAnimations of bool
    | Caption of string
    | ContainerFocusable of bool
    | Icon of System.Windows.Media.ImageSource
    | CloseOnEscape of bool
    | ShowModal of bool
    | ShowActivated of bool
    | DialogResult of obj
    | LogicalOwner of System.Windows.FrameworkElement
    | ShowCloseButton of bool
    | SizeToContent of System.Windows.SizeToContent
    | OnHidden of System.Windows.RoutedEventHandler
    | OnHiding of DevExpress.Xpf.Core.CancelRoutedEventHandler

module FloatingContainer =
    let containerStartupLocation v : obj = box (FloatingContainerProp.ContainerStartupLocation v)
    let contentTemplate v : obj = box (FloatingContainerProp.ContentTemplate v)
    let allowMoving v : obj = box (FloatingContainerProp.AllowMoving v)
    let allowSizing v : obj = box (FloatingContainerProp.AllowSizing v)
    let allowShowAnimations v : obj = box (FloatingContainerProp.AllowShowAnimations v)
    let caption v : obj = box (FloatingContainerProp.Caption v)
    let containerFocusable v : obj = box (FloatingContainerProp.ContainerFocusable v)
    let icon v : obj = box (FloatingContainerProp.Icon v)
    let closeOnEscape v : obj = box (FloatingContainerProp.CloseOnEscape v)
    let showModal v : obj = box (FloatingContainerProp.ShowModal v)
    let showActivated v : obj = box (FloatingContainerProp.ShowActivated v)
    let dialogResult v : obj = box (FloatingContainerProp.DialogResult v)
    let logicalOwner v : obj = box (FloatingContainerProp.LogicalOwner v)
    let showCloseButton v : obj = box (FloatingContainerProp.ShowCloseButton v)
    let sizeToContent v : obj = box (FloatingContainerProp.SizeToContent v)
    let onHidden v : obj = box (EventProp(box (FloatingContainerProp.OnHidden v)))
    let onHiding v : obj = box (EventProp(box (FloatingContainerProp.OnHiding v)))

    let apply (el: DevExpress.Xpf.Core.FloatingContainer) (prop: FloatingContainerProp) =
        match prop with
        | FloatingContainerProp.Base p -> BaseFloatingContainer.apply el p
        | FloatingContainerProp.ContainerStartupLocation v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.ContainerStartupLocationProperty, box v)
        | FloatingContainerProp.ContentTemplate v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.ContentTemplateProperty, box v)
        | FloatingContainerProp.AllowMoving v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.AllowMovingProperty, box v)
        | FloatingContainerProp.AllowSizing v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.AllowSizingProperty, box v)
        | FloatingContainerProp.AllowShowAnimations v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.AllowShowAnimationsProperty, box v)
        | FloatingContainerProp.Caption v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.CaptionProperty, box v)
        | FloatingContainerProp.ContainerFocusable v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.ContainerFocusableProperty, box v)
        | FloatingContainerProp.Icon v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.IconProperty, box v)
        | FloatingContainerProp.CloseOnEscape v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.CloseOnEscapeProperty, box v)
        | FloatingContainerProp.ShowModal v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.ShowModalProperty, box v)
        | FloatingContainerProp.ShowActivated v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.ShowActivatedProperty, box v)
        | FloatingContainerProp.DialogResult v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.DialogResultProperty, box v)
        | FloatingContainerProp.LogicalOwner v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.LogicalOwnerProperty, box v)
        | FloatingContainerProp.ShowCloseButton v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.ShowCloseButtonProperty, box v)
        | FloatingContainerProp.SizeToContent v -> el.SetValue(DevExpress.Xpf.Core.FloatingContainer.SizeToContentProperty, box v)
        | FloatingContainerProp.OnHidden h -> el.Hidden.AddHandler(h)
        | FloatingContainerProp.OnHiding h -> el.Hiding.AddHandler(h)

    let floatLocation v : obj = box (BaseFloatingContainerProp.FloatLocation v)
    let floatSize v : obj = box (BaseFloatingContainerProp.FloatSize v)
    let isOpen v : obj = box (BaseFloatingContainerProp.IsOpen v)
    let content v : obj = box (BaseFloatingContainerProp.Content v)
    let containerTemplate v : obj = box (BaseFloatingContainerProp.ContainerTemplate v)
    let showContentOnly v : obj = box (BaseFloatingContainerProp.ShowContentOnly v)
    let owner v : obj = box (BaseFloatingContainerProp.Owner v)
    ...
    let isActive (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.FloatingContainer.IsActiveProperty, box v)) :: node.Props }
    let floatingContainer (v: DevExpress.Xpf.Core.FloatingContainer) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.FloatingContainer.FloatingContainerProperty, box v)) :: node.Props }
    let isMaximized (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.FloatingContainer.IsMaximizedProperty, box v)) :: node.Props }
    let dialogOwner (v: DevExpress.Xpf.Core.IDialogOwner) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Core.FloatingContainer.DialogOwnerProperty, box v)) :: node.Props }
