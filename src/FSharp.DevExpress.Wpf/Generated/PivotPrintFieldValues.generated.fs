...
type PivotPrintFieldValuesProp =
    | Base of FieldValuesPresenterProp
    | PrintItems of obj
    | HasTopBorder of bool

module PivotPrintFieldValues =
    let printItems v : obj = box (PivotPrintFieldValuesProp.PrintItems v)
    let hasTopBorder v : obj = box (PivotPrintFieldValuesProp.HasTopBorder v)

    let apply (el: DevExpress.Xpf.PivotGrid.Printing.PivotPrintFieldValues) (prop: PivotPrintFieldValuesProp) =
        match prop with
        | PivotPrintFieldValuesProp.Base p -> FieldValuesPresenter.apply el p
        | PivotPrintFieldValuesProp.PrintItems v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintFieldValues.PrintItemsProperty, box v)
        | PivotPrintFieldValuesProp.HasTopBorder v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintFieldValues.HasTopBorderProperty, box v)

    let isColumn v : obj = box (FieldValuesPresenterProp.IsColumn v)
    let currentWidth v : obj = box (FieldValuesPresenterProp.CurrentWidth v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.Printing.PivotPrintFieldValues>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
