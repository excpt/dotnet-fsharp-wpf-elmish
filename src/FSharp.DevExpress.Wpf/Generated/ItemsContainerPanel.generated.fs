...
type ItemsContainerPanelProp =
    | ItemTemplate of System.Windows.DataTemplate

module ItemsContainerPanel =
    let itemTemplate v : obj = box (ItemsContainerPanelProp.ItemTemplate v)

    let apply (el: DevExpress.Xpf.Scheduling.Panels.ItemsContainerPanel) (prop: ItemsContainerPanelProp) =
        match prop with
        | ItemsContainerPanelProp.ItemTemplate v -> el.SetValue(DevExpress.Xpf.Scheduling.Panels.ItemsContainerPanel.ItemTemplateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduling.Panels.ItemsContainerPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
