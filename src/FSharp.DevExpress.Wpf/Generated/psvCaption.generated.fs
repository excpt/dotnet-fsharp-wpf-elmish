...
type psvCaptionProp =
    ...
    | RecognizeAccessKey of bool

module psvCaption =
    let text v : obj = box (psvCaptionProp.Text v)
    let textTrimming v : obj = box (psvCaptionProp.TextTrimming v)
    let textWrapping v : obj = box (psvCaptionProp.TextWrapping v)
    let recognizeAccessKey v : obj = box (psvCaptionProp.RecognizeAccessKey v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.psvCaption) (prop: psvCaptionProp) =
        match prop with
        | psvCaptionProp.Text v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.psvCaption.TextProperty, box v)
        | psvCaptionProp.TextTrimming v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.psvCaption.TextTrimmingProperty, box v)
        | psvCaptionProp.TextWrapping v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.psvCaption.TextWrappingProperty, box v)
        | psvCaptionProp.RecognizeAccessKey v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.psvCaption.RecognizeAccessKeyProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.psvCaption>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
