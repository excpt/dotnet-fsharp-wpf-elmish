...
open System
open System.Windows
open DevExpress.Xpf.Scheduler
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type AppointmentMappingProp =
    | Start of string
    | AppointmentId of string
    | AllDay of string
    | Description of string
    | End of string
    | Label of string
    | Location of string
    | RecurrenceInfo of string
    | ReminderInfo of string
...
module AppointmentMapping =
    let start v : obj = box (AppointmentMappingProp.Start v)
    let appointmentId v : obj = box (AppointmentMappingProp.AppointmentId v)
    let allDay v : obj = box (AppointmentMappingProp.AllDay v)
    let description v : obj = box (AppointmentMappingProp.Description v)
    let end v : obj = box (AppointmentMappingProp.End v)
    let label v : obj = box (AppointmentMappingProp.Label v)
    let location v : obj = box (AppointmentMappingProp.Location v)
    let recurrenceInfo v : obj = box (AppointmentMappingProp.RecurrenceInfo v)
    let reminderInfo v : obj = box (AppointmentMappingProp.ReminderInfo v)
    let resourceId v : obj = box (AppointmentMappingProp.ResourceId v)
    let status v : obj = box (AppointmentMappingProp.Status v)
    let subject v : obj = box (AppointmentMappingProp.Subject v)
    let type v : obj = box (AppointmentMappingProp.Type v)
    let timeZoneId v : obj = box (AppointmentMappingProp.TimeZoneId v)

    let apply (el: DevExpress.Xpf.Scheduler.AppointmentMapping) (prop: AppointmentMappingProp) =
        match prop with
        | AppointmentMappingProp.Start v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.StartProperty, box v)
        | AppointmentMappingProp.AppointmentId v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.AppointmentIdProperty, box v)
        | AppointmentMappingProp.AllDay v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.AllDayProperty, box v)
        | AppointmentMappingProp.Description v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.DescriptionProperty, box v)
        | AppointmentMappingProp.End v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.EndProperty, box v)
        | AppointmentMappingProp.Label v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.LabelProperty, box v)
        | AppointmentMappingProp.Location v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.LocationProperty, box v)
        | AppointmentMappingProp.RecurrenceInfo v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.RecurrenceInfoProperty, box v)
        | AppointmentMappingProp.ReminderInfo v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.ReminderInfoProperty, box v)
        | AppointmentMappingProp.ResourceId v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.ResourceIdProperty, box v)
        | AppointmentMappingProp.Status v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.StatusProperty, box v)
        | AppointmentMappingProp.Subject v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.SubjectProperty, box v)
        | AppointmentMappingProp.Type v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.TypeProperty, box v)
        | AppointmentMappingProp.TimeZoneId v -> el.SetValue(DevExpress.Xpf.Scheduler.AppointmentMapping.TimeZoneIdProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.AppointmentMapping>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
