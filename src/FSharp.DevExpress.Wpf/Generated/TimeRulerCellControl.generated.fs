...
type TimeRulerCellControlProp =
    | Base of ChromeBaseProp
    | HourStringFormat of string
    | MinuteStringFormat of string
    | TimeDesignatorStringFormat of string

module TimeRulerCellControl =
    let hourStringFormat v : obj = box (TimeRulerCellControlProp.HourStringFormat v)
    let minuteStringFormat v : obj = box (TimeRulerCellControlProp.MinuteStringFormat v)
    let timeDesignatorStringFormat v : obj = box (TimeRulerCellControlProp.TimeDesignatorStringFormat v)

    let apply (el: DevExpress.Xpf.Scheduling.Visual.TimeRulerCellControl) (prop: TimeRulerCellControlProp) =
        match prop with
        | TimeRulerCellControlProp.Base p -> ChromeBase.apply el p
        | TimeRulerCellControlProp.HourStringFormat v -> el.SetValue(DevExpress.Xpf.Scheduling.Visual.TimeRulerCellControl.HourStringFormatProperty, box v)
        | TimeRulerCellControlProp.MinuteStringFormat v -> el.SetValue(DevExpress.Xpf.Scheduling.Visual.TimeRulerCellControl.MinuteStringFormatProperty, box v)
        | TimeRulerCellControlProp.TimeDesignatorStringFormat v -> el.SetValue(DevExpress.Xpf.Scheduling.Visual.TimeRulerCellControl.TimeDesignatorStringFormatProperty, box v)

    let staticRenderTemplate v : obj = box (ChromeBaseProp.StaticRenderTemplate v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduling.Visual.TimeRulerCellControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
