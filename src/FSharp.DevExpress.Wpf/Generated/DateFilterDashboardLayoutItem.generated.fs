...
type DateFilterDashboardLayoutItemProp =
    | Base of DashboardLayoutItemProp

module DateFilterDashboardLayoutItem =

    let apply (el: DevExpress.DashboardWpf.DateFilterDashboardLayoutItem) (prop: DateFilterDashboardLayoutItemProp) =
        match prop with
        | DateFilterDashboardLayoutItemProp.Base p -> DashboardLayoutItem.apply el p
    ...
    let datePickerTemplate (v: System.Windows.DataTemplate) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.DashboardWpf.DateFilterDashboardLayoutItem.DatePickerTemplateProperty, box v)) :: node.Props }
    let quickButtonTemplate (v: System.Windows.DataTemplate) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.DashboardWpf.DateFilterDashboardLayoutItem.QuickButtonTemplateProperty, box v)) :: node.Props }
    let datePickerPopupTemplate (v: System.Windows.DataTemplate) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.DashboardWpf.DateFilterDashboardLayoutItem.DatePickerPopupTemplateProperty, box v)) :: node.Props }
    let dateNavigatorStyle (v: System.Windows.Style) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.DashboardWpf.DateFilterDashboardLayoutItem.DateNavigatorStyleProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.DashboardWpf.DateFilterDashboardLayoutItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
