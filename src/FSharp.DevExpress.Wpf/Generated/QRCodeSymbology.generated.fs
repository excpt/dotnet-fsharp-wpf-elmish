...
module QRCodeSymbology =
    let compactionMode v : obj = box (QRCodeSymbologyProp.CompactionMode v)
    let errorCorrectionLevel v : obj = box (QRCodeSymbologyProp.ErrorCorrectionLevel v)
    let version v : obj = box (QRCodeSymbologyProp.Version v)

    let apply (el: DevExpress.Xpf.Controls.QRCodeSymbology) (prop: QRCodeSymbologyProp) =
        match prop with
        | QRCodeSymbologyProp.CompactionMode v -> el.SetValue(DevExpress.Xpf.Controls.QRCodeSymbology.CompactionModeProperty, box v)
        | QRCodeSymbologyProp.ErrorCorrectionLevel v -> el.SetValue(DevExpress.Xpf.Controls.QRCodeSymbology.ErrorCorrectionLevelProperty, box v)
        | QRCodeSymbologyProp.Version v -> el.SetValue(DevExpress.Xpf.Controls.QRCodeSymbology.VersionProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Controls.QRCodeSymbology>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
