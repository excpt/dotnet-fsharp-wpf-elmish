...
type PdfSignaturesViewerControlProp =
    | TreeViewStyle of System.Windows.Style
    | Settings of DevExpress.Xpf.PdfViewer.PdfSignaturesViewerSettings
    | HideAfterUse of bool

module PdfSignaturesViewerControl =
    let treeViewStyle v : obj = box (PdfSignaturesViewerControlProp.TreeViewStyle v)
    let settings v : obj = box (PdfSignaturesViewerControlProp.Settings v)
    let hideAfterUse v : obj = box (PdfSignaturesViewerControlProp.HideAfterUse v)

    let apply (el: DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl) (prop: PdfSignaturesViewerControlProp) =
        match prop with
        | PdfSignaturesViewerControlProp.TreeViewStyle v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl.TreeViewStyleProperty, box v)
        | PdfSignaturesViewerControlProp.Settings v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl.SettingsProperty, box v)
        | PdfSignaturesViewerControlProp.HideAfterUse v -> el.SetValue(DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl.HideAfterUseProperty, box v)
    ...
    let signatureViewer (v: DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl.SignatureViewerProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.PdfViewer.PdfSignaturesViewerControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
