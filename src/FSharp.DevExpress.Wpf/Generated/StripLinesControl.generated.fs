...
type StripLinesControlProp =
    | IsPrinting of bool

module StripLinesControl =
    let isPrinting v : obj = box (StripLinesControlProp.IsPrinting v)

    let apply (el: DevExpress.Xpf.Gantt.Native.StripLinesControl) (prop: StripLinesControlProp) =
        match prop with
        | StripLinesControlProp.IsPrinting v -> el.SetValue(DevExpress.Xpf.Gantt.Native.StripLinesControl.IsPrintingProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gantt.Native.StripLinesControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
