...
type GanttProgressPanelProp =
    | Progress of float

module GanttProgressPanel =
    let progress v : obj = box (GanttProgressPanelProp.Progress v)

    let apply (el: DevExpress.Xpf.Gantt.GanttProgressPanel) (prop: GanttProgressPanelProp) =
        match prop with
        | GanttProgressPanelProp.Progress v -> el.SetValue(DevExpress.Xpf.Gantt.GanttProgressPanel.ProgressProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Gantt.GanttProgressPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
