...
type SizeLimitControlProp =
    | InitialWidth of float
    | InitialHeight of float

module SizeLimitControl =
    let initialWidth v : obj = box (SizeLimitControlProp.InitialWidth v)
    let initialHeight v : obj = box (SizeLimitControlProp.InitialHeight v)

    let apply (el: DevExpress.Xpf.SpellChecker.Forms.SizeLimitControl) (prop: SizeLimitControlProp) =
        match prop with
        | SizeLimitControlProp.InitialWidth v -> el.SetValue(DevExpress.Xpf.SpellChecker.Forms.SizeLimitControl.InitialWidthProperty, box v)
        | SizeLimitControlProp.InitialHeight v -> el.SetValue(DevExpress.Xpf.SpellChecker.Forms.SizeLimitControl.InitialHeightProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.SpellChecker.Forms.SizeLimitControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
