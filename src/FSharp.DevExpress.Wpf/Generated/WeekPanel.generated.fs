...
module WeekPanel =
    let isCompressed v : obj = box (WeekPanelProp.IsCompressed v)
    let compressedIndex v : obj = box (WeekPanelProp.CompressedIndex v)

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.WeekPanel) (prop: WeekPanelProp) =
        match prop with
        | WeekPanelProp.Base p -> SchedulerPanel.apply el p
        | WeekPanelProp.IsCompressed v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.WeekPanel.IsCompressedProperty, box v)
        | WeekPanelProp.CompressedIndex v -> el.SetValue(DevExpress.Xpf.Scheduler.Drawing.WeekPanel.CompressedIndexProperty, box v)
