...
type RadialMenuItemsPanelProp =
    | FirstSectorIndex of int

module RadialMenuItemsPanel =
    let firstSectorIndex v : obj = box (RadialMenuItemsPanelProp.FirstSectorIndex v)

    let apply (el: DevExpress.Xpf.Bars.RadialMenuItemsPanel) (prop: RadialMenuItemsPanelProp) =
        match prop with
        | RadialMenuItemsPanelProp.FirstSectorIndex v -> el.SetValue(DevExpress.Xpf.Bars.RadialMenuItemsPanel.FirstSectorIndexProperty, box v)
    ...
    let angle (v: float) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Bars.RadialMenuItemsPanel.AngleProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Bars.RadialMenuItemsPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
