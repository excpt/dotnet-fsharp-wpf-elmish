...
open System
open System.Windows
open DevExpress.Xpf.Dialogs
open FSharp.Windows.Dsl

[<RequireQualifiedAccess>]
type FileDialogWindowProp =
    | Base of DialogWindowProp
    | NavigationPaneMinWidth of float
    | NavigationPaneWidth of float
    | NavigationPaneMaxWidth of float

module FileDialogWindow =
    let navigationPaneMinWidth v : obj = box (FileDialogWindowProp.NavigationPaneMinWidth v)
    let navigationPaneWidth v : obj = box (FileDialogWindowProp.NavigationPaneWidth v)
    let navigationPaneMaxWidth v : obj = box (FileDialogWindowProp.NavigationPaneMaxWidth v)

    let apply (el: DevExpress.Xpf.Dialogs.FileDialogWindow) (prop: FileDialogWindowProp) =
        match prop with
        | FileDialogWindowProp.Base p -> DialogWindow.apply el p
        | FileDialogWindowProp.NavigationPaneMinWidth v -> el.SetValue(DevExpress.Xpf.Dialogs.FileDialogWindow.NavigationPaneMinWidthProperty, box v)
        | FileDialogWindowProp.NavigationPaneWidth v -> el.SetValue(DevExpress.Xpf.Dialogs.FileDialogWindow.NavigationPaneWidthProperty, box v)
        | FileDialogWindowProp.NavigationPaneMaxWidth v -> el.SetValue(DevExpress.Xpf.Dialogs.FileDialogWindow.NavigationPaneMaxWidthProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Dialogs.FileDialogWindow>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
