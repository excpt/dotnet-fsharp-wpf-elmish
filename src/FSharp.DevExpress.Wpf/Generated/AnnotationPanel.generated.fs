...
type AnnotationPanelProp =
    | AnnotationItem of DevExpress.Xpf.Charts.AnnotationItem

module AnnotationPanel =
    let annotationItem v : obj = box (AnnotationPanelProp.AnnotationItem v)

    let apply (el: DevExpress.Xpf.Charts.AnnotationPanel) (prop: AnnotationPanelProp) =
        match prop with
        | AnnotationPanelProp.AnnotationItem v -> el.SetValue(DevExpress.Xpf.Charts.AnnotationPanel.AnnotationItemProperty, box v)
    ...
    let elementType (v: DevExpress.Xpf.Charts.AnnotationElementType) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Charts.AnnotationPanel.ElementTypeProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Charts.AnnotationPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
