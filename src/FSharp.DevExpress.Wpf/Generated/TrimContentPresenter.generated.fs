...
type TrimContentPresenterProp =
    | TrimmingMode of DevExpress.Xpf.Core.Native.TrimmingMode
    | TrimmingSize of float
    | DisableToolTipWhenFullyVisible of bool
    | ToolTipOwner of System.Windows.FrameworkElement

module TrimContentPresenter =
    let trimmingMode v : obj = box (TrimContentPresenterProp.TrimmingMode v)
    let trimmingSize v : obj = box (TrimContentPresenterProp.TrimmingSize v)
    let disableToolTipWhenFullyVisible v : obj = box (TrimContentPresenterProp.DisableToolTipWhenFullyVisible v)
    let toolTipOwner v : obj = box (TrimContentPresenterProp.ToolTipOwner v)

    let apply (el: DevExpress.Xpf.Core.Native.TrimContentPresenter) (prop: TrimContentPresenterProp) =
        match prop with
        | TrimContentPresenterProp.TrimmingMode v -> el.SetValue(DevExpress.Xpf.Core.Native.TrimContentPresenter.TrimmingModeProperty, box v)
        | TrimContentPresenterProp.TrimmingSize v -> el.SetValue(DevExpress.Xpf.Core.Native.TrimContentPresenter.TrimmingSizeProperty, box v)
        | TrimContentPresenterProp.DisableToolTipWhenFullyVisible v -> el.SetValue(DevExpress.Xpf.Core.Native.TrimContentPresenter.DisableToolTipWhenFullyVisibleProperty, box v)
        | TrimContentPresenterProp.ToolTipOwner v -> el.SetValue(DevExpress.Xpf.Core.Native.TrimContentPresenter.ToolTipOwnerProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.Native.TrimContentPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
