...
type AnimatedPanelProp =
    | Alignment of DevExpress.Xpf.WindowsUI.AppBarAlignment
    | IsAnimationEnabled of bool
    | ItemSpacing of float

module AnimatedPanel =
    let alignment v : obj = box (AnimatedPanelProp.Alignment v)
    let isAnimationEnabled v : obj = box (AnimatedPanelProp.IsAnimationEnabled v)
    let itemSpacing v : obj = box (AnimatedPanelProp.ItemSpacing v)

    let apply (el: DevExpress.Xpf.WindowsUI.Internal.AnimatedPanel) (prop: AnimatedPanelProp) =
        match prop with
        | AnimatedPanelProp.Alignment v -> el.SetValue(DevExpress.Xpf.WindowsUI.Internal.AnimatedPanel.AlignmentProperty, box v)
        | AnimatedPanelProp.IsAnimationEnabled v -> el.SetValue(DevExpress.Xpf.WindowsUI.Internal.AnimatedPanel.IsAnimationEnabledProperty, box v)
        | AnimatedPanelProp.ItemSpacing v -> el.SetValue(DevExpress.Xpf.WindowsUI.Internal.AnimatedPanel.ItemSpacingProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.Internal.AnimatedPanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
