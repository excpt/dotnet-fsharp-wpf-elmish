...
type LayoutControlBaseProp =
    | Base of ScrollControlProp
    | ItemSpace of float
    | Padding of System.Windows.Thickness

module LayoutControlBase =
    let itemSpace v : obj = box (LayoutControlBaseProp.ItemSpace v)
    let padding v : obj = box (LayoutControlBaseProp.Padding v)

    let apply (el: DevExpress.Xpf.LayoutControl.LayoutControlBase) (prop: LayoutControlBaseProp) =
        match prop with
        | LayoutControlBaseProp.Base p -> ScrollControl.apply el p
        | LayoutControlBaseProp.ItemSpace v -> el.SetValue(DevExpress.Xpf.LayoutControl.LayoutControlBase.ItemSpaceProperty, box v)
        | LayoutControlBaseProp.Padding v -> el.SetValue(DevExpress.Xpf.LayoutControl.LayoutControlBase.PaddingProperty, box v)
