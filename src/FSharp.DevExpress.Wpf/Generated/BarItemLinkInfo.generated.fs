...
type BarItemLinkInfoProp =
    | Index of int
    | Altitude of int
    | IsRemoved of bool
    | IsInSimplifiedRibbon of bool

module BarItemLinkInfo =
    let index v : obj = box (BarItemLinkInfoProp.Index v)
    let altitude v : obj = box (BarItemLinkInfoProp.Altitude v)
    let isRemoved v : obj = box (BarItemLinkInfoProp.IsRemoved v)
    let isInSimplifiedRibbon v : obj = box (BarItemLinkInfoProp.IsInSimplifiedRibbon v)

    let apply (el: DevExpress.Xpf.Bars.BarItemLinkInfo) (prop: BarItemLinkInfoProp) =
        match prop with
        | BarItemLinkInfoProp.Index v -> el.SetValue(DevExpress.Xpf.Bars.BarItemLinkInfo.IndexProperty, box v)
        | BarItemLinkInfoProp.Altitude v -> el.SetValue(DevExpress.Xpf.Bars.BarItemLinkInfo.AltitudeProperty, box v)
        | BarItemLinkInfoProp.IsRemoved v -> el.SetValue(DevExpress.Xpf.Bars.BarItemLinkInfo.IsRemovedProperty, box v)
        | BarItemLinkInfoProp.IsInSimplifiedRibbon v -> el.SetValue(DevExpress.Xpf.Bars.BarItemLinkInfo.IsInSimplifiedRibbonProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.BarItemLinkInfo>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
