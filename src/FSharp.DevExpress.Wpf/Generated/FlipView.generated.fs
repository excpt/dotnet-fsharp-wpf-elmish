...
type FlipViewProp =
    ...
    | IsManipulationInertiaEnabled of bool
    | Orientation of System.Windows.Controls.Orientation
    | ShowNextButton of bool
    | ShowPreviousButton of bool
    | NextCommand of System.Windows.Input.ICommand
    | PreviousCommand of System.Windows.Input.ICommand
    | ItemCacheMode of DevExpress.Xpf.WindowsUI.ItemCacheMode
    | ItemSizeMode of DevExpress.Xpf.WindowsUI.ItemSizeMode

module FlipView =
    let isAnimationEnabled v : obj = box (FlipViewProp.IsAnimationEnabled v)
    let isManipulationInertiaEnabled v : obj = box (FlipViewProp.IsManipulationInertiaEnabled v)
    let orientation v : obj = box (FlipViewProp.Orientation v)
    let showNextButton v : obj = box (FlipViewProp.ShowNextButton v)
    let showPreviousButton v : obj = box (FlipViewProp.ShowPreviousButton v)
    let nextCommand v : obj = box (FlipViewProp.NextCommand v)
    let previousCommand v : obj = box (FlipViewProp.PreviousCommand v)
    let itemCacheMode v : obj = box (FlipViewProp.ItemCacheMode v)
    let itemSizeMode v : obj = box (FlipViewProp.ItemSizeMode v)

    let apply (el: DevExpress.Xpf.WindowsUI.FlipView) (prop: FlipViewProp) =
        match prop with
        | FlipViewProp.Base p -> veSelector.apply el p
        | FlipViewProp.IsAnimationEnabled v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.IsAnimationEnabledProperty, box v)
        | FlipViewProp.IsManipulationInertiaEnabled v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.IsManipulationInertiaEnabledProperty, box v)
        | FlipViewProp.Orientation v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.OrientationProperty, box v)
        | FlipViewProp.ShowNextButton v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.ShowNextButtonProperty, box v)
        | FlipViewProp.ShowPreviousButton v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.ShowPreviousButtonProperty, box v)
        | FlipViewProp.NextCommand v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.NextCommandProperty, box v)
        | FlipViewProp.PreviousCommand v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.PreviousCommandProperty, box v)
        | FlipViewProp.ItemCacheMode v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.ItemCacheModeProperty, box v)
        | FlipViewProp.ItemSizeMode v -> el.SetValue(DevExpress.Xpf.WindowsUI.FlipView.ItemSizeModeProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.WindowsUI.FlipView>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
