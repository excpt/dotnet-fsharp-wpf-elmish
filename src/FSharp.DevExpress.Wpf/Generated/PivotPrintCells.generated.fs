...
type PivotPrintCellsProp =
    | Base of CellsAreaPresenterProp
    | Bottom of int
    | Right of int
    | PageIndex of int

module PivotPrintCells =
    let bottom v : obj = box (PivotPrintCellsProp.Bottom v)
    let right v : obj = box (PivotPrintCellsProp.Right v)
    let pageIndex v : obj = box (PivotPrintCellsProp.PageIndex v)

    let apply (el: DevExpress.Xpf.PivotGrid.Printing.PivotPrintCells) (prop: PivotPrintCellsProp) =
        match prop with
        | PivotPrintCellsProp.Base p -> CellsAreaPresenter.apply el p
        | PivotPrintCellsProp.Bottom v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintCells.BottomProperty, box v)
        | PivotPrintCellsProp.Right v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintCells.RightProperty, box v)
        | PivotPrintCellsProp.PageIndex v -> el.SetValue(DevExpress.Xpf.PivotGrid.Printing.PivotPrintCells.PageIndexProperty, box v)

    let rowsLeft v : obj = box (CellsAreaPresenterProp.RowsLeft v)
    let rowsLeftOffset v : obj = box (CellsAreaPresenterProp.RowsLeftOffset v)
    ...
    let onLeftTopChanged v : obj = box (EventProp(box (CellsAreaPresenterProp.OnLeftTopChanged v)))
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.Printing.PivotPrintCells>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
