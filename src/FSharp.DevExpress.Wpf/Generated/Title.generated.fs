...
type TitleProp =
    | Base of TitleBaseProp
    | Dock of System.Windows.Controls.Dock

module Title =
    let dock v : obj = box (TitleProp.Dock v)

    let apply (el: DevExpress.Xpf.Charts.Title) (prop: TitleProp) =
        match prop with
        | TitleProp.Base p -> TitleBase.apply el p
        | TitleProp.Dock v -> el.SetValue(DevExpress.Xpf.Charts.Title.DockProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.Title>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
