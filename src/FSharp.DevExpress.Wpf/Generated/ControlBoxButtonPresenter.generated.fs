...
type ControlBoxButtonPresenterProp =
    | Base of psvContentPresenterProp
    | CommandParameter of obj
    | Command of System.Windows.Input.ICommand
    | CommandTarget of System.Windows.IInputElement

module ControlBoxButtonPresenter =
    let commandParameter v : obj = box (ControlBoxButtonPresenterProp.CommandParameter v)
    let command v : obj = box (ControlBoxButtonPresenterProp.Command v)
    let commandTarget v : obj = box (ControlBoxButtonPresenterProp.CommandTarget v)

    let apply (el: DevExpress.Xpf.Docking.VisualElements.ControlBoxButtonPresenter) (prop: ControlBoxButtonPresenterProp) =
        match prop with
        | ControlBoxButtonPresenterProp.Base p -> psvContentPresenter.apply el p
        | ControlBoxButtonPresenterProp.CommandParameter v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.ControlBoxButtonPresenter.CommandParameterProperty, box v)
        | ControlBoxButtonPresenterProp.Command v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.ControlBoxButtonPresenter.CommandProperty, box v)
        | ControlBoxButtonPresenterProp.CommandTarget v -> el.SetValue(DevExpress.Xpf.Docking.VisualElements.ControlBoxButtonPresenter.CommandTargetProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Docking.VisualElements.ControlBoxButtonPresenter>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
