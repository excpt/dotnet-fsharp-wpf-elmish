...
type ItemsLayerLegendProp =
    | Base of DataProviderLegendProp
    | Layer of DevExpress.Xpf.Map.LayerBase

module ItemsLayerLegend =
    let layer v : obj = box (ItemsLayerLegendProp.Layer v)

    let apply (el: DevExpress.Xpf.Map.ItemsLayerLegend) (prop: ItemsLayerLegendProp) =
        match prop with
        | ItemsLayerLegendProp.Base p -> DataProviderLegend.apply el p
        | ItemsLayerLegendProp.Layer v -> el.SetValue(DevExpress.Xpf.Map.ItemsLayerLegend.LayerProperty, box v)
