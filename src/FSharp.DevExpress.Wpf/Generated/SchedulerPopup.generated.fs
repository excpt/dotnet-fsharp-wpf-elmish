...
type SchedulerPopupProp =
    | Base of PopupBaseProp

module SchedulerPopup =

    let apply (el: DevExpress.Xpf.Scheduler.Drawing.SchedulerPopup) (prop: SchedulerPopupProp) =
        match prop with
        | SchedulerPopupProp.Base p -> PopupBase.apply el p
    ...
    let container (v: DevExpress.Xpf.Scheduler.Drawing.SchedulerPopup) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Scheduler.Drawing.SchedulerPopup.ContainerProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Scheduler.Drawing.SchedulerPopup>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
