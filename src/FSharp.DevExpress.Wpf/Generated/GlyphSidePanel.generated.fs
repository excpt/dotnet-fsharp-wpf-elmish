...
type GlyphSidePanelProp =
    | SubMenu of DevExpress.Xpf.Bars.SubMenuBarControl

module GlyphSidePanel =
    let subMenu v : obj = box (GlyphSidePanelProp.SubMenu v)

    let apply (el: DevExpress.Xpf.Bars.GlyphSidePanel) (prop: GlyphSidePanelProp) =
        match prop with
        | GlyphSidePanelProp.SubMenu v -> el.SetValue(DevExpress.Xpf.Bars.GlyphSidePanel.SubMenuProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Bars.GlyphSidePanel>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
