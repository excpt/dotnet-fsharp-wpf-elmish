...
type FieldValueElementProp =
    | Base of DataAreaElementProp
    | AllowVerticalSizing of bool
    | AllowHorizontalSizing of bool

module FieldValueElement =
    let allowVerticalSizing v : obj = box (FieldValueElementProp.AllowVerticalSizing v)
    let allowHorizontalSizing v : obj = box (FieldValueElementProp.AllowHorizontalSizing v)

    let apply (el: DevExpress.Xpf.PivotGrid.FieldValueElement) (prop: FieldValueElementProp) =
        match prop with
        | FieldValueElementProp.Base p -> DataAreaElement.apply el p
        | FieldValueElementProp.AllowVerticalSizing v -> el.SetValue(DevExpress.Xpf.PivotGrid.FieldValueElement.AllowVerticalSizingProperty, box v)
        | FieldValueElementProp.AllowHorizontalSizing v -> el.SetValue(DevExpress.Xpf.PivotGrid.FieldValueElement.AllowHorizontalSizingProperty, box v)

    let valueItem v : obj = box (DataAreaElementProp.ValueItem v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.PivotGrid.FieldValueElement>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
