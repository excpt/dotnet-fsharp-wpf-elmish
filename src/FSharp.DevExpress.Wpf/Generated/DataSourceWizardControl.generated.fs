...
type DataSourceWizardControlProp =
    | Base of DataSourceWizardControlBaseProp
    | DataSourceWizardExtensions of DevExpress.Xpf.DataAccess.Native.IDataSourceWizardExtensions
    | ParameterService of DevExpress.DataAccess.Wizard.Services.IParameterService
    | DataSourceModel of DevExpress.DataAccess.Wizard.Model.IDataSourceModel

module DataSourceWizardControl =
    let dataSourceWizardExtensions v : obj = box (DataSourceWizardControlProp.DataSourceWizardExtensions v)
    let parameterService v : obj = box (DataSourceWizardControlProp.ParameterService v)
    let dataSourceModel v : obj = box (DataSourceWizardControlProp.DataSourceModel v)

    let apply (el: DevExpress.Xpf.DataAccess.DataSourceWizard.DataSourceWizardControl) (prop: DataSourceWizardControlProp) =
        match prop with
        | DataSourceWizardControlProp.Base p -> DataSourceWizardControlBase.apply el p
        | DataSourceWizardControlProp.DataSourceWizardExtensions v -> el.SetValue(DevExpress.Xpf.DataAccess.DataSourceWizard.DataSourceWizardControl.DataSourceWizardExtensionsProperty, box v)
        | DataSourceWizardControlProp.ParameterService v -> el.SetValue(DevExpress.Xpf.DataAccess.DataSourceWizard.DataSourceWizardControl.ParameterServiceProperty, box v)
        | DataSourceWizardControlProp.DataSourceModel v -> el.SetValue(DevExpress.Xpf.DataAccess.DataSourceWizard.DataSourceWizardControl.DataSourceModelProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.DataAccess.DataSourceWizard.DataSourceWizardControl>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
