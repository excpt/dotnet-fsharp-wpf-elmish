...
module WilliamsR =
    let pointsCount v : obj = box (WilliamsRProp.PointsCount v)

    let apply (el: DevExpress.Xpf.Charts.WilliamsR) (prop: WilliamsRProp) =
        match prop with
        | WilliamsRProp.Base p -> SeparatePaneIndicatorWithPoints.apply el p
        | WilliamsRProp.PointsCount v -> el.SetValue(DevExpress.Xpf.Charts.WilliamsR.PointsCountProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Charts.WilliamsR>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
