...
type DXWindowHeaderProp =
    | IsAeroModeEnabled of bool

module DXWindowHeader =
    let isAeroModeEnabled v : obj = box (DXWindowHeaderProp.IsAeroModeEnabled v)

    let apply (el: DevExpress.Xpf.Core.DXWindowHeader) (prop: DXWindowHeaderProp) =
        match prop with
        | DXWindowHeaderProp.IsAeroModeEnabled v -> el.SetValue(DevExpress.Xpf.Core.DXWindowHeader.IsAeroModeEnabledProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Core.DXWindowHeader>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
