...
type BaseTabHintProp =
    | Base of psvControlProp
    | TabHeaderLocation of System.Windows.Controls.Dock

module BaseTabHint =
    let tabHeaderLocation v : obj = box (BaseTabHintProp.TabHeaderLocation v)

    let apply (el: DevExpress.Xpf.Docking.Platform.BaseTabHint) (prop: BaseTabHintProp) =
        match prop with
        | BaseTabHintProp.Base p -> psvControl.apply el p
        | BaseTabHintProp.TabHeaderLocation v -> el.SetValue(DevExpress.Xpf.Docking.Platform.BaseTabHint.TabHeaderLocationProperty, box v)
