...
type WizardProp =
    ...
    | BackwardNavigation of DevExpress.Xpf.Controls.BackwardNavigation
    | DefaultButton of DevExpress.Xpf.Controls.WizardDefaultButton
    ...
    | ShowFinish of bool
    | ShowNext of bool
    | DefaultFocus of DevExpress.Xpf.Controls.WizardDefaultFocus

module Wizard =
    let animationSelector v : obj = box (WizardProp.AnimationSelector v)
    let animationType v : obj = box (WizardProp.AnimationType v)
    let backwardNavigation v : obj = box (WizardProp.BackwardNavigation v)
    let defaultButton v : obj = box (WizardProp.DefaultButton v)
    let footerTemplate v : obj = box (WizardProp.FooterTemplate v)
    let allowBack v : obj = box (WizardProp.AllowBack v)
    let allowCancel v : obj = box (WizardProp.AllowCancel v)
    let allowFinish v : obj = box (WizardProp.AllowFinish v)
    let allowNext v : obj = box (WizardProp.AllowNext v)
    let showBack v : obj = box (WizardProp.ShowBack v)
    let showCancel v : obj = box (WizardProp.ShowCancel v)
    let showFinish v : obj = box (WizardProp.ShowFinish v)
    let showNext v : obj = box (WizardProp.ShowNext v)
    let defaultFocus v : obj = box (WizardProp.DefaultFocus v)

    let apply (el: DevExpress.Xpf.Controls.Wizard) (prop: WizardProp) =
        match prop with
        | WizardProp.AnimationSelector v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.AnimationSelectorProperty, box v)
        | WizardProp.AnimationType v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.AnimationTypeProperty, box v)
        | WizardProp.BackwardNavigation v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.BackwardNavigationProperty, box v)
        | WizardProp.DefaultButton v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.DefaultButtonProperty, box v)
        | WizardProp.FooterTemplate v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.FooterTemplateProperty, box v)
        | WizardProp.AllowBack v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.AllowBackProperty, box v)
        | WizardProp.AllowCancel v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.AllowCancelProperty, box v)
        | WizardProp.AllowFinish v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.AllowFinishProperty, box v)
        | WizardProp.AllowNext v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.AllowNextProperty, box v)
        | WizardProp.ShowBack v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.ShowBackProperty, box v)
        | WizardProp.ShowCancel v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.ShowCancelProperty, box v)
        | WizardProp.ShowFinish v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.ShowFinishProperty, box v)
        | WizardProp.ShowNext v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.ShowNextProperty, box v)
        | WizardProp.DefaultFocus v -> el.SetValue(DevExpress.Xpf.Controls.Wizard.DefaultFocusProperty, box v)
    ...
    let isButtonBack (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Wizard.IsButtonBackProperty, box v)) :: node.Props }
    let isButtonNext (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Wizard.IsButtonNextProperty, box v)) :: node.Props }
    let isButtonCancel (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Wizard.IsButtonCancelProperty, box v)) :: node.Props }
    let isButtonFinish (v: bool) (node: VirtualNode) : VirtualNode =
        { node with Props = box (AttachedProp(DevExpress.Xpf.Controls.Wizard.IsButtonFinishProperty, box v)) :: node.Props }

    let create (props: obj list) : VirtualNode =
        let cs, uk, ps = VirtualTree.extractSpecialProps props
        { Type = typeof<DevExpress.Xpf.Controls.Wizard>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
