...
module GroupItem =
    let fontHeight v : obj = box (GroupItemProp.FontHeight v)
    let text v : obj = box (GroupItemProp.Text v)
    let isHovered v : obj = box (GroupItemProp.IsHovered v)
    let isPressed v : obj = box (GroupItemProp.IsPressed v)
    let isExpanded v : obj = box (GroupItemProp.IsExpanded v)
    let isCollapsed v : obj = box (GroupItemProp.IsCollapsed v)

    let apply (el: DevExpress.Xpf.Spreadsheet.Internal.GroupItem) (prop: GroupItemProp) =
        match prop with
        | GroupItemProp.FontHeight v -> el.SetValue(DevExpress.Xpf.Spreadsheet.Internal.GroupItem.FontHeightProperty, box v)
        | GroupItemProp.Text v -> el.SetValue(DevExpress.Xpf.Spreadsheet.Internal.GroupItem.TextProperty, box v)
        | GroupItemProp.IsHovered v -> el.SetValue(DevExpress.Xpf.Spreadsheet.Internal.GroupItem.IsHoveredProperty, box v)
        | GroupItemProp.IsPressed v -> el.SetValue(DevExpress.Xpf.Spreadsheet.Internal.GroupItem.IsPressedProperty, box v)
        | GroupItemProp.IsExpanded v -> el.SetValue(DevExpress.Xpf.Spreadsheet.Internal.GroupItem.IsExpandedProperty, box v)
        | GroupItemProp.IsCollapsed v -> el.SetValue(DevExpress.Xpf.Spreadsheet.Internal.GroupItem.IsCollapsedProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Spreadsheet.Internal.GroupItem>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
