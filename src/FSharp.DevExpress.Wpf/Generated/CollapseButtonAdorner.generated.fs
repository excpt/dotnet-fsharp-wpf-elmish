...
type CollapseButtonAdornerProp =
    | State of DevExpress.Diagram.Core.Native.CollapseButtonState

module CollapseButtonAdorner =
    let state v : obj = box (CollapseButtonAdornerProp.State v)

    let apply (el: DevExpress.Xpf.Diagram.Native.CollapseButtonAdorner) (prop: CollapseButtonAdornerProp) =
        match prop with
        | CollapseButtonAdornerProp.State v -> el.SetValue(DevExpress.Xpf.Diagram.Native.CollapseButtonAdorner.StateProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Diagram.Native.CollapseButtonAdorner>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
