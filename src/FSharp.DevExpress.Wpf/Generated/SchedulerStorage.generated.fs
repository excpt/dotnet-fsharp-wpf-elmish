...
type SchedulerStorageProp =
    | Base of SchedulerFrameworkElementProp
    | DateTimeSavingMode of DevExpress.XtraScheduler.DateTimeSavingMode
    | AppointmentStorage of DevExpress.Xpf.Scheduler.AppointmentStorage
    | ResourceStorage of DevExpress.Xpf.Scheduler.ResourceStorage
    | TimeZoneId of string
    | EnableTimeZones of bool
    | RemindersCheckInterval of int
    | EnableSmartFetch of bool
    | FetchOnVisibleResourcesChanged of bool

module SchedulerStorage =
    let dateTimeSavingMode v : obj = box (SchedulerStorageProp.DateTimeSavingMode v)
    let appointmentStorage v : obj = box (SchedulerStorageProp.AppointmentStorage v)
    let resourceStorage v : obj = box (SchedulerStorageProp.ResourceStorage v)
    let timeZoneId v : obj = box (SchedulerStorageProp.TimeZoneId v)
    let enableTimeZones v : obj = box (SchedulerStorageProp.EnableTimeZones v)
    let remindersCheckInterval v : obj = box (SchedulerStorageProp.RemindersCheckInterval v)
    let enableSmartFetch v : obj = box (SchedulerStorageProp.EnableSmartFetch v)
    let fetchOnVisibleResourcesChanged v : obj = box (SchedulerStorageProp.FetchOnVisibleResourcesChanged v)

    let apply (el: DevExpress.Xpf.Scheduler.SchedulerStorage) (prop: SchedulerStorageProp) =
        match prop with
        | SchedulerStorageProp.Base p -> SchedulerFrameworkElement.apply el p
        | SchedulerStorageProp.DateTimeSavingMode v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.DateTimeSavingModeProperty, box v)
        | SchedulerStorageProp.AppointmentStorage v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.AppointmentStorageProperty, box v)
        | SchedulerStorageProp.ResourceStorage v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.ResourceStorageProperty, box v)
        | SchedulerStorageProp.TimeZoneId v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.TimeZoneIdProperty, box v)
        | SchedulerStorageProp.EnableTimeZones v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.EnableTimeZonesProperty, box v)
        | SchedulerStorageProp.RemindersCheckInterval v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.RemindersCheckIntervalProperty, box v)
        | SchedulerStorageProp.EnableSmartFetch v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.EnableSmartFetchProperty, box v)
        | SchedulerStorageProp.FetchOnVisibleResourcesChanged v -> el.SetValue(DevExpress.Xpf.Scheduler.SchedulerStorage.FetchOnVisibleResourcesChangedProperty, box v)
    ...
    let create (props: obj list) : VirtualNode =
        ...
        { Type = typeof<DevExpress.Xpf.Scheduler.SchedulerStorage>
          Props = ps
          Children = cs
          UserKey = uk
          InternalId = "" }
